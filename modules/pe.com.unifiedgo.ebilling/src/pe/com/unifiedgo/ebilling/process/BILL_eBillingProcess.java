...
public class BILL_eBillingProcess extends DalBaseProcess {
  ...
  private final static String FACTURA_CON_ANTICIPO = "FV_APL_PR"; // Factura con Anticipo Aplicado
      ...
      for (int i = 0; i < settings_list.size(); i++) {
        ...
        try {
          ...
          for (int j = 0; j < invoices.length; j++) {
            ...
            Date date01Ago = df.parse("01-08-2017");
//            if (invoice.getAccountingDate().compareTo(date01Jul) >= 0) {
//              continue;
//            }     
//            if (invoice.getAccountingDate().compareTo(date01Ago) >= 0) {
//              continue;
//            }     
//            if (invoice.getDocumentStatus().compareTo("VO") == 0) {
//              continue; 	
//            }
//            if (true) {
//            	continue;
...
//            }
            ...
            if (invoices[j].type.equals(FACTURA_GUIA)) {
              ...
              String responseStatus = EbizEbill_Utils.eBizzSend_Anticipo(org_setting.getUrlsend(),
                  org_setting.getUrlsendEndpoint(), invoices[j].cInvoiceId);

              // BILLEBillingProcessData.updateeInvoiceStatus(connection, conn, responseStatus,
              // invoices[j].cInvoiceId);

            } else if (invoices[j].type.equals(FACTURA_CON_ANTICIPO)) {
              String responseStatus = EbizEbill_Utils.eBizzSend_FacturaConAnticipo(
                  org_setting.getUrlsend(), org_setting.getUrlsendEndpoint(),
                  invoices[j].cInvoiceId);
