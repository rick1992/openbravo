...
public class SCO_Internal_Doc_DocType extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strSCOInternalDocId = vars.getStringParameter("inpscoInternalDocId");
      String strDescription = vars.getStringParameter("inpdescription");
      String strTabId = vars.getStringParameter("inpTabId");

      try {
        printPage(response, vars, strDocTypeTarget, strDocType, docNo, strSCOInternalDocId, strDescription, strTabId);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strDocTypeTarget, String strDocType, String docNo, String strSCOInternalDocId, String strDescription, String strTabId) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    if (strDocTypeTarget.equals(""))
      ...
      resultado.append("var calloutName='SCO_Internal_Doc_DocType';\n\n");
      ...
      SCOInternalDocDocTypeData[] data = null;

      if (!newDocNo && !"0".equals(strDocType)) {
        data = SCOInternalDocDocTypeData.select(this, strDocType);
      ...
      SCOInternalDocDocTypeData[] dataNew = SCOInternalDocDocTypeData.select(this, strDocTypeTarget);
      if (dataNew != null && dataNew.length > 0) {
        DocSubTypeSO = dataNew[0].docsubtypeso;
        if (DocSubTypeSO == null)
          DocSubTypeSO = "--";
        String strOldDocTypeTarget = SCOInternalDocDocTypeData.selectOldDocSubType(this, strSCOInternalDocId);
        if (!DocSubTypeSO.equals("OB") && strOldDocTypeTarget.equals("OB")) {
          String strOldDocNo = SCOInternalDocDocTypeData.selectOldDocNo(this, strSCOInternalDocId);
        ...
        if (dataNew[0].isdocnocontrolled.equals("Y")) {
          if (!newDocNo && !AD_Sequence_ID.equals(dataNew[0].adSequenceId) && !SCOInternalDocDocTypeData.selectOldDocTypeTargetId(this, strSCOInternalDocId).equalsIgnoreCase(strDocTypeTarget))
            newDocNo = true;
          if (newDocNo) {
