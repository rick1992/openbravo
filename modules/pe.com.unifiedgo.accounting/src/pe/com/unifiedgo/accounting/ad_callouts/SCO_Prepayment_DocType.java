...
public class SCO_Prepayment_DocType extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strSCOPrepaymentId = vars.getStringParameter("inpscoPrepaymentId");
      String strDescription = vars.getStringParameter("inpdescription");
      String strTabId = vars.getStringParameter("inpTabId");

      try {
        printPage(response, vars, strDocTypeTarget, strDocType, docNo, strSCOPrepaymentId, strDescription, strTabId);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strDocTypeTarget, String strDocType, String docNo, String strSCOPrepaymentId, String strDescription, String strTabId) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    if (strDocTypeTarget.equals(""))
      ...
      resultado.append("var calloutName='SCO_Prepayment_DocType';\n\n");
      ...
      SCOPrepaymentDocTypeData[] data = null;

      if (!newDocNo && !"0".equals(strDocType)) {
        data = SCOPrepaymentDocTypeData.select(this, strDocType);
      ...
      SCOPrepaymentDocTypeData[] dataNew = SCOPrepaymentDocTypeData.select(this, strDocTypeTarget);
      if (dataNew != null && dataNew.length > 0) {
        DocSubTypeSO = dataNew[0].docsubtypeso;
        if (DocSubTypeSO == null)
          DocSubTypeSO = "--";
        String strOldDocTypeTarget = SCOPrepaymentDocTypeData.selectOldDocSubType(this, strSCOPrepaymentId);
        if (!DocSubTypeSO.equals("OB") && strOldDocTypeTarget.equals("OB")) {
          String strOldDocNo = SCOPrepaymentDocTypeData.selectOldDocNo(this, strSCOPrepaymentId);
        ...
        if (dataNew[0].isdocnocontrolled.equals("Y")) {
          if (!newDocNo && !AD_Sequence_ID.equals(dataNew[0].adSequenceId) && !SCOPrepaymentDocTypeData.selectOldDocTypeTargetId(this, strSCOPrepaymentId).equalsIgnoreCase(strDocTypeTarget))
            newDocNo = true;
          if (newDocNo) {
