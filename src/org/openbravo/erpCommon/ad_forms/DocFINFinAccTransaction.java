...
public class DocFINFinAccTransaction extends AcctServer {
  ...
  public FieldProviderFactory[] loadLinesPaymentDetailsFieldProvider(
    ...
    try {
      for (int i = 0; i < data.length; i++) {
        ...
        // Ni entregas ni anticipo generan transaccion
        ...
        if (payment.isReceipt()) {
          depositAmt = amountAndWriteOff.get("amountpay");
          paymentAmt = BigDecimal.ZERO;
          if (amountAndWriteOff.get("amountpay").compareTo(BigDecimal.ZERO) < 0) {
            depositAmt = BigDecimal.ZERO;
            paymentAmt = amountAndWriteOff.get("amountpay").negate();
          }

        } else {
          paymentAmt = amountAndWriteOff.get("amountpay");
          depositAmt = BigDecimal.ZERO;
          if (amountAndWriteOff.get("amountpay").compareTo(BigDecimal.ZERO) < 0) {
            paymentAmt = BigDecimal.ZERO;
            depositAmt = amountAndWriteOff.get("amountpay").negate();
          }
        }
        ...
        FieldProviderFactory.setField(data[i], "DepositAmount", depositAmt.toString());// transaction.getDepositAmount().toString());
        FieldProviderFactory.setField(data[i], "PaymentAmount", paymentAmt.toString());// transaction.getPaymentAmount().toString());
        ...
        // Check if payment against invoice is in a previous date than
        ...
        FieldProviderFactory.setField(data[i], "cCurrencyId",
            paymentDetails.get(i).getScoDocCurrency().getId());
  ...
  public FieldProviderFactory[] loadLinesGLItemFieldProvider(FIN_FinaccTransaction transaction) {
    ...
    try {
      ...
      if (transaction.getScoInternalDoc() != null) {
        FieldProviderFactory.setField(data[0], "internalDoc",
            transaction.getScoInternalDoc().getId());
      } else {
        FieldProviderFactory.setField(data[0], "internalDoc", "");
      }

      if (transaction.getScoInvoiceGlref() != null) {
        FieldProviderFactory.setField(data[0], "invoiceRef",
            transaction.getScoInvoiceGlref().getId());
      } else {
        FieldProviderFactory.setField(data[0], "invoiceRef", "");
      }

      FieldProviderFactory.setField(data[0], "scoEeffCashflowId", "");
  ...
  private DocLine[] loadLines() {
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      try {
        ...
        docLine.setCurrency(data[i].getField("cCurrencyId"));
        ...
        if (invoice != null) {
          ...
          if (internalDoc != null && !internalDoc.equals("")) {
          ...
          if (invoiceRef != null && !invoiceRef.equals("")) {
  ...
  public Fact createFact(AcctSchema as, ConnectionProvider conn, Connection con,
    ...
    try {
      ...
      isReceiptForConversion = false; // todo bancos es T/C compra

      // 3 Scenarios: 1st Bank fee 2nd payment related transaction 3rd
      // glitem transaction
  ...
  public Fact createFactFee(FIN_FinaccTransaction transaction, AcctSchema as,
    ...
    /*
     * if(transaction.getCostCenter()!=null){
     * fact.vCostCenters.add(transaction.getCostCenter().getId());
     * fact.vCostCentersPercentage.add(new BigDecimal(100)); }
     */
    ...
    for (int i = 0; p_lines != null && i < p_lines.length; i++) {
      ...
      FactLine fl = fact.createLine(line,
          getWithdrawalAccount(as, null, transaction.getAccount(), conn), C_Currency_ID,
          line.getDepositAmount(), line.getPaymentAmount(), Fact_Acct_Group_ID, nextSeqNo(SeqNo),
          DocumentType, conn);

      if (transaction.getAccount().getScoInternalDoc() != null)
        fl.setRecordId3(transaction.getAccount().getScoInternalDoc().getId());
      if (transaction.getScoEeffCashflow() != null)
        fl.setEEFF_Cashflow(transaction.getScoEeffCashflow().getId());
    }
    SeqNo = "0";
    return fact;
  }
  ...
  public Fact createFactPaymentDetails(AcctSchema as, ConnectionProvider conn, Fact fact)
    ...
    Currency paymentCurrency = transaction.getFinPayment().getAccount().getCurrency();
    if (!getDocumentPaymentConfirmation(transaction.getFinPayment())) {
      for (int i = 0; p_lines != null && i < p_lines.length; i++) {
        ...
        System.out.println("BPAmount:" + bpamount);
        ...
        if (!"".equals(line.getWriteOffAmt())
          ...
          // Write off amount is generated at payment time so
          // conversion is calculated taking into
          // account conversion at payment date to calculate gains or
          // losses
          // Vector<BigDecimal> vTcDummy = new Vector<BigDecimal>();
          BigDecimal writeOffAmt = new BigDecimal(line.getWriteOffAmt());
          fact.createLine(line, account, strcCurrencyId, (isReceipt ? writeOffAmt.toString() : ""),
        ...
        if (!"".equals(line.cGlItemId)) {
          // FIXME Should this be posted taking into account payment
          // date?? Diferences among
          // currencies
          // BOE NO TIENE TRANSACION
          /*
           * if(!line.Sco_Discboe_ID.equals("")){
           * 
           * Vector<BigDecimal> vTcOut = new Vector<BigDecimal>(); Invoice invoice =
           * OBDal.getInstance().get(Invoice.class, line.Sco_Discboe_ID); boolean tc = true; if
           * (isReceipt) tc = AcctServer.getTCenVigencia(invoice, DateAcct);
           * 
           * String bpAmountConverted = convertAmount(new BigDecimal(line.getAmount()), !isReceipt,
           * DateAcct, TABLEID_Invoice, invoice.getId(), strcCurrencyId, as.m_C_Currency_ID, line,
           * as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo), conn, true, tc, false, vTcOut)
           * .toString(); bpAmountConverted = (new BigDecimal(line.getAmount())).toString();
           * 
           * 
           * FactLine fl = fact.createLine( line,
           * getAccountGLItem(OBDal.getInstance().get(GLItem.class, line.getCGlItemId()), as,
           * isReceipt, conn), strcCurrencyId, isReceipt ? "" : line.getAmount(), isReceipt ?
           * line.getAmount() : "", Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, DateAcct,
           * vTcOut.get(0), conn);
           * 
          ...
          FactLine fl = fact.createLine(line,
              getAccountGLItem(OBDal.getInstance().get(GLItem.class, line.getCGlItemId()), as,
                  isReceipt, conn),
              strcCurrencyId, isReceipt ? "" : line.getAmount(), isReceipt ? line.getAmount() : "",
          ...
          if (!record3.equals("")) {
            ...
            if (doc != null) {
              ...
              if (docInv != null) {
                if (fl != null) {
                  fl.doc_Ad_Org_ID = docInv.getOrganization().getId();
                  fl.doc_C_Currency_ID = docInv.getCurrency().getId();
                  fl.doc_C_Doctype_ID = docInv.getTransactionDocument().getId();
                }
              }
            }
          }
          ...
          if (!isPrepayment && invoice != null) {
            ...
            // looking backwards
            System.out.println("bpAmountConverted: " + bpAmountConverted + " " + strcCurrencyId);
            bpAmountConverted = convertAmount(bpAmountConverted, !isReceipt, DateAcct,
                ...
                fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo), conn, true, tc, false, vTcOut);
            bpAmountConverted = bpamount;
            System.out.println("bpAmountConvertedTC: " + vTcOut.get(0));
            // Cash VAT
            ...
            SeqNo = CashVATUtil.createFactCashVAT(as, conn, fact, Fact_Acct_Group_ID, line, invoice,
                DocumentType, strcCurrencyId, SeqNo, parameters, vCashvatTc.get(0));
            isReceiptForConversion = tmpRecForConv;

            // No dif de cambio en taxes

            // taxCashVatAmount = convertAmount(new
            // BigDecimal(parameters.get(0)), !isReceipt, DateAcct,
            // TABLEID_Invoice, invoice.getId(),
            // paymentCurrency.getId(), as.m_C_Currency_ID, line,
            // as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo), conn,
            // true, tc, !isReceipt, vTcOut);//es el mismo que el de
            // arriba
          ...
          if (isPrepayment) {
            // To force opposite posting isReceipt is opposite as
            // well. this is required when
            // looking backwards. When prepayments date for event is
            // always date for PAYMENT
            bpAmountConverted = convertAmount(bpAmountConverted, !isReceipt, DateAcct,
                TABLEID_Payment, transaction.getFinPayment().getId(), strcCurrencyId,
                as.m_C_Currency_ID, line, as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo), conn,
                true, tc, false, vTcOut);
            bpAmountConverted = bpamount;
          }
          ...
          if (line.getDoubtFulDebtAmount().signum() != 0) {
            ...
            BigDecimal doubtFulDebtAmount = convertAmount(line.getDoubtFulDebtAmount(), isReceipt,
                ...
                line, as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo), conn, false, tc, false,
                vDebtTcOut);
            doubtFulDebtAmount = line.getDoubtFulDebtAmount();

            fl = fact.createLine(line,
                diverseAccount == null ? getAccountBPartner(bpartnerId, as, true, false, true, conn)
                    : diverseAccount,
            ...
            fl = fact.createLine(line,
                getAccountBPartnerAllowanceForDoubtfulDebt(bpartnerId, as, conn), strcCurrencyId,
                doubtFulDebtAmount.toString(), "", Fact_Acct_Group_ID2, nextSeqNo(SeqNo),
                DocumentType, DateAcct, vDebtTcOut.get(0), conn);
            if (invref != null) {
              if (fl != null) {
                fl.doc_Ad_Org_ID = invref.getOrganization().getId();
                fl.doc_C_Currency_ID = invref.getCurrency().getId();
          ...
          if (taxCashVatAmount.doubleValue() == 0) {
            ...
            if (isOverpayment) {// payment only for overpayment
              ...
              System.out.println("CCAmount:" + bpAmountConverted);
            } else {
              fl = fact.createLine(line,
                  (diverseAccount == null || (isPrepayment || line.isPrepaymentAgainstInvoice))
                      ? getAccountBPartner(bpartnerId, as, isReceipt,
                          (isPrepayment || line.isPrepaymentAgainstInvoice) ? true : false, conn)
                      : diverseAccount,
                  strcCurrencyId, (isReceipt ? "" : bpAmountConverted.toString()),
                  (isReceipt ? bpAmountConverted.toString() : ""), Fact_Acct_Group_ID,
                  nextSeqNo(SeqNo), DocumentType, DateAcct, vTcOut.get(0), conn);
              if (invref != null) {
                if (fl != null) {
              ...
              System.out.println("CDAmount:" + bpAmountConverted.toString());
            }
          } else {

            BigDecimal partialAmount = bpAmountConverted;
          ...
          if (line.isPrepaymentAgainstInvoice()) {
            ...
            if (!getAccountBPartner(bpartnerId, as, isReceipt, true, conn).Account_ID
              ...
              fl = fact.createLine(line2,
                  diverseAccount == null
                      ? getAccountBPartner(bpartnerId, as, isReceipt, false, conn)
                      : diverseAccount,
                  strcCurrencyId, (isReceipt ? "" : bpAmountConverted.toString()),
              ...
              fl = fact.createLine(line2, getAccountBPartner(bpartnerId, as, isReceipt, true, conn),
                  strcCurrencyId, (!isReceipt ? "" : bpAmountConverted.toString()),
                  (!isReceipt ? bpAmountConverted.toString() : ""), Fact_Acct_Group_ID3,
                  nextSeqNo(SeqNo), DocumentType, DateAcct, vTcOut.get(0), conn);
              if (invref != null) {
      ...
      if (transaction.getFinPayment().getUsedCredit().compareTo(ZERO) != 0
        ...
        for (FIN_Payment_Credit creditPayment : creditPayments) {
          ...
          String creditAmountConverted = convertAmount(creditPayment.getAmount(), isReceiptPayment,
              ...
              as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo), conn, true, false, false,
              vTcCreditOut).toString();
          creditAmountConverted = creditPayment.getAmount().toString();
          System.out.println("Credito!.:" + creditAmountConverted + " " + creditPayment.getAmount()
              + " " + DateAcct);
          ...
          fact.createLine(null, getAccountBPartner(C_BPartner_ID, as, isReceiptPayment, true, conn),
              ...
              DocumentType, DateAcct, vTcCreditOut.get(0), conn);
          // amtDiff =
          // amtDiff.add(creditPayment.getAmount()).subtract(new
          // BigDecimal(creditAmountConverted));
        }
        /*
      ...
      BigDecimal convertedAmount = convertAmount(transaction.getFinPayment().getAmount(),
          false/* compra */, DateAcct, TABLEID_Payment, transaction.getFinPayment().getId(),
          paymentCurrency.getId(), as.m_C_Currency_ID, null, as, fact, Fact_Acct_Group_ID,
          nextSeqNo(SeqNo), conn, vTcDummy);
    ...
    if (exceptionPosting) {
      // The Payment FinAcct and Transaction FinAcct are different. To
      // post the transaction
      // the amount of the payment need to be moved from destiny account
      // of the payment of FinAcct1
      ...
      FactLine fl = fact.createLine(line,
          ...
          paymentCurrency.getId(), transaction.getDepositAmount().toString(),
  ...
  public Fact createFactGLItem(AcctSchema as, ConnectionProvider conn, Fact fact)
    ...
    for (int i = 0; p_lines != null && i < p_lines.length; i++) {
      ...
      if (transaction.getScoAltConvertRate() != null
          && transaction.getScoAltConvertRate().compareTo(BigDecimal.ZERO) > 0) {
        conversionRate = transaction.getScoAltConvertRate();
      }
      ...
      if (!"".equals(line.getCGlItemId()))
        fact.createLine(line,
            ...
            nextSeqNo(SeqNo), DocumentType, null, conversionRate, conn);
      ...
      FactLine fl = fact.createLine(line,
          ...
          "999999", DocumentType, null, conversionRate, conn);
  ...
  public BigDecimal getBalance() {
    ...
    if (payment != null) {
      ...
      // payment.getWriteoffAmount());//.negate());

    }
    System.out.println("Balance right: " + retValue);
    sb.append(retValue);
    // - Lines
    ...
    try {
      for (int i = 0; i < p_lines.length; i++) {
        ...
        System.out.println("Balance left: " + lineBalance);
        if (payment != null)
          retValue = retValue.subtract(payment.isReceipt() ? lineBalance : lineBalance.negate());
        else
          retValue = retValue.subtract(lineBalance);

        /*
         * } else { BigDecimal lineBalance = payment.isReceipt() ? new
         * BigDecimal(((DocLine_FINFinAccTransaction) p_lines[i]).getAmount()) : new
         * BigDecimal(((DocLine_FINFinAccTransaction) p_lines[i]).getAmount());//.negate(); //
         * BigDecimal lineWriteoff = payment.isReceipt() ? new
         * BigDecimal(((DocLine_FINFinAccTransaction) p_lines[i]).getWriteOffAmt()) : new
         * BigDecimal(((DocLine_FINFinAccTransaction) p_lines[i]).getWriteOffAmt());//.negate();
         * retValue = retValue.subtract(lineBalance);//.subtract(lineWriteoff);
         * 
         * System.out.println(":line: "+lineBalance +" "+ retValue); }
         */
      }
    } finally {
      OBContext.restorePreviousMode();
    ...
    sb.append("]");
    //
    System.out.println(":balance: " + sb.toString());
  ...
  public boolean getDocumentConfirmation(ConnectionProvider conn, String strRecordId) {
    ...
    try {
      ...
      if (transaction.getPosted().equals("sco_M")) {
  ...
  public void loadObjectFieldProvider(ConnectionProvider conn, String strAD_Client_ID, String Id)
    ...
    try {
      ...
      FieldProviderFactory.setField(data[0], "C_DocType_ID", transaction.getDocumentType().getId());
      FieldProviderFactory.setField(data[0], "DocumentNo", transaction.getDocumentNo());
      ...
      FieldProviderFactory.setField(data[0], "SCO_Area_ID",
          transaction.getScoArea() != null ? transaction.getScoArea().getId() : "");
