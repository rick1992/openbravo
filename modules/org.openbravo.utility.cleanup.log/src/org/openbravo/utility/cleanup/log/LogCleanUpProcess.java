...
public class LogCleanUpProcess extends DalBaseProcess {
  ...
  protected void doExecute(ProcessBundle bundle) throws Exception {
    ...
    System.out.println("beanmanager:" + bm.toString());
    final Set<Bean<?>> beans = bm.getBeans(CleanEntity.class, new ComponentProvider.Selector("Default"));
    System.out.println("beans:" + beans.size());
    Bean<?> bean = beans.iterator().next();
    CleanEntity defaultCleaner = (CleanEntity) bm.getReference(bean, CleanEntity.class, bm.createCreationalContext(bean));
    ...
    try {
      ...
      String logMsg = "Starting log clean up process for Client:" + client.getName() + " - Organization:" + org.getName();
      bgLogger.log(logMsg + "\n\n");
      log.debug(logMsg);

      OBCriteria<LogCleanUpConfig> qConfig = OBDal.getInstance().createCriteria(LogCleanUpConfig.class);
      ...
      for (LogCleanUpConfig config : qConfig.list()) {
        ...
        for (Bean<?> beanCleaner : bm.getBeans(CleanEntity.class, new ComponentProvider.Selector(entity.getName()))) {
          // in case there are specific handlers for this entity, use them
          useDefault = false;
          CleanEntity cleaner = (CleanEntity) bm.getReference(beanCleaner, CleanEntity.class, bm.createCreationalContext(beanCleaner));
        ...
        logMsg = "Entity " + entity.getName() + " cleaned up in " + (System.currentTimeMillis() - t) + "ms";
        bgLogger.log(logMsg + "\n\n");
        log.debug(logMsg);
  ...
  private void truncateTables(Set<String> tablesToTruncate, ProcessLogger bgLogger) {
    ...
    for (String tableName : tablesToTruncate) {
      try {
        ...
        ps.execute();
        logMsg = "Truncated table " + tableName + " in " + (System.currentTimeMillis() - truncateTime) + " ms.";
        bgLogger.log(logMsg + "\n");
        log.debug(logMsg);
  ...
  private void vacuumTables(Set<String> tablesWithDeletions, ProcessLogger bgLogger) {
    if (!"POSTGRE".equals(OBPropertiesProvider.getInstance().getOpenbravoProperties().get("bbdd.rdbms"))) {
    ...
    try {
      ...
      for (String tableName : tablesWithDeletions) {
        ...
        try {
          ...
          ps.execute();
          logMsg = "Vacuum for " + tableName + " executed in " + (System.currentTimeMillis() - vacuumTime) + " ms";
          log.debug(logMsg);
          bgLogger.log(logMsg + "\n\n");
        } catch (SQLException e) {
