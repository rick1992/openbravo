...
public class DeleteTransaction implements Process {
  ...
  public void execute(ProcessBundle bundle) throws Exception {
    ...
    FIN_FinaccTransaction transaction = OBDal.getInstance().get(FIN_FinaccTransaction.class, strKey);

    try {
      OBError msgt = processTransaction(vars, bundle.getConnection(), "R", transaction);
      if ("Success".equals(msgt.getType())) {
        OBContext.setAdminMode();
        try {

          // check if the transaction is a cash transfer
          FIN_Payment payment = transaction.getScoCtransPayin();
          if (payment != null) {

            // put the payment em_sco_cashtransferstatus column to
            // 'SCO_CASH' status.
            payment.setScoCashtransferstatus("SCO_CASH");
          }
