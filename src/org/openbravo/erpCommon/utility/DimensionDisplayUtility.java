...
public class DimensionDisplayUtility {
  ...
  public static String displayAcctDimensions(String centrally, String dimemsion, String docBaseType, String level) {
    String var = "";
  ...
  public static String computeAccountingDimensionDisplayLogic(Tab tab, Field field) {
    ...
    String displayLogicPart1 = "(context." + IsAcctDimCentrally + " === 'N' && context.$Element_%s === 'Y')";
    String displayLogicPart2 = " || (context." + IsAcctDimCentrally + " === 'Y' && context['$Element_%s_' + OB.Utilities.getValue(currentValues, \"%s\") + '_%s'] === 'Y')";
    ...
    try {
      ...
      if (field.getColumn() != null) {
        ...
        log4j.error("Field (" + field.getId() + " | " + field.getName() + ") not linked to any column.");
        return "";
      }
      String dimension = columnDimensionMap.get(columnName.toUpperCase());
      if (dimension == null) {
        log4j.error("Field (" + field.getId() + " | " + field.getName() + ") not mapping any dimension.");
        return "";
      }
      ...
      if (dimension.equals(DIM_Campaign) || dimension.equals(DIM_Activity) || dimension.equals(DIM_Asset)) {
        log4j.error(field.getName() + " field contains @ACCT_DIMENSION_DISPLAY@ display logic but is not supported. Change it.");
      ...
      for (String l : levelList) {
        ...
        return String.format(displayLogicPart1 + displayLogicPart2, dimension, dimension, DIM_AUXILIAR_INPUT, l);
      }
  ...
  public static Map<String, String> getAccountingDimensionConfiguration(Client client) {
    ...
    try {
      ...
      for (ADClientAcctDimension cad : client.getADClientAcctDimensionList()) {
        clientAcctDimensionCache.put(cad.getDimension() + "_" + cad.getDocBaseType() + "_" + DIM_Header, cad.isShowInHeader() ? "Y" : "N");
        clientAcctDimensionCache.put(cad.getDimension() + "_" + cad.getDocBaseType() + "_" + DIM_Lines, cad.isShowInLines() ? "Y" : "N");
        clientAcctDimensionCache.put(cad.getDimension() + "_" + cad.getDocBaseType() + "_" + DIM_BreakDown, cad.isShowInBreakdown() ? "Y" : "N");
      }
          ...
          for (org.openbravo.model.ad.domain.List level : levelsRef.getADListList()) {
            ...
            if (DIM_Organization.equals(dimValue)) {
              if (client.isOrgAcctdimIsenable()) {
                isDisplayed = clientAcctDimensionCache.get(dimValue + "_" + docValue + "_" + levelValue);
                if (isDisplayed == null) {
              ...
              if (client.isUser1AcctdimIsenable()) {
                isDisplayed = clientAcctDimensionCache.get(dimValue + "_" + docValue + "" + levelValue);
                if (isDisplayed == null) {
                  if (DIM_Header.equals(levelValue)) {
  ...
  public static List<String> getRequiredSessionVariablesForTab(Tab tab, Field field) {
    ...
    try {
      ...
      final Query queryDoc = session.createQuery(String.format(hql.toString(), DimensionMapping.PROPERTY_DOCUMENTCATEGORY));
      queryDoc.setParameter(0, tableId);
      queryDoc.setParameter(1, dimension);
      List<String> docBaseTypeList = queryDoc.list();

      final Query queryLevel = session.createQuery(String.format(hql.toString(), DimensionMapping.PROPERTY_LEVEL));
      queryLevel.setParameter(0, tableId);
      queryLevel.setParameter(1, dimension);
      List<String> levelList = queryLevel.list();
  ...
  public static Map<String, String> getReadOnlyLogicSessionVariables() {
    ...
    try {
      ...
      log4j.error("Not possible to load session variables for read only logic based on dimension mapping table", e);
      return new HashMap<String, String>();
    } finally {
      OBContext.restorePreviousMode();
    }
  ...
  public static List<Object[]> getGroupDimensionMapping(List<String> dimensionList) {
    try {
      ...
      hql.append(" from " + DimensionMapping.ENTITY_NAME + " as dm ");
      hql.append(" where dm." + DimensionMapping.PROPERTY_ACCOUNTINGDIMENSION + " IN (:dimensionList) ");
      hql.append(" group by dm." + DimensionMapping.PROPERTY_ACCOUNTINGDIMENSION + ", ");
      hql.append("          dm." + DimensionMapping.PROPERTY_DOCUMENTCATEGORY);
