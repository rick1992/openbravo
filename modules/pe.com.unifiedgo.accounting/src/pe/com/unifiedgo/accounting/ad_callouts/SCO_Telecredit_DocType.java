...
public class SCO_Telecredit_DocType extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strSCOTelecreditId = vars.getStringParameter("inpscoTelecreditId");
      String strDescription = vars.getStringParameter("inpdescription");
      String strTabId = vars.getStringParameter("inpTabId");

      try {
        printPage(response, vars, strDocTypeTarget, strDocType, docNo, strSCOTelecreditId, strDescription, strTabId);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strDocTypeTarget, String strDocType, String docNo, String strSCOTelecreditId, String strDescription, String strTabId) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    if (strDocTypeTarget.equals(""))
      ...
      resultado.append("var calloutName='SCO_Telecredit_DocType';\n\n");
      ...
      SCOTelecreditDocTypeData[] data = null;

      if (!newDocNo && !"0".equals(strDocType)) {
        data = SCOTelecreditDocTypeData.select(this, strDocType);
      ...
      SCOTelecreditDocTypeData[] dataNew = SCOTelecreditDocTypeData.select(this, strDocTypeTarget);
      if (dataNew != null && dataNew.length > 0) {
        DocSubTypeSO = dataNew[0].docsubtypeso;
        if (DocSubTypeSO == null)
          DocSubTypeSO = "--";
        String strOldDocTypeTarget = SCOTelecreditDocTypeData.selectOldDocSubType(this, strSCOTelecreditId);
        if (!DocSubTypeSO.equals("OB") && strOldDocTypeTarget.equals("OB")) {
          String strOldDocNo = SCOTelecreditDocTypeData.selectOldDocNo(this, strSCOTelecreditId);
        ...
        if (dataNew[0].isdocnocontrolled.equals("Y")) {
          if (!newDocNo && !AD_Sequence_ID.equals(dataNew[0].adSequenceId) && !SCOTelecreditDocTypeData.selectOldDocTypeTargetId(this, strSCOTelecreditId).equalsIgnoreCase(strDocTypeTarget))
            newDocNo = true;
          if (newDocNo) {
