...
OB.SCR.execCheckInvToRefundComission = {
  execute: function (params, view) {
    ...
    for (i = 0; i < selection.length; i++) {
     ...
     isc.SCR_CheckInvToRefundComissionPopup.create({
  ...
  execFunction: function (params, view) {
    params.actionHandler = 'pe.com.unifiedgo.core.CheckInvToRefundComissionActionHandler';
    params.adTabId = view.activeView.tabId;
    OB.SCR.execCheckInvToRefundComission.execute(params, view);
  }

};

isc.defineClass('SCR_CheckInvToRefundComissionPopup', isc.OBPopup);

isc.SCR_CheckInvToRefundComissionPopup.addProperties({
  ...
  initWidget: function () {
	...
	this.confirmation = isc.Label.create({
		    ...
		    wrap: false,
		    showEdges: false,
		    contents: OB.I18N.getLabel('SCR_ChkInvToRefundComission_confirm')
    });
