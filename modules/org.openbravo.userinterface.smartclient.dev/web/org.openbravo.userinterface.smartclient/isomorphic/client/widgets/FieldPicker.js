...
//>	@class FieldPickerField
// Class containing properties that configures the display of
// and interaction with the fields showin in a +link{FieldPicker}.
//
...
createExpansionDynamicForm : function (editableProperties, record) {
    ...
    // allow summary/fomrula fields to be edited
...
openEditor : function (methodName, argument) {
    ...
    case "editFormulaField":
        var window = dataBoundComponent._formulaEditor;
        if (window) {
...
initWidget : function () {
    ...
    for (var i = 0; i < availableControls.length; i++) {
        ...
        if (availableControl == "hilitesButton") {
            currentControls.add(isc.IButton.create({
                shuttle: this,
                autoFit: true,
...
//> @attr fieldPicker.dataBoundComponent (DataBoundComponent : null : IR)
...
// @param fields (Array of ListGridFields) committed fields from disposable ListGrid
// @param hilites (Array) Array of hilite objects
// @see fieldPicker.dataSource
// @visibility external
//<

//> @attr fieldPicker.sampleRecord (Record|"first"|"firstOpenLeaf" : "first" : IR)
...
// Causes a "Highlights..." button to appear to edit hilites.
// @visibility external
//<
...
createDataSourceFromFields : function (fields, exclusions, available, skipDuplicate) {
    ...
    if (isc.isA.String(this.sampleRecord)) {
        ...
        if (this.sampleRecord == "first") {
            ...
            if (isc.isA.ListGrid(component)) {
                if (this.dataBoundComponent.fieldPickerShowSampleValues) {
    ...
    var nameField = {name: "name",   title: title, autoFitWidth: auto, primaryKey: true},
       titleField = {name: "_title", title: title, autoFitWidth: auto, canEdit: !available};
    ...
    if (this.shouldUseTrees()) {
        var id       = this.pickerIdField,
            ...
            rootValue = this.dataBoundComponent.dataSource.fieldTreeRootValue;
...
refresh : function (builder, hiliter) {
    ...
    if (builder) {
        ...
        var newWindow = this.dataBoundComponent._formulaEditor;
...
initWidget : function () {
    ...
    this.addMember( isc.HLayout.create({
        ...
        members: [
            this.createAutoChild("saveAndExitButton", {
                picker: this,
                title: this.saveAndExitButtonTitle
            }),
            this.createAutoChild("cancelChangesButton", {
                picker: this,
...
if (isc.Window != null) {
    ...
    // defaults for AutoChild fieldPicker
    
    fieldPickerDefaults: {
        autoParent: "none",
        _constructor: isc.FieldPicker,
