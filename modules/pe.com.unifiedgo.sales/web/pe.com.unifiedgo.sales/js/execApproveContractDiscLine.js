...
OB.SSA.execApproveContractDiscLine = {
  execute: function (params, view) {
    ...
    for (i = 0; i < selection.length; i++) {
     ...
     isc.SSA_ApproveContractDiscLinePopup.create({
  ...
  execFunction: function (params, view) {
    params.actionHandler = 'pe.com.unifiedgo.sales.ApproveContractDiscLineActionHandler';
    params.adTabId = view.activeView.tabId;
    OB.SSA.execApproveContractDiscLine.execute(params, view);
  }

};

isc.defineClass('SSA_ApproveContractDiscLinePopup', isc.OBPopup);

isc.SSA_ApproveContractDiscLinePopup.addProperties({
  ...
  initWidget: function () {
	...
	this.confirmation = isc.Label.create({
		    ...
		    wrap: false,
		    showEdges: false,
		    contents: OB.I18N.getLabel('SSA_ApproveContractDiscount_confirmation')
    });
