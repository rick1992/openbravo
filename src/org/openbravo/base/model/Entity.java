...
public class Entity {
  ...
  private List<Property> sortingIdentifierProperties;
  ...
  private boolean fkHasspecialSorting = false;
  ...
  public void initialize(Table table) {
    ...
    sortingIdentifierProperties = new ArrayList<Property>();
    ...
    for (final Column c : table.getColumns()) {
      ...
      if (p.isIdentifier() && !p.isScrIgnoreforsorting()) {
        sortingIdentifierProperties.add(p);
      }
    }
    ...
    Collections.sort(sortingIdentifierProperties, new Comparator<Property>() {
      public int compare(Property p1, Property p2) {
        if (p1.getSeqno() == null && p2.getSeqno() == null) {
          return 0;
        }
    ...
    if (table.getAccessLevel().equals("1")) {
      ...
      Check.fail("Access level " + table.getAccessLevel() + " for table " + table.getName() + " is not supported");
    }
  ...
  void initializeComputedColumns(Table t, Entity e) {
    ...
    setClassName(e.getPackageName() + "." + e.getSimpleClassName() + COMPUTED_COLUMNS_CLASS_APPENDIX);
    ...
    for (final Column c : t.getColumns()) {
      if (!(c.isKey() || c.getSqlLogic() != null || "AD_Client_ID".equalsIgnoreCase(c.getColumnName()) || "AD_Org_ID".equalsIgnoreCase(c.getColumnName()))) {
        continue;
      }
      final Property p = new Property();
      p.setEntity(this);
  ...
  public void addProperty(Property property) {
    ...
    if (property.isIdentifier()) {
      ...
      // In case of sortingproperties
      if (!property.isScrIgnoreforsorting()) {
        Property duplicatedSIdentifier = null;
        for (Property i : sortingIdentifierProperties) {
          if (i.getColumnName().equalsIgnoreCase(property.getColumnName())) {
            duplicatedSIdentifier = i;
            break;
          }
        }
        if (duplicatedSIdentifier != null) {
          sortingIdentifierProperties.remove(duplicatedSIdentifier);
          log.debug("Removed duplicated sorting identifier property " + property);
        }
        getSortingIdentifierProperties().add(property);
      }
    }
    if (property.isId()) {
  ...
  private String removeIllegalChars(String fromName) {
    ...
    if (nameChanged) {
      ...
      for (char c : fromName.trim().toCharArray()) {
        ...
        if (!normalChar) {
          log.warn("The entity name " + fromName + " contains a character (" + c + ") which could result in issues in HQL or " + "webservices. Use characters from a to z, A to Z or 0 to 9 or the _");
        }
      }
    }
  ...
  public void checkIsValidProperty(String propertyName) {
    Check.isNotNull(propertiesByName.get(propertyName), "Property " + propertyName + " not defined for entity " + this);
  }
  ...
  public List<Property> getRealProperties(boolean includeComputed) {
    ...
    for (Property p : properties) {
      if ((includeComputed || !p.isComputedColumn()) && !Entity.COMPUTED_COLUMNS_PROXY_PROPERTY.equals(p.getName())) {
        result.add(p);
      }
    }
    return result;
  ...
  /**
   * @return the properties which make up the sorting identifying string of this Entity
   */
  public List<Property> getSortingIdentifierProperties() {
    return sortingIdentifierProperties;
  }

  public void setSortingIdentifierProperties(List<Property> sortingIdentifierProperties) {
    this.sortingIdentifierProperties = sortingIdentifierProperties;
  }
  ...
  public void setfkHasspecialSorting(boolean fkHasspecialSorting) {
    this.fkHasspecialSorting = fkHasspecialSorting;
  }

  public boolean isfkHasspecialSorting() {
    return fkHasspecialSorting;
  }
}
