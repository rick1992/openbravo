...
public class ReportReconciliation extends HttpSecureAppServlet {
  ...
  public void postReconciliation(HttpServletRequest request, HttpServletResponse response,
      String strReportType) throws IOException, ServletException {
    VariablesSecureApp vars = new VariablesSecureApp(request);
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strFinFinancialAccountId = vars.getGlobalVariable("inpfinFinancialAccountId", "");
      String strDateTo = vars.getGlobalVariable("inpdateto", "");
      printPageDataPDFReport(request, response, vars, strFinReconciliationID,
          strFinFinancialAccountId, strDateTo, strReportType);
    }
  }

  private void printPageDataPDFReport(HttpServletRequest request, HttpServletResponse response,
      VariablesSecureApp vars, String strFinReconciliationId, String strFinFinancialAccountId,
      String strDateTo, String strReportType) throws IOException, ServletException {
    log4j.debug("Output: Reconciliation PDF report");

    String strMainReportName = "@basedesign@/pe/com/unifiedgo/report/ad_reports/Rpt_Reconciliation_ugo.jrxml";
    FIN_Reconciliation reconciliation = null;
    OBContext.setAdminMode(true);
    try {
      reconciliation = OBDal.getInstance().get(FIN_Reconciliation.class, strFinReconciliationId);
    ...
    try {
      JasperDesign jasperDesignLines = JRXmlLoader.load(strBaseDesign
          + "/pe/com/unifiedgo/report/ad_reports/Rpt_Reconciliation_Lines_ugo.jrxml");
    ...
    // Parameters
    parameters.put("DOCUMENT_ID", strFinReconciliationId);
    // parameters.put("FINANCIAL_ACCOUNT_ID", strFinFinancialAccountId);

    parameters.put("DOCUMENT_NAME", Utility.messageBD(this, "Reconciliation", strLanguage));
    ...
    parameters.put("SUBREP_Rpt_Reconciliation_Lines_ugo", subReportVourcherLibroLines);
