...
public class LoginHandler extends HttpBaseServlet {
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
    ...
    try {
      ...
      if (strUser.equals("") && !OBVersion.getInstance().is30()) {
        res.sendRedirect(res.encodeRedirectURL(getDireccion() + "/security/Login_F1.html"));
      } else {
        try {
          ...
          if (errorMsg != null) {
            ...
            goToRetry(res, vars, failureMessage, failureTitle, "Error", "../security/Login_FS.html", doRedirect);
  ...
  public String getDireccion() {
    ...
    return HttpBaseUtils.getLocalHostAddress(request);
  }

  private void checkLicenseAndGo(HttpServletResponse res, VariablesSecureApp vars, String strUserAuth, String username, String sessionId, boolean redirect) throws IOException, ServletException {
    OBContext.setAdminMode();
    try {
      ...
      boolean doRedirect = redirect || (limitation == LicenseRestriction.NO_RESTRICTION && forceNamedUserLogin);
      ...
      case NUMBER_OF_CONCURRENT_USERS_REACHED:
        ...
        String title = Utility.messageBD(myPool, "NUMBER_OF_CONCURRENT_USERS_REACHED_TITLE", vars.getLanguage());
      ...
      if (sysInfo.getSystemStatus() == null || sysInfo.getSystemStatus().equals("RB70") || this.globalParameters.getOBProperty("safe.mode", "false").equalsIgnoreCase("false")) {
      ...
      if (hasSystem && ak.getWsCallsExceededDays() > 0) {
        ...
        case EXCEEDED_WARN_WS_CALLS:
          msg = Utility.messageBD(myPool, "OPS_MAX_WS_CALLS_SOFT_MSG", vars.getLanguage(), false).replace("@daysExceeding@", Integer.toString(ak.getWsCallsExceededDays())).replace("@extraDays@", Integer.toString(ak.getExtraWsExceededDaysAllowed())).replace("@numberOfDays@", Integer.toString(ak.getNumberOfDaysLeftInPeriod()));
          goToRetry(res, vars, msg, title, msgType, action, doRedirect);
          return;
        case EXCEEDED_MAX_WS_CALLS:
          msg = Utility.messageBD(myPool, "OPS_MAX_WS_CALLS_MSG", vars.getLanguage(), false).replace("@daysExceeding@", Integer.toString(ak.getWsCallsExceededDays()));
          goToRetry(res, vars, msg, title, msgType, action, doRedirect);
          return;
        }
      }
      ...
      try {
        ...
        String title = Utility.messageBD(myPool, "InvalidDefaultLoginTitle", vars.getLanguage()).replace("%0", e.getDefaultField());
        String msg = Utility.messageBD(myPool, "InvalidDefaultLoginMsg", vars.getLanguage()).replace("%0", e.getDefaultField());
  ...
  private void goToTarget(HttpServletResponse response, VariablesSecureApp vars, boolean doRedirect) throws IOException, ServletException {

    String target = vars.getSessionValue("target");

    if (target.equals("")) {
      target = getDireccion() + "/security/Menu.html";
    }
  ...
  private void goToRetry(HttpServletResponse response, VariablesSecureApp vars, String message, String title, String msgType, String action, boolean doRedirect) throws IOException, ServletException {
    String msg = (message != null && !message.equals("")) ? message : "Please enter your username and password.";
    ...
    if (OBVersion.getInstance().is30() && !doRedirect) {
      ...
      final XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/base/secureApp/HtmlErrorLogin", discard).createXmlDocument();
