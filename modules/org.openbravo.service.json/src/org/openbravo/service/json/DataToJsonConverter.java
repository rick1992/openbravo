...
public class DataToJsonConverter {
  ...
  private final static SimpleDateFormat xmlTimeFormatWithoutMTOffset = JsonUtils.createTimeFormatWithoutGMTOffset();
  ...
  public JSONObject toJsonObject(BaseOBObject bob, DataResolvingMode dataResolvingMode) {
    try {
      ...
      final boolean isDerivedReadable = OBContext.getOBContext().getEntityAccessChecker().isDerivedReadable(bob.getEntity());
      ...
      for (Property property : bob.getEntity().getProperties()) {
        ...
        if (value != null) {
          if (property.isPrimitive()) {
            ...
            addBaseOBObject(jsonObject, property, property.getName(), property.getReferencedProperty(), (BaseOBObject) value);
          }
        } else {
      ...
      for (String additionalProperty : additionalProperties) {
        ...
        if (value == null) {
          ...
          addBaseOBObject(jsonObject, additonalPropertyObject, additionalProperty, additonalPropertyObject.getReferencedProperty(), (BaseOBObject) value);
        } else {
  ...
  private void addBaseOBObject(JSONObject jsonObject, Property referencingProperty, String propertyName, Property referencedProperty, BaseOBObject obObject) throws JSONException {

    String identifier = null;
    // jsonObject.put(propertyName, toJsonObject(obObject, DataResolvingMode.SHORT));
    if (referencedProperty != null) {
      try {
        jsonObject.put(propertyName.replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR), obObject.get(referencedProperty.getName()));
    ...
    if (referencingProperty != null && referencingProperty.hasDisplayColumn()) {
      ...
      Property displayColumnProperty = DalUtil.getPropertyFromPath(referencedProperty.getEntity(), referencingProperty.getDisplayPropertyName());
      ...
      if (referencingProperty.hasDisplayColumn()) {
        Object referenceObject = obObject.get(referencingProperty.getDisplayPropertyName(), OBContext.getOBContext().getLanguage(), (String) obObject.getId());
        ...
        if (referencingProperty.isDisplayValue()) {
          ...
          if (obObject.getEntity().hasProperty("searchKey")) {
            Object valueObject = obObject.get("searchKey", OBContext.getOBContext().getLanguage(), (String) obObject.getId());
            if (valueObject != null) {
            ...
            log.warn("Entity " + obObject.getEntity().getName() + " does not have a searchKey property, the flag Displayed Value should not be used");
          }
        }
        // Allowing one level deep of displayed column pointing to references with display column
        jsonObject.put(propertyName.replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR) + DalUtil.FIELDSEPARATOR + JsonConstants.IDENTIFIER, identifier);
      } else if (!displayColumnProperty.isPrimitive()) {
        // Displaying identifier for non primitive properties
        BaseOBObject bo = ((BaseOBObject) obObject.get(referencingProperty.getDisplayPropertyName()));
        jsonObject.put(propertyName.replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR) + DalUtil.FIELDSEPARATOR + JsonConstants.IDENTIFIER, bo.getIdentifier());
        if (bo.getEntity().isfkHasspecialSorting()) {
          jsonObject.put(propertyName.replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR) + DalUtil.FIELDSEPARATOR + JsonConstants.SORTINGIDENTIFIER, bo.getSortingIdentifier());
        }
      ...
      jsonObject.put(propertyName.replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR) + DalUtil.FIELDSEPARATOR + JsonConstants.IDENTIFIER, obObject.getIdentifier());
      if (obObject.getEntity().isfkHasspecialSorting()) {
        jsonObject.put(propertyName.replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR) + DalUtil.FIELDSEPARATOR + JsonConstants.SORTINGIDENTIFIER, obObject.getSortingIdentifier());
      }
    }
  }
  ...
  protected Object convertPrimitiveValue(Property property, Object value) {
    ...
    } else if (property.getDomainType() instanceof HashedStringDomainType || property.getDomainType() instanceof EncryptedStringDomainType) {
      return "***";
