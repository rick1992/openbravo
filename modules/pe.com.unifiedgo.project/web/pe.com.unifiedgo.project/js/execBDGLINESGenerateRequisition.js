...
OB.SPR.execBDGLINESGenerateRequisition = {
  execute: function (params, view) {
    ...
    for (i = 0; i < selection.length; i++) {
     ...
     isc.SPR_BDGLiGenerateRequisitionPopup.create({
  ...
  execFunction: function (params, view) {
    params.actionHandler = 'pe.com.unifiedgo.project.BDGLiGenerateRequisitionActionHandler';
    params.adTabId = view.activeView.tabId;
    OB.SPR.execBDGLINESGenerateRequisition.execute(params, view);
  }

};

isc.defineClass('SPR_BDGLiGenerateRequisitionPopup', isc.OBPopup);

isc.SPR_BDGLiGenerateRequisitionPopup.addProperties({
  ...
  initWidget: function () {
	...
	this.confirmation = isc.Label.create({
		    ...
		    wrap: false,
		    showEdges: false,
		    contents: OB.I18N.getLabel('SPR_BDGLiGenerateRequistion_confirmation')
    });
