...
public class SRMOPickEditLines extends BaseProcessActionHandler {
  ...
  protected JSONObject doExecute(Map<String, Object> parameters, String content) {
    ...
    try {
      ...
      if ("SSARFCORDER".compareTo(order.getTransactionDocument().getScoSpecialdoctype()) == 0) {
        order.setSsaInvoiceref(null);

        Invoice inv = getRFCRelateSalesInvoice(order);
        if (inv != null) {
          order.setSsaInvoiceref(inv);
        }
  ...
  private void createOrderLines(JSONObject jsonRequest, List<String> idList) throws JSONException,
    ...
    // Add by Vafaster; esto es para que cada vez que inserte desde el botòn pick/edit lines, el
    // campo linea se reestablescan en CERO, sino lo que hacia era poner dònde se quedo en contador
    if (order.getTransactionDocument().getScoSpecialdoctype().equals("SSARTVORDER")
        || order.getTransactionDocument().getScoSpecialdoctype().equals("SSARFCORDER")) {
      lineNo = 0L;
    }
    ...
    for (long i = 0; i < selectedLines.length(); i++) {
      ...
      } else if (shipmentLine != null && shipmentLine.getInvoiceLineList().size() != 0) {// added by
                                                                                         // pedro
        tax = shipmentLine.getInvoiceLineList().get(0).getTax();
      } else {
        String taxId = "";
      ...
      if (selectedLine.get("unitPrice").equals(null) || selectedLine.get("unitPrice").equals("")) {
        try {
  ...
  private Invoice getRFCRelateSalesInvoice(Order order) {
    Invoice inv = null;
    List<OrderLine> orderlines = order.getOrderLineList();
    for (int i = 0; i < orderlines.size(); i++) {
      OrderLine orderline = orderlines.get(i);
      ShipmentInOutLine inoutline = orderline.getGoodsShipmentLine();
      if (inoutline != null) {
        OrderLine orderline2 = inoutline.getSalesOrderLine();
        if (orderline2 != null) {
          OBCriteria<InvoiceLine> invoicelines = OBDal.getInstance().createCriteria(
              InvoiceLine.class);
          invoicelines.add(Restrictions.eq(InvoiceLine.PROPERTY_SALESORDERLINE, orderline2));
          invoicelines.add(Restrictions.eq(InvoiceLine.PROPERTY_CLIENT, order.getClient()));
          invoicelines.add(Restrictions.eq(InvoiceLine.PROPERTY_ACTIVE, true));
          invoicelines.createAlias(InvoiceLine.PROPERTY_INVOICE, "inv");
          invoicelines.addOrder(org.hibernate.criterion.Order.asc("inv."
              + Invoice.PROPERTY_INVOICEDATE));
          if (invoicelines.list().size() > 0 && invoicelines.list().get(0) != null) {
            inv = invoicelines.list().get(0).getInvoice();
            break;
          }
          ...
          OBCriteria<InvoiceLine> invoicelines = OBDal.getInstance().createCriteria(
              InvoiceLine.class);
          invoicelines.add(Restrictions.eq(InvoiceLine.PROPERTY_GOODSSHIPMENTLINE, inoutline));
          invoicelines.add(Restrictions.eq(InvoiceLine.PROPERTY_CLIENT, order.getClient()));
          invoicelines.add(Restrictions.eq(InvoiceLine.PROPERTY_ACTIVE, true));
          invoicelines.createAlias(InvoiceLine.PROPERTY_INVOICE, "inv");
          invoicelines.addOrder(org.hibernate.criterion.Order.asc("inv."
