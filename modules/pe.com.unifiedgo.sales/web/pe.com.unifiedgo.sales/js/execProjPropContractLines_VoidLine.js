...
OB.SSA.execProjPropContractLines_VoidLine = {
  execute: function (params, view) {
    ...
    for (i = 0; i < selection.length; i++) {
     ...
     isc.SSA_ProjPropVoidContractLinePopup.create({
  ...
  execFunction: function (params, view) {
    params.actionHandler = 'pe.com.unifiedgo.sales.ProjPropVoidContractLineActionHandler';
    params.adTabId = view.activeView.tabId;
    OB.SSA.execProjPropContractLines_VoidLine.execute(params, view);
  }

};

isc.defineClass('SSA_ProjPropVoidContractLinePopup', isc.OBPopup);

isc.SSA_ProjPropVoidContractLinePopup.addProperties({
  ...
  initWidget: function () {
	...
	this.confirmation = isc.Label.create({
		    ...
		    wrap: false,
		    showEdges: false,
		    contents: OB.I18N.getLabel('SSA_PropVoidCntctLinePopup_confirm')
    });
