...
removeChild : function (child, name) {
    ...
    //>EditMode
    if (this.selectedComponents && this.selectedComponents.remove(child)) this.fireSelectedComponentsUpdated();
    //<EditMode
},
...
scrollToPercent : function (left, top, reason) {
    ...
    // Resolve bad coordinates
    
    if (left != null && !isc.isA.Number(left)) left = 0;
    if (top  != null && !isc.isA.Number(top))  top  = 0;
...
snapGridPaneConstructor: "DrawPane",
snapGridPaneDefaults: {

    parentResized : function () {
        var parentCanvas = this.getParentCanvas();
        this.resizeTo(parentCanvas.getWidth(), parentCanvas.getHeight());
        this.drawGrid();
    },
    ...
    drawGrid : function () {
        ...
        var parentCanvas = this.getParentCanvas(),
            hGap = parentCanvas.snapHGap,
            vGap = parentCanvas.snapVGap,
            vStopCount = this.getHeight() / vGap,
            hStopCount = this.getWidth() / hGap,
            halfCrossLength = this.crossLength / 2
        ;

        for (var row = 1; row < vStopCount; row++) {
            for (var col = 1; col < hStopCount; col++) {
                var x = (col * hGap),
                    y = (row * vGap)
                ;

                var hLineProperties = isc.addProperties({}, this.lineDefaults, {
                   drawPane: this,
                ...
                var vLineProperties = isc.addProperties({}, this.lineDefaults, {
                   drawPane: this,
...
_showSnapGrid : function () {
    ...
    if (this.snapVGap < 20 || this.snapHGap < 20) {
        this.logWarn("Attempt to showSnapGrid ignored - snapVGap or snapHGap < 10");
        return;
    }
...
propertyChanged : function (propName, value) {
    ...
    if ((propName == this._$snapHGap || propName == this._$snapVGap) && this.snapGridPane) this.snapGridPane.drawGrid();
...
// </smartclient>
// <smartgwt>
// Given a SmartClient component that you want to skin, open it's JavaDoc:
// </smartgwt>
// <ul>
...
//    })
// </pre>
// <smartgwt>This creates a &#83;martClient class, which does not exist as a Java class.  To
