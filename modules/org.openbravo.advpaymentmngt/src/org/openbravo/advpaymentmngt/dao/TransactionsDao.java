...
public class TransactionsDao {
  ...
  public static OBObjectFieldProvider[] getAccTrxData(String finFinancialAccountId) {
    ...
    try {
      final OBQuery<FIN_FinaccTransaction> obQuery = OBDal.getInstance().createQuery(FIN_FinaccTransaction.class, whereClause.toString());
      obQuery.setParameters(parameters);
  ...
  public static FIN_FinaccTransaction createFinAccTransaction(FIN_Payment payment) {
    final FIN_FinaccTransaction newTransaction = OBProvider.getInstance().get(FIN_FinaccTransaction.class);
    OBContext.setAdminMode();
    try {
      newTransaction.setFinPayment(payment);
      newTransaction.setOrganization(payment.getOrganization());
      newTransaction.setDocumentType(SCO_Utils.getDocTypeFromSpecial(newTransaction.getOrganization(), "SCOFINANCIALACCOUNT"));
      newTransaction.setDocumentNo(SCO_Utils.getDocumentNo(newTransaction.getOrganization(), "SCOFINANCIALACCOUNT", "FIN_Finacc_Transaction"));
      ...
      newTransaction.setScoEeffCashflow(payment.getScoEeffCashflow());
      ...
      BigDecimal depositAmt = FIN_Utility.getDepositAmount(payment.getDocumentType().getDocumentCategory().equals("ARR"), payment.getFinancialTransactionAmount());
      BigDecimal paymentAmt = FIN_Utility.getPaymentAmount(payment.getDocumentType().getDocumentCategory().equals("ARR"), payment.getFinancialTransactionAmount());

      newTransaction.setDepositAmount(depositAmt);
      newTransaction.setPaymentAmount(paymentAmt);
      newTransaction.setStatus(newTransaction.getDepositAmount().compareTo(newTransaction.getPaymentAmount()) > 0 ? "RPR" : "PPM");
  ...
  public static Long getTransactionMaxLineNo(FIN_FinancialAccount financialAccount) {
    Query query = OBDal.getInstance().getSession().createQuery("select max(f.lineNo) as maxLineno from FIN_Finacc_Transaction as f where account.id=?");
  ...
  public static void process(FIN_FinaccTransaction finFinancialAccountTransaction) {
    final FIN_FinancialAccount financialAccount = OBDal.getInstance().get(FIN_FinancialAccount.class, finFinancialAccountTransaction.getAccount().getId());
    financialAccount.setCurrentBalance(financialAccount.getCurrentBalance().add(finFinancialAccountTransaction.getDepositAmount().subtract(finFinancialAccountTransaction.getPaymentAmount())));
    ...
    if (payment != null) {
      ...
      finFinancialAccountTransaction.setStatus(finFinancialAccountTransaction.getDepositAmount().compareTo(finFinancialAccountTransaction.getPaymentAmount()) > 0 ? "RDNC" : "PWNC");
    }
  ...
  public static void post(VariablesSecureApp vars, ConnectionProvider connectionProvider, FIN_FinaccTransaction finFinancialAccountTransaction) {
    final String AD_TABLE_ID = "4D8C3B3C31D1410DA046140C9F024D17";
    try {
      AcctServer acct = AcctServer.get(AD_TABLE_ID, vars.getClient(), finFinancialAccountTransaction.getOrganization().getId(), connectionProvider);
      if (acct == null) {
        throw new OBException("Accounting process failed for the financial account transaction");
      } else if (!acct.post(finFinancialAccountTransaction.getId(), false, vars, connectionProvider, connectionProvider.getConnection()) || acct.errors != 0) {
  ...
  public static int getPendingToMatchCount(FIN_FinancialAccount financialAccount) {
    ...
    try {
      ...
      whereClause.append("'");
      whereClause.append(" and fatrx.").append(FIN_FinaccTransaction.PROPERTY_RECONCILIATION).append(" is null ");
      final OBQuery<FIN_FinaccTransaction> obqFATrx = OBDal.getInstance().createQuery(FIN_FinaccTransaction.class, whereClause.toString());
  ...
  public static void updateAccountingDate(FIN_FinaccTransaction transaction) {
    ...
    obcAF.add(Restrictions.eq(AccountingFact.PROPERTY_TABLE, OBDal.getInstance().get(Table.class, FIN_FINACC_TRANSACTION_TABLE)));
  ...
  public static List<FIN_FinaccTransaction> getTransactionsToReconciled(FIN_FinancialAccount account, Date statementDate, boolean hideAfterDate) {
    ...
    try {
      ...
      final OBQuery<FIN_FinaccTransaction> obQuery = OBDal.getInstance().createQuery(FIN_FinaccTransaction.class, whereClause.toString(), parameters);

      obQuery.setMaxResult(1000);
      return obQuery.list();
  ...
  public static FieldProvider[] getTransactionsFiltered(FIN_FinancialAccount account, Date statementDate, boolean hideAfterDate) {
    ...
    try {
      ...
      whereClause.append(" left outer join ft.reconciliation as rec");
      whereClause.append(" where ft.account.id = ?");
      whereClause.append(" and ((ft.status<>'RPPC') or (ft.status='RPPC' and rec is not null and rec.processed='N'))");
      parameters.add(account.getId());
      if (hideAfterDate) {
        whereClause.append(" and ft.transactionDate < ?");
        parameters.add(statementDate);
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        FieldProviderFactory.setField(data[i], "transactionDate", dateFormater.format(FIN_Transactions[i].getTransactionDate()));
        ...
        String truncateBPname = (strBusinessPartner.length() > 30) ? strBusinessPartner.substring(0, 27).concat("...").toString() : strBusinessPartner;
        FieldProviderFactory.setField(data[i], "businessPartner", (strBusinessPartner.length() > 30) ? strBusinessPartner : "");
        ...
        if (description != null) {
          truncateDescription = (description.length() > 42) ? description.substring(0, 39).concat("...").toString() : description;
        }
        FieldProviderFactory.setField(data[i], "description", (description != null && description.length() > 42) ? description : "");
        FieldProviderFactory.setField(data[i], "descriptionTrunc", truncateDescription);

        FieldProviderFactory.setField(data[i], "paymentAmount", FIN_Transactions[i].getPaymentAmount().toString());
        FieldProviderFactory.setField(data[i], "depositAmount", FIN_Transactions[i].getDepositAmount().toString());
        FieldProviderFactory.setField(data[i], "rownum", "" + (i + 1));
        FieldProviderFactory.setField(data[i], "markSelectedId", (FIN_Transactions[i].getStatus().equals("RPPC")) ? FIN_Transactions[i].getId() : "");
      }
  ...
  public static FieldProvider[] getTransactionsFilteredNotMarked(FIN_FinancialAccount account, Date statementDate, boolean hideAfterDate) {
