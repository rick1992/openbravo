  ...
  protected JSONObject execute(Map<String, Object> parameters, String data) {
    ...
    try {
      ...
      for (Tab tab : window.getADTabList()) {
        String uiPattern = "";
        final boolean noVisibleAccess = org.openbravo.erpCommon.utility.WindowAccessData.hasNoVisibleAccess(dalConnectionProvider, roleId, tab.getId());
        if (noVisibleAccess) {
          uiPattern = "NV";
        } else {
          final boolean readOnlyAccess = org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(dalConnectionProvider, roleId, tab.getId());
          uiPattern = readOnlyAccess ? "RO" : tab.getUIPattern();
        }
        jsonUIPattern.put(tab.getId(), uiPattern);
      }
      final JSONObject json = new JSONObject();
      json.put("uiPattern", jsonUIPattern);
      final String autoSaveStr = Preferences.getPreferenceValue("Autosave", false, OBContext.getOBContext().getCurrentClient(), OBContext.getOBContext().getCurrentOrganization(), OBContext.getOBContext().getUser(), OBContext.getOBContext().getRole(), window);
      ...
      final String showConfirmationStr = Preferences.getPreferenceValue("ShowConfirmationDefault", false, OBContext.getOBContext().getCurrentClient(), OBContext.getOBContext().getCurrentOrganization(), OBContext.getOBContext().getUser(), OBContext.getOBContext().getRole(), window);
      ...
      for (WindowAccess winAccess : window.getADWindowAccessList()) {
        if (winAccess.getRole().getId().equals(roleId)) {
          for (TabAccess tabAccess : winAccess.getADTabAccessList()) {
            ...
            final Entity entity = ModelProvider.getInstance().getEntityByTableId(tabAccess.getTab().getTable().getId());
      ...
      try {
        securedProcess = "Y".equals(Preferences.getPreferenceValue("SecuredProcess", true, OBContext.getOBContext().getCurrentClient(), OBContext.getOBContext().getCurrentOrganization(), OBContext.getOBContext().getUser(), OBContext.getOBContext().getRole(), window));
      ...
      restrictedProcessesQry += " and not exists (select 1 from " //
          + " OBUIAPP_Process_Access a" + " where a.obuiappProcess = f.column.oBUIAPPProcess" + " and a.role.id = :role and a.active=true))"//
      ...
      restrictedProcessesQry += " and not exists (select 1 from ADProcessAccess a where a.process = f.column.process and " + " a.role.id = :role and a.active=true))";
