...
import pe.com.unifiedgo.accounting.data.SCOLoandocLine;

public class AddPaymentFromLoanDocument extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (vars.commandIn("DEFAULT")) {
      String strWindowId = vars.getGlobalVariable("inpwindowId",
          "AddPaymentFromLoanDocument|Window_ID");
      String strTabId = vars.getGlobalVariable("inpTabId", "AddPaymentFromLoanDocument|Tab_ID");
      String strScoLoanDocId = vars.getGlobalVariable("inpscoLoanDocId", "");
      String strFinancialAccountId = vars.getStringParameter("inpfinFinancialAccountId");

      try {
        printPage(response, vars, strScoLoanDocId, strWindowId, strTabId, strFinancialAccountId);
      } catch (Exception e) {
      ...
      String strScoLoanDocId = vars.getRequiredStringParameter("inpfinPaymentId");
      String strTabId = vars.getRequiredStringParameter("inpTabId");
      String strPaymentDate = vars.getRequiredStringParameter("inpPaymentDate");

      SCOLoanDoc loandoc = new AdvPaymentMngtDao().getObject(SCOLoanDoc.class, strScoLoanDocId);

      if (loandoc.getPayment() != null) {
      ...
      List<SCOLoandocLine> lineasTotales = loandoc.getSCOLoandocLineList();
      List<SCOLoandocLine> lineas = new ArrayList<SCOLoandocLine>();
      ...
      try {
        ...
        FIN_FinancialAccount finaccount = loandoc.getFinancialAccount();
        if (!FIN_Utility.isPeriodOpen(vars.getClient(), strDocBaseType,
            loandoc.getOrganization().getId(), strPaymentDate)) {
        ...
        String strPaymentDocumentNo = Utility.getDocumentNo(this, vars,
            "AddPaymentFromLoanDocument", "FIN_Payment", strDocTypeId, strDocTypeId, false, true);
        FIN_Payment payment = dao.getNewPayment(isReceipt, org,
            ...
            loandoc.getPaymentMethod(), finaccount, strPaymentAmount,
        ...
        if (addedGLITemsArray != null) {
          for (int i = 0; i < addedGLITemsArray.length(); i++) {
            ...
            String loandocInvId = glItem.getString("glitemLoanDocInvId");
            ...
            FIN_AddPayment.saveGLItem(payment, glItemAmt, dao.getObject(GLItem.class, strGLItemId),
                null, null, null, null, null, null, costCenter, null, null, glItemDescription, null,
                loandocInvId, null, null, null, null, null);
          }
        }
        ...
        boolean processed = loandoc.isProcessed();
        loandoc.setProcessed(false);
        OBDal.getInstance().save(loandoc);
        OBDal.getInstance().flush();

        OBDal.getInstance().refresh(loandoc);

        loandoc.setProcessed(processed);
        loandoc.setPayment(payment);
        OBDal.getInstance().save(loandoc);
        OBDal.getInstance().flush();
        ...
        OBDal.getInstance().rollbackAndClose();
        log4j.error("AddPaymentFromLoanDocument - SAVE AND PROCESS", ex);
        return;
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars,
      String strScoLoanDocId, String strWindowId, String strTabId, String strFinancialAccountId)
      throws Exception {
    ...
    try {
      ...
      XmlDocument xmlDocument = xmlEngine
          .readXmlTemplate("pe/com/unifiedgo/accounting/ad_actionbutton/AddPaymentFromLoanDocument")
          .createXmlDocument();
      ...
      xmlDocument.setParameter("paymentId", strScoLoanDocId);
      ...
      final Currency financialAccountCurrency = loandoc.getCurrency();
      ...
      xmlDocument.setParameter("currencyId", loandoc.getCurrency().getId());
      xmlDocument.setParameter("currencyName", loandoc.getCurrency().getISOCode());

      boolean isReceipt = true;
      boolean forcedFinancialAccountTransaction = FIN_AddPayment
          .isForcedFinancialAccountTransaction(loandoc.getFinancialAccount(),
              loandoc.getPaymentMethod(), isReceipt);
      ...
      try {
        ComboTableData comboTableData = new ComboTableData(vars, this, "LIST", "",
            ...
            Utility.getContext(this, vars, "#AccessibleOrgTree", "AddPaymentFromLoanDocument"),
            Utility.getContext(this, vars, "#User_Client", "AddPaymentFromLoanDocument"), 0);
        Utility.fillSQLParameters(this, vars, null, comboTableData, "AddPaymentFromLoanDocument",
            "");
      ...
      ArrayList<GLItem> itemloandocInvs = new ArrayList<GLItem>();

      // --NOW GET THE loan_glitem_id
      /*
       * Invoice loandocInv = loandoc.getInvoice(); final StringBuilder whereClausePuente = new
       * StringBuilder(); whereClausePuente.append(" as vesa "); whereClausePuente.append(
       * " where vesa.businessPartner.id = '" + loandocInv.getBusinessPartner().getId() + "'");
       * 
       * final OBQuery<VendorAccounts> obqParametersPuente = OBDal.getInstance()
       * .createQuery(VendorAccounts.class, whereClausePuente.toString());
       * obqParametersPuente.setFilterOnReadableClients(false);
       * obqParametersPuente.setFilterOnReadableOrganization(false); final List<VendorAccounts>
       * vendorAccounts = obqParametersPuente.list();
       * 
       * AccountingCombination combination = vendorAccounts.get(0).getScoPoloanAcct();
       * 
       * String stmGlitem = "SELECT c_glitem_id FROM c_glitem_acct WHERE glitem_debit_acct=?";
       * String glitemId = ""; ResultSet result; Vector<java.lang.Object> vector = new
       * Vector<java.lang.Object>(0); PreparedStatement st = null;
       * 
       * int iParameter = 0; try { st = this.getPreparedStatement(stmGlitem);
       * QueryTimeOutUtil.getInstance().setQueryTimeOut(st, SessionInfo.getQueryProfile());
       * iParameter++; UtilSql.setValue(st, iParameter, 12, null, combination.getId()); result =
       * st.executeQuery(); if (result.next()) { glitemId = result.getString("c_glitem_id"); }
       * result.close(); } catch (SQLException e) { log4j.error("SQL error in query: " + stmGlitem +
       * "Exception:" + e); throw new ServletException( "@CODE=" +
       * Integer.toString(e.getErrorCode()) + "@" + e.getMessage()); } catch (Exception ex) {
       * log4j.error("Exception in query: " + stmGlitem + "Exception:" + ex); throw new
       * ServletException("@CODE=@" + ex.getMessage()); } finally { try {
       * this.releasePreparedStatement(st); } catch (Exception ignore) { ignore.printStackTrace(); }
       * }
       * 
       * GLItem glitem = OBDal.getInstance().get(GLItem.class, glitemId); if (glitem != null) {
       * 
       * } else { throw new Exception( "Cuenta para loandocInv " +
       * loandocInv.getScrPhysicalDocumentno() + " no existe."); }
       */

      itemloandocInvs.add(loandoc.getLoanGlitem());
      ...
      int i = 0;
      for (GLItem itemloandocInv : itemloandocInvs) {

        try {
          JSONObject glItem = new JSONObject();
          glItem.put("glitemId", itemloandocInv.getId());
          glItem.put("glitemDesc", itemloandocInv.getIdentifier());
          // Amounts
          glItem.put("glitemPaidOutAmt", BigDecimal.ZERO);
          glItem.put("glitemReceivedInAmt", loandoc.getInvoice().getGrandTotalAmount());
          glItem.put("glitemDescription",
              "Por cobro de préstamo " + loandoc.getInvoice().getScrPhysicalDocumentno());
          glItem.put("glitemLoanDocInvId", loandoc.getInvoice().getId());
  ...
  private void businessPartnerBlocked(HttpServletResponse response, VariablesSecureApp vars,
    ...
    try {
      ...
      log4j.error("AddPaymentFromLoanDocument - Callback", e);
    }
  }
