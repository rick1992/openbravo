...
public class ActivationKey {
  ...
  private void loadRestrictions() {
    ...
    return;
    /*
     * if (isActive() && licenseClass == LicenseClass.STD && !golden) { // Don't read restrictions
     * for Standard instances return; }
     * 
     * try { // read restriction file from context directory String restrictionsFilePath = null; if
     * (DalContextListener.getServletContext() != null) { // Taking restrictions from Tomcat context
     * restrictionsFilePath = DalContextListener.getServletContext().getRealPath(
     * "src-loc/design/org/openbravo/erpCommon/obps/licenseRestrictions"); } else { // Not in Tomcat
     * context, taking restrictions from sources restrictionsFilePath =
     ...
     * 
     * File restrictionsFile = new File(restrictionsFilePath); log4j.debug("Restrictions file: " +
     * restrictionsFile.getAbsolutePath());
     * 
     * FileInputStream fis = new FileInputStream(restrictionsFile); byte fileContent[] = new
     * byte[(int) restrictionsFile.length()]; fis.read(fileContent); ByteArrayOutputStream bos = new
     * ByteArrayOutputStream(); decrypt(fileContent, getPublicKey(OB_PUBLIC_KEY), bos,
     * OB_PUBLIC_KEY); ObjectInputStream ois = new ObjectInputStream(new
     * ByteArrayInputStream(bos.toByteArray())); HashMap<String, ArrayList<String>> m1 =
     * (HashMap<String, ArrayList<String>>) ois.readObject(); ois.close();
     * 
     ...
     * 
     * if (isGolden()) { goldenExcludedArtifacts.addAll(m1.get(GOLDEN_EXCLUDED)); } } catch
     * (Exception e) { log4j.error("Error reading license restriction file", e); tier1Artifacts =
     * null; tier2Artifacts = null; goldenExcludedArtifacts = null; }
     */
  }
  ...
  private HashMap<String, CommercialModuleStatus> getSubscribedModules(boolean includeDisabled) {
    ...
    for (String moduleInfo : modulesInfo) {
      ...
      /*
       * Date validFrom = null; Date validTo = null; try { validFrom = sd.parse(moduleData[1]); if
       * (moduleData.length > 2) { validTo = sd.parse(moduleData[2]); } if (includeDisabled &&
       * !DisabledModules.isEnabled(Artifacts.MODULE, moduleData[0])) {
       * moduleList.put(moduleData[0], CommercialModuleStatus.DISABLED); } else if
       * (subscriptionActuallyConverted) { moduleList.put(moduleData[0],
       * CommercialModuleStatus.CONVERTED_SUBSCRIPTION); } else if (validFrom.before(now) &&
       * (validTo == null || validTo.after(now))) { moduleList.put(moduleData[0],
       * CommercialModuleStatus.ACTIVE); } else if (validFrom.after(now)) {
       * moduleList.put(moduleData[0], CommercialModuleStatus.NO_ACTIVE_YET); } else if (validTo !=
       * null && validTo.before(now)) { if (subscriptionConvertedProperty) {
       * moduleList.put(moduleData[0], CommercialModuleStatus.CONVERTED_SUBSCRIPTION); } else {
       * moduleList.put(moduleData[0], CommercialModuleStatus.EXPIRED); } } } catch (Exception e) {
  ...
  public boolean isModuleEnableable(Module module) {
    return true;
    /*
     * if (!module.isCommercial()) { return true; } if (!isActive()) { return false; }
     * 
     * HashMap<String, CommercialModuleStatus> moduleList = getSubscribedModules(false);
     * 
     * if (!moduleList.containsKey(module.getId())) { return false; }
     * 
     * CommercialModuleStatus status = moduleList.get(module.getId()); return status ==
     * CommercialModuleStatus.ACTIVE || status == CommercialModuleStatus.CONVERTED_SUBSCRIPTION;
     */
  }
