...
public class FIN_ReconciliationProcess implements org.openbravo.scheduling.Process {
    ...
    try {
      ...
      if (strAction.equals("P")) {
        ...
        if (reconciliation.getFINReconciliationLineVList().size() == 0) {
          ...
          msg.setMessage(Utility.parseTranslation(conProvider, vars, language, "@APRM_ReconciliationNoLines@" + ": " + reconciliation.getDocumentNo()));
          bundle.setResult(msg);
          return;
        } else {
          for (FIN_ReconciliationLine_v recLine : reconciliation.getFINReconciliationLineVList()) {
            boolean orgLegalWithAccounting = FIN_Utility.periodControlOpened(recLine.getFinancialAccountTransaction().getReconciliation().TABLE_NAME, recLine.getFinancialAccountTransaction().getReconciliation().getId(), recLine.getFinancialAccountTransaction().getReconciliation().TABLE_NAME + "_ID", "LE");
            if (!FIN_Utility.isPeriodOpen(recLine.getFinancialAccountTransaction().getClient().getId(), AcctServer.DOCTYPE_Reconciliation, recLine.getFinancialAccountTransaction().getOrganization().getId(), OBDateUtils.formatDate(recLine.getFinancialAccountTransaction().getDateAcct())) && orgLegalWithAccounting) {
              msg.setType("Error");
              msg.setTitle(Utility.messageBD(conProvider, "Error", language));
              msg.setMessage(String.format(Utility.parseTranslation(conProvider, vars, language, "@APRM_PeriodNotAvailableClearedItem@"), recLine.getIdentifier()));
              bundle.setResult(msg);
        ...
        if ("Y".equals(reconciliation.getPosted())) {
          msg.setType("Error");
          msg.setTitle(Utility.messageBD(conProvider, "Error", language));
          msg.setMessage(Utility.parseTranslation(conProvider, vars, language, "@PostedDocument@" + ": " + reconciliation.getDocumentNo()));
          bundle.setResult(msg);
          return;
        }
        final boolean isForceProcess = (Boolean) "6BF16EFC772843AC9A17552AE0B26AB7".equals(bundle.getProcessId());
        // Transaction exists
        if (!isForceProcess && !MatchTransactionDao.islastreconciliation(reconciliation)) {
          msg.setType("Error");
          msg.setTitle(Utility.messageBD(conProvider, "Error", language));
          msg.setMessage(Utility.parseTranslation(conProvider, vars, language, "@APRM_FutureReconciliationExists@"));
          bundle.setResult(msg);
          return;
        }
        if (existsDraftReconciliation(reconciliation.getAccount())) {
          msg.setType("Error");
          msg.setTitle(Utility.messageBD(conProvider, "Error", language));
          msg.setMessage(Utility.parseTranslation(conProvider, vars, language, "@APRM_DraftReconciliationExists@"));
          bundle.setResult(msg);
          return;
        }

        if (reconciliation.getSCOFixedcashRepositionList().size() != 0) {
          msg.setType("Error");
          msg.setTitle(Utility.messageBD(conProvider, "Error", language));
          msg.setMessage(Utility.parseTranslation(conProvider, vars, language, "@SCO_ReactReconciliationFFExists@"));
          bundle.setResult(msg);
          return;
        }
  ...
  private void updateReconciliations(FIN_Reconciliation reconciliation) {
    ...
    obc.add(Restrictions.ge(FIN_Reconciliation.PROPERTY_CREATIONDATE, reconciliation.getCreationDate()));
