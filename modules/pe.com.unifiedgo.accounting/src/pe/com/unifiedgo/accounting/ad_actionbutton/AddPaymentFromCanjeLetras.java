...
public class AddPaymentFromCanjeLetras extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (vars.commandIn("DEFAULT")) {
      String strWindowId = vars.getGlobalVariable("inpwindowId",
          "AddPaymentFromCanjeLetras|Window_ID");
      String strTabId = vars.getGlobalVariable("inpTabId", "AddPaymentFromCanjeLetras|Tab_ID");
      String strCanjeId = vars.getGlobalVariable("inpscoBoeToDiscountId", "");
      String strFinancialAccountId = vars.getStringParameter("inpfinFinancialAccountId");

      try {
        printPage(response, vars, strCanjeId, strWindowId, strTabId, strFinancialAccountId);
      } catch (Exception e) {
      ...
      String strCanjeId = vars.getRequiredStringParameter("inpfinPaymentId");
      String strTabId = vars.getRequiredStringParameter("inpTabId");
      String strPaymentDate = vars.getRequiredStringParameter("inpPaymentDate");

      SCOBoeToDiscount canje = new AdvPaymentMngtDao().getObject(SCOBoeToDiscount.class,
          strCanjeId);

      if (canje.getPayment() != null) {
      ...
      List<SCOBoetodiscLine> lineasTotales = canje.getSCOBoetodiscLineList();
      List<SCOBoetodiscLine> lineas = new ArrayList<SCOBoetodiscLine>();
      ...
      try {
        ...
        FIN_FinancialAccount finaccount = canje.getFinancialAccount();

        if (!FIN_Utility.isPeriodOpen(vars.getClient(), strDocBaseType,
            canje.getOrganization().getId(), strPaymentDate)) {
        ...
        FIN_Payment payment = dao.getNewPayment(isReceipt, org,
            ...
            canje.getPaymentMethod(), finaccount, strPaymentAmount,
        ...
        if (addedGLITemsArray != null) {
          for (int i = 0; i < addedGLITemsArray.length(); i++) {
            ...
            String boeId = glItem.getString("glitemBoeId");
            ...
            FIN_AddPayment.saveGLItem(payment, glItemAmt, dao.getObject(GLItem.class, strGLItemId),
                null, null, null, null, null, null, costCenter, null, null, glItemDescription,
                boeId, null, null, null, null, null, null);
          }
        }
        ...
        if (payment.getScrPaymenttype() != null
          ...
          payment.setScoIsmigrationpayment(true);// ya mandado a
          // cuenta financiera
          OBDal.getInstance().save(payment);
        ...
        boolean processed = canje.isProcessed();
        canje.setProcessed(false);
        OBDal.getInstance().save(canje);
        OBDal.getInstance().flush();

        OBDal.getInstance().refresh(canje);

        canje.setProcessed(processed);
        canje.setPayment(payment);
        OBDal.getInstance().save(canje);
        OBDal.getInstance().flush();
        ...
        OBDal.getInstance().rollbackAndClose();
        log4j.error("AddPaymentFromCanjeLetras - SAVE AND PROCESS", ex);
        return;
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strCanjeId,
      String strWindowId, String strTabId, String strFinancialAccountId) throws Exception {
    ...
    try {
      ...
      XmlDocument xmlDocument = xmlEngine
          .readXmlTemplate("pe/com/unifiedgo/accounting/ad_actionbutton/AddPaymentFromCanjeLetras")
          .createXmlDocument();
      ...
      xmlDocument.setParameter("paymentId", strCanjeId);
      ...
      final Currency financialAccountCurrency = canje.getCurrency();
      ...
      xmlDocument.setParameter("currencyId", canje.getCurrency().getId());
      xmlDocument.setParameter("currencyName", canje.getCurrency().getISOCode());

      boolean isReceipt = true;
      boolean forcedFinancialAccountTransaction = FIN_AddPayment
          .isForcedFinancialAccountTransaction(canje.getFinancialAccount(),
              canje.getPaymentMethod(), isReceipt);
      ...
      try {
        ...
        Utility.fillSQLParameters(this, vars, null, comboTableData, "AddPaymentFromCanjeLetras",
            "");
      ...
      ArrayList<GLItem> itemLetras = new ArrayList<GLItem>();
      // String defaultAccount = "1233102";
      for (int i = 0; i < lineas.size(); i++) {
        Invoice letra = lineas.get(i).getInvoiceref();

        /*
         * AccountingCombination comb = letra.getBusinessPartner().getCustomerAccountsList
         ...
         * whereClause.append(" where ad_isorgincluded('" + letra.getOrganization().getId() +
         * "',cusa.organization.id,'" + letra.getClient().getId() + "')<>-1");
         * 
         ...
         * .add(Restrictions.eq(SCOPostingMapping.PROPERTY_CLIENT, letra.getClient()));
         ...
         * posting_mapping_c.add(Restrictions.eq(SCOPostingMapping.
         * PROPERTY_ORGANIZATIONTHATTRIGGERSMAPPING, letra.getOrganization()));
         * 
        ...
        whereClausePuente.append(
            " where cusa.businessPartner.id = '" + letra.getBusinessPartner().getId() + "'");
        ...
        AccountingCombination combination = customerAccounts.get(0).getScoPuenteletraAcct();
        ...
        GLItem itemLetra = OBDal.getInstance().get(GLItem.class, glitemId);
        if (itemLetra != null) {

        } else {
          throw new Exception(
              "Cuenta para letra " + letra.getScrPhysicalDocumentno() + " no existe.");
        }

        itemLetras.add(itemLetra);
      }
      ...
      int i = 0;
      for (GLItem itemLetra : itemLetras) {

        try {
          JSONObject glItem = new JSONObject();
          glItem.put("glitemId", itemLetra.getId());
          glItem.put("glitemDesc", itemLetra.getIdentifier());
          // Amounts
          ...
          glItem.put("glitemDescription",
              "Por cobro de letra " + lineas.get(i).getInvoiceref().getScrPhysicalDocumentno());
          glItem.put("glitemBoeId", lineas.get(i).getInvoiceref().getId());
  ...
  private void businessPartnerBlocked(HttpServletResponse response, VariablesSecureApp vars,
    ...
    try {
      ...
      log4j.error("AddPaymentFromCanjeLetras - Callback", e);
    }
  }
