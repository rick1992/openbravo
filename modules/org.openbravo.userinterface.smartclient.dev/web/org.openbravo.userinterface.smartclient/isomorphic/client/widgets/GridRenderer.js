...
getInnerHTML : function () {
    ...
    template[2] = ";width:" + this._fieldWidths.sum() + "px";
    template[4] = tableHTML;
...
//                        component + " went asynchronous", "printing");
	...
	var cellHeight = this.cellHeight,
        skipNOBR = isc.Browser.isIE8Strict || this.wrapCells,
	...
	if (isc.Browser.isDOM) {
		...
		for (var rowNum = startRow; rowNum < endRow; rowNum++) {
            ...
            if (ariaEnabled && this.getRowRole != null) {
                var rowRole = this.getRowRole(rowNum, record);
                
                if (rowRole != null) output.append(" role='", rowRole, "'");

                // output attributes such as "selected"
                var ariaState = this.getRowAriaState(rowNum, record);
                if (ariaState) output.append(isc.Canvas.getAriaStateAttributes(ariaState));
            }
            output.append(gt);
...
_getCellValue : function (record, rowNum, colNum) { 
    ...
    if (isc.screenReader && this.screenReaderCellSeparator != null) {
        value += "<div style='overflow:hidden;width:0px;height:0px;'>" +
    ...
    // Not disabled - check for selected and/or over
        ...
        if (this.showSelectedStyle && this.selectionEnabled()) {
            ...
            if (this.canSelectCells) {
                ...
                isSelected = this.selection.isSelected(record); 
            }
...
refreshCellValue : function (rowNum, colNum) {
    ...
    if (writeDiv) {
        ...
        if (!drawRecordAsSingleCell) {
            sb.append(this._getFieldDivWidthHTML(colNum));
        }
    ...
    var writeNOBR = !this.wrapCells && !isc.Browser.isIE8Strict;
    if (writeNOBR) {
        sb.append("<NOBR>");
    }
...
mouseMove : function (arg1, arg2) {
    ...
    if (notSameMouseRowCol || returnedBackToCurrentGrid) {
        ...
        if (hasNewCell || returnedBackToCurrentGrid) {
...
_shouldSelectOnMouseUp : function () {
    if (isc.EH.dragTarget != null && isc.EH.dragOperation == isc.EH.DRAG_SCROLL) {
        return true;
    }
    return false;
},
