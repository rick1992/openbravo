...
public class ReporteIngresosPorCompras extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
    ...
    if (vars.commandIn("DEFAULT")) {
      String strOrg = vars.getGlobalVariable("inpOrg", "ReporteIngresosPorCompras|Org", "");
      String strDocumentno = vars.getGlobalVariable("inpDocumentno",
          "ReporteIngresosPorCompras|Documentno", "");
      String strDocDate = vars.getGlobalVariable("inpDocDate", "ReporteIngresosPorCompras|docDate",
          SREDateTimeData.FirstDayOfMonth(this));
      String strDateto = vars.getGlobalVariable("inpDateTo", "ReporteIngresosPorCompras|dateTo",
          SREDateTimeData.today(this));
      String showLines = vars.getGlobalVariable("inpMostrarDetalles",
          "ReporteIngresosPorCompras|MostrarDetalles", "");
  ...
  private void printPagePDF(HttpServletRequest request, HttpServletResponse response,
      ...
      String strDateTo, String adLanguage, String showLines,String strIdProveedor, String showSinIngresos,String strIdProducto) throws IOException, ServletException {
		...
		String paramShownoIngresos = null;
		if(showSinIngresos.compareTo("Y")==0){
			paramShownoIngresos = "Y";
		}
		
	    ReporteIngresosPorComprasData[] data = null;
	    ...
	    try {
	        data = ReporteIngresosPorComprasData.select(this, Utility
					.getContext(this, vars, "#User_Client",
							"ReporteIngresosPorCompras"),
							strOrgFamily, strDocumentno,strIdProveedor, strDateFrom, strDateTo,strIdProducto,paramShownoIngresos);
	    } catch (OBException e) {
	      advisePopUp(request, response, "WARNING",
	          Utility.messageBD(this, "ProcessStatus-W", vars.getLanguage()),
	          Utility.messageBD(this, "FINPR_NoConversionFound", vars.getLanguage()) + e.getMessage());
	      data = ReporteIngresosPorComprasData.set("");
	    }
	    int ini=0;
	    int lim=0;
	    String estado="";
	    ...
	    for(int i=0 ; i< data.length ; i ++){
	    	...
	    	ReporteIngresosPorComprasData current = data[i];
	    	
	    	current.saldoTotales=new BigDecimal(current.cantTotalOrder).subtract(new BigDecimal(current.cantTotalGuia)).toString();
	    	current.saldoXProducto=new BigDecimal(current.cantProOrder).subtract(new BigDecimal(current.cantProGuia)).toString();
	    	
	    	if(lim==i||i==0){
	    		
	    		String order_id=current.ordenid;
	    		estado="";
	        	for (lim=i;lim<data.length && data[lim].ordenid.equals(order_id) ;lim++){
		
	        		BigDecimal num1=new BigDecimal(data[lim].cantProReservada);
	        		BigDecimal num2=new BigDecimal(data[lim].cantProOrder);

	        		if(num1.compareTo(num2.multiply(new BigDecimal("-1")))!=0){
	        			estado="Pendiente";
	        		}
	        		
	        		num1=new BigDecimal(data[lim].cantProGuia);
	        		num2=new BigDecimal(data[lim].cantProOrder);
	    	...
	    	if(!estado.equals(""))
	    		current.estadoorden=estado;
	    	
	    }
    
    strOutput = "pdf";
    strReportName = "@basedesign@/pe/com/unifiedgo/report/ad_reports/ReporteIngresosPorCompras2.jrxml";
  ...
  private void printPageXLS(HttpServletRequest request, HttpServletResponse response,
    ...
    try {
        data = ReporteIngresosPorComprasData.select(this, Utility
				.getContext(this, vars, "#User_Client",
						"ReporteIngresosPorCompras"),
						strOrgFamily, documentNo,strIdProveedor, strDateFrom, strDateTo,strIdProducto,paramShownoIngresos);
    } catch (OBException e) {
      advisePopUp(request, response, "WARNING",
          Utility.messageBD(this, "ProcessStatus-W", vars.getLanguage()),
          Utility.messageBD(this, "FINPR_NoConversionFound", vars.getLanguage()) + e.getMessage());
    ...
    strOutput = "xls";
    strReportName = "@basedesign@/pe/com/unifiedgo/report/ad_reports/ReporteIngresosPorComprasExcel2.jrxml";
  ...
  private void printPageDataSheet(HttpServletRequest request, HttpServletResponse response,
    ...
    // ReporteIngresosPorComprasData[] datausers = null;

    ReporteIngresosPorComprasData[] dataorgs = null;

    String strConvRateErrorMsg = "";
    String discard[] = { "discard" };

    xmlDocument = xmlEngine.readXmlTemplate(
        "pe/com/unifiedgo/report/ad_reports/ReporteIngresosPorCompras", discard)
        .createXmlDocument();
    ...
    if (strConvRateErrorMsg.equals("") || strConvRateErrorMsg == null) {
      ...
      ToolBar toolbar = new ToolBar(this, vars.getLanguage(), "ReporteIngresosPorCompras", false,
      ...
      try {
        WindowTabs tabs = new WindowTabs(this, vars,
            "pe.com.unifiedgo.report.ad_reports.ReporteIngresosPorCompras");
        xmlDocument.setParameter("parentTabContainer", tabs.parentTabs());
        xmlDocument.setParameter("mainTabContainer", tabs.mainTabs());
        xmlDocument.setParameter("childTabContainer", tabs.childTabs());
        xmlDocument.setParameter("theme", vars.getTheme());
        NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
            "ReporteIngresosPorCompras.html", classInfo.id, classInfo.type, strReplaceWith,
            tabs.breadcrumb());
        xmlDocument.setParameter("navigationBar", nav.toString());
        LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
            "ReporteIngresosPorCompras.html", strReplaceWith);
      ...
      try {
        ...
        dataorgs = ReporteIngresosPorComprasData.selectOrgs(this);
      ...
      {
        OBError myMessage = vars.getMessage("ReporteIngresosPorCompras");
        vars.removeMessage("ReporteIngresosPorCompras");
        if (myMessage != null) {
