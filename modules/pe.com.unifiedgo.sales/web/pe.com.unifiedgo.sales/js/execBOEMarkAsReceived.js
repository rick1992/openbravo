...
OB.SSA.execBOEMarkAsReceived = {
  execute: function (params, view) {
    ...
    for (i = 0; i < selection.length; i++) {
     ...
     isc.SSA_BOEMarkAsReceivedPopup.create({
  ...
  execFunction: function (params, view) {
    params.actionHandler = 'pe.com.unifiedgo.sales.BOEMarkAsReceivedActionHandler';
    params.adTabId = view.activeView.tabId;
    OB.SSA.execBOEMarkAsReceived.execute(params, view);
  }

};

isc.defineClass('SSA_BOEMarkAsReceivedPopup', isc.OBPopup);

isc.SSA_BOEMarkAsReceivedPopup.addProperties({
  ...
  initWidget: function () {
	...
	this.confirmation = isc.Label.create({
		    ...
		    wrap: false,
		    showEdges: false,
		    contents: OB.I18N.getLabel('SSA_BOEMarkAsReceived_confirmation')
    });
