...
public class SCO_FactoringInvoice_DocType extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strSCOFactoringinvoiceId = vars.getStringParameter("inpscoFactoringinvoiceId");
      String strDescription = vars.getStringParameter("inpdescription");
      String strTabId = vars.getStringParameter("inpTabId");

      try {
        printPage(response, vars, strDocTypeTarget, strDocType, docNo, strSCOFactoringinvoiceId, strDescription, strTabId);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strDocTypeTarget, String strDocType, String docNo, String strSCOFactoringinvoiceId, String strDescription, String strTabId) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    if (strDocTypeTarget.equals(""))
      ...
      resultado.append("var calloutName='SCO_Factoringinvoice_DocType';\n\n");
      ...
      SCOFactoringInvoiceDocTypeData[] data = null;

      if (!newDocNo && !"0".equals(strDocType)) {
        data = SCOFactoringInvoiceDocTypeData.select(this, strDocType);
      ...
      SCOFactoringInvoiceDocTypeData[] dataNew = SCOFactoringInvoiceDocTypeData.select(this, strDocTypeTarget);
      if (dataNew != null && dataNew.length > 0) {
        DocSubTypeSO = dataNew[0].docsubtypeso;
        if (DocSubTypeSO == null)
          DocSubTypeSO = "--";
        String strOldDocTypeTarget = SCOFactoringInvoiceDocTypeData.selectOldDocSubType(this, strSCOFactoringinvoiceId);
        if (!DocSubTypeSO.equals("OB") && strOldDocTypeTarget.equals("OB")) {
          String strOldDocNo = SCOFactoringInvoiceDocTypeData.selectOldDocNo(this, strSCOFactoringinvoiceId);
        ...
        if (dataNew[0].isdocnocontrolled.equals("Y")) {
          if (!newDocNo && !AD_Sequence_ID.equals(dataNew[0].adSequenceId) && !SCOFactoringInvoiceDocTypeData.selectOldDocTypeTargetId(this, strSCOFactoringinvoiceId).equalsIgnoreCase(strDocTypeTarget))
            newDocNo = true;
          if (newDocNo) {
