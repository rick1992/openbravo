...
OB.SCR.execCheckInvToDiscountComission = {
  execute: function (params, view) {
    ...
    for (i = 0; i < selection.length; i++) {
     ...
     isc.SCR_CheckInvToDiscountComissionPopup.create({
  ...
  execFunction: function (params, view) {
    params.actionHandler = 'pe.com.unifiedgo.core.CheckInvToDiscountComissionActionHandler';
    params.adTabId = view.activeView.tabId;
    OB.SCR.execCheckInvToDiscountComission.execute(params, view);
  }

};

isc.defineClass('SCR_CheckInvToDiscountComissionPopup', isc.OBPopup);

isc.SCR_CheckInvToDiscountComissionPopup.addProperties({
  ...
  initWidget: function () {
	...
	this.confirmation = isc.Label.create({
		    ...
		    wrap: false,
		    showEdges: false,
		    contents: OB.I18N.getLabel('SCR_ChkInvToDiscComission_confirm')
    });
