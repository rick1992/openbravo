 ...
 * @throws OBException 
  ...
  public static BigDecimal getTransactionCost(MaterialTransaction transaction, Date date,
      Currency currency) throws OBException, ImportException {
    return getTransactionCost(transaction, date, true, currency);
  }
 ...
 * @throws OBException 
   */
  public static BigDecimal getTransactionCost(MaterialTransaction transaction, Date date,
      boolean calculateTrx, Currency currency) throws OBException, ImportException {
  ...
  public static Costing getStandardCostDefinition(Product product, Organization org, Date date,
    ...
    if (stdCost == null) {
      ...
      // If no cost is found, search valid legacy cost
    	stdCost = getStandardCostDefinition(product, org, date, costDimensions,
          recheckWithoutDimensions, "ST");
    	if(stdCost == null){
    		stdCost = getStandardCostDefinition(product, org, date, costDimensions,
    		          recheckWithoutDimensions, "AVA");
    	
    		if(stdCost == null){
        		stdCost = getStandardCostDefinition(product, org, date, costDimensions,
        		          recheckWithoutDimensions, "AV");
    		}
