...
public class Menu extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (!OBContext.getOBContext().isAdminContext() && request.getParameter("noprefs") == null) {
      try {
        final String startPage = Preferences.getPreferenceValue("StartPage", true, OBContext.getOBContext().getCurrentClient(), OBContext.getOBContext().getCurrentOrganization(), OBContext.getOBContext().getUser(), OBContext.getOBContext().getRole(), null);
    ...
    printPageFrameIdentificacion(response, menuURL, (targetmenu.equals("") ? "../utility/Home.html" : targetmenu), menuLoadingURL, textDirection, hideMenu, vScroll);
  }

  private void printPageFrameIdentificacion(HttpServletResponse response, String strMenu, String strDetalle, String strMenuLoading, String textDirection, String hideMenu, String vScroll) throws IOException, ServletException {
  ...
  private String getTargetMenu(VariablesSecureApp vars, String queryString) throws ServletException {
    ...
    if (qString != null && qString.contains("url") && url != null && !url.equals("")) {
      if (!url.startsWith("/")) {
        log4j.error("Invalid deep-link URL: URL parameter is relative to application context, must start with slash");
        return "";
      }
      // Removing "url=" from query string
      targetmenu = getDireccion() + qString.substring(4);
    ...
    try { // Trying to deep-link using tabId
      ...
      if (!windowId.equals("")) {
        ...
        if (!tab.getWindow().equals(window)) {
          log4j.error("Invalid deep-link URL: tab " + tabId + " doesn't belong to window " + windowId);
          return "";
        }
      } else {
      ...
      if (vars.commandIn("EDIT")) {
        ...
        if (ob == null) {
          log4j.error("Invalid deep-link URL: Record id: " + recordId + " doesn't exist in table: " + table.getDBTableName());
          return "";
        }
      ...
      // Getting the tab URL
      final String tabURL = Utility.getTabURL(tabId, type, true, getDireccion());
