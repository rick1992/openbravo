...
isc.OBGrid.addProperties({
  ...
  toggleSort: function (fieldName, direction) {
	...
	var fullSortingIdentifierName = fieldName + OB.Constants.FIELDSEPARATOR + OB.Constants.SORTINGIDENTIFIER;
    if (this.lazyFiltering) {
      this.sortingHasChanged = true;
      ...
      if (this.savedSortSpecifiers && this.savedSortSpecifiers.length > 0) {
        if (this.savedSortSpecifiers[0].property === fieldName || this.savedSortSpecifiers[0].property === fullIdentifierName || this.savedSortSpecifiers[0].property === fullSortingIdentifierName) {
  ...
  isSortField: function (fieldName) {
    var i, len, 
    fullIdentifierName = fieldName + OB.Constants.FIELDSEPARATOR + OB.Constants.IDENTIFIER,
    fullSortingIdentifierName = fieldName + OB.Constants.FIELDSEPARATOR + OB.Constants.SORTINGIDENTIFIER;
    if (this.lazyFiltering) {
      if (!this.savedSortSpecifiers) {
        return false;
      } else {
        ...
        for (i = 0; i < len; i++) {
          if (this.savedSortSpecifiers[i].property === fieldName || this.savedSortSpecifiers[0].property === fullIdentifierName || this.savedSortSpecifiers[0].property === fullSortingIdentifierName) {
            return true;
          }
        }
        return false;
  ...
  getFieldByName: function (fieldname) {
	  ...
	  var separatorIndex = fieldname.lastIndexOf(OB.Constants.FIELDSEPARATOR);
      if (separatorIndex !== -1) {
    	  return null;
      }
