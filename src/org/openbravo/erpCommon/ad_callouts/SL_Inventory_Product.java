...
public class SL_Inventory_Product extends SimpleCallout {
  ...
  protected void execute(CalloutInfo info) throws ServletException {
    ...
    if (strIsOutTrx != null) {
      info.addResult("inpemSwaInoutqtycount", "0");
    }
    
    //Add by Vafaster. add value in new field inpemSwaProductvalue
    
    try {
    	Product product = OBDal.getInstance().get(Product.class, strMProductID);
    	info.addResult("inpemSwaProductvalue",product.getSearchKey());	
	} catch (Exception e) {
