...
public class SCO_Pwithholding_Receipt_DocType extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strSCOPwithholdingReceiptId = vars.getStringParameter("inpscoPwithholdingReceiptId");
      String strDescription = vars.getStringParameter("inpdescription");
      String strTabId = vars.getStringParameter("inpTabId");

      try {
        printPage(response, vars, strDocTypeTarget, strDocType, docNo, strSCOPwithholdingReceiptId, strDescription, strTabId);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strDocTypeTarget, String strDocType, String docNo, String strSCOPwithholdingReceiptId, String strDescription, String strTabId) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    if (strDocTypeTarget.equals(""))
      ...
      resultado.append("var calloutName='SCO_Pwithholding_Receipt_DocType';\n\n");
      ...
      SCOPwithholdingReceiptDocTypeData[] data = null;

      if (!newDocNo && !"0".equals(strDocType)) {
        data = SCOPwithholdingReceiptDocTypeData.select(this, strDocType);
      ...
      SCOPwithholdingReceiptDocTypeData[] dataNew = SCOPwithholdingReceiptDocTypeData.select(this, strDocTypeTarget);
      if (dataNew != null && dataNew.length > 0) {
        DocSubTypeSO = dataNew[0].docsubtypeso;
        if (DocSubTypeSO == null)
          DocSubTypeSO = "--";
        String strOldDocTypeTarget = SCOPwithholdingReceiptDocTypeData.selectOldDocSubType(this, strSCOPwithholdingReceiptId);
        if (!DocSubTypeSO.equals("OB") && strOldDocTypeTarget.equals("OB")) {
          String strOldDocNo = SCOPwithholdingReceiptDocTypeData.selectOldDocNo(this, strSCOPwithholdingReceiptId);
        ...
        if (dataNew[0].isdocnocontrolled.equals("Y")) {
          if (!newDocNo && !AD_Sequence_ID.equals(dataNew[0].adSequenceId) && !SCOPwithholdingReceiptDocTypeData.selectOldDocTypeTargetId(this, strSCOPwithholdingReceiptId).equalsIgnoreCase(strDocTypeTarget))
            newDocNo = true;
          if (newDocNo) {
