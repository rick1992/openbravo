...
public class SCO_Loan_Doc_DocType extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strSCOLoanDocId = vars.getStringParameter("inpscoLoanDocId");
      String strDescription = vars.getStringParameter("inpdescription");
      String strTabId = vars.getStringParameter("inpTabId");

      try {
        printPage(response, vars, strDocTypeTarget, strDocType, docNo, strSCOLoanDocId,
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars,
      String strDocTypeTarget, String strDocType, String docNo, String strSCOLoanDocId,
    ...
    if (strDocTypeTarget.equals(""))
      ...
      resultado.append("var calloutName='SCO_Loan_Doc_DocType';\n\n");
      ...
      SCOLoanDocDocTypeData[] data = null;

      if (!newDocNo && !"0".equals(strDocType)) {
        data = SCOLoanDocDocTypeData.select(this, strDocType);
      ...
      SCOLoanDocDocTypeData[] dataNew = SCOLoanDocDocTypeData.select(this, strDocTypeTarget);
      if (dataNew != null && dataNew.length > 0) {
        DocSubTypeSO = dataNew[0].docsubtypeso;
        if (DocSubTypeSO == null)
          DocSubTypeSO = "--";
        String strOldDocTypeTarget = SCOLoanDocDocTypeData.selectOldDocSubType(this,
            strSCOLoanDocId);
        if (!DocSubTypeSO.equals("OB") && strOldDocTypeTarget.equals("OB")) {
          String strOldDocNo = SCOLoanDocDocTypeData.selectOldDocNo(this, strSCOLoanDocId);
        ...
        if (dataNew[0].isdocnocontrolled.equals("Y")) {
          if (!newDocNo && !AD_Sequence_ID.equals(dataNew[0].adSequenceId) && !SCOLoanDocDocTypeData
              .selectOldDocTypeTargetId(this, strSCOLoanDocId).equalsIgnoreCase(strDocTypeTarget))
            newDocNo = true;
          if (newDocNo) {
            if (vars.getRole().equalsIgnoreCase("System")
                && new BigDecimal(vars.getClient()).compareTo(new BigDecimal("1000000.0")) < 0)
              resultado.append(
                  ", new Array(\"inpdocumentno\", \"<" + dataNew[0].currentnextsys + ">\")\n");
            else
              resultado
                  .append(", new Array(\"inpdocumentno\", \"<" + dataNew[0].currentnext + ">\")\n");
          }
        }
