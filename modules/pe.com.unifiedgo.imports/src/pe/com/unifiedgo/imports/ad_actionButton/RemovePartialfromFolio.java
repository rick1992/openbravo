...
import pe.com.unifiedgo.imports.ad_actionButton.RemovePartialfromFolio;
...
public class RemovePartialfromFolio extends HttpSecureAppServlet {
 ...
 //   	    { System.out.println(params.nextElement()); }
         ...
         strDuaID = vars.getStringParameter("inpscoDuaId");
       	 ...
       	 printPageDataSheet2(response, vars, 
                     strWindowId, 
                     strTabId 
                     );
    	 ...
    	 if(temporal == 0) {
    		 try {
   	    		for(int i = 0; selectedLines != null && i < selectedLines.length; i++) {
   	    			final int total = RemovePartialfromFolioData.removePartialWithFolio(this, selectedLines[i].trim());
   	        	}
  ...
  private void printPageDataSheet2(HttpServletResponse response, VariablesSecureApp vars,
                                  String strWindowId,
                                  String strTabId) throws IOException, ServletException {
	  
	  
	  
	  RemovePartialfromFolioData[] data = null;
	 // AddPartialtoFolioData[] dataLines = null;
	    XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
		        "pe/com/unifiedgo/imports/ad_actionButton/RemovePartialfromFolio").createXmlDocument();

	    OBError myMessage = null;
	      myMessage = new OBError();
	      try {
	          data = RemovePartialfromFolioData.select4(this, strFolioID);
	    	  //data = AddPartialtoFolioData.select3(this);
