...
public class AcctServerProcess extends DalBaseProcess {
  ...
  private String[] AdicionalWhereClauses = null;
  ...
  static public void writeToLog(String text, String logfilepath) {
    try {
  ...
  static public void writeToLog(String text) {
    String logfilepath = "/tmp/erplima.log";
    writeToLog(text, logfilepath);
  }
  ...
  private void processClient(VariablesSecureApp vars, ProcessBundle bundle)
    ...
    // use process Organization <--- UGO FIX : if isDirect then ignore the preference
    if (AcctServerProcessData.useRequestProcessOrg(connection) && !isDirect) {
      strOrg = vars.getOrg();
    ...
    if (isDirect) {
      ...
      // reinicilizar cuentas con error
      if (!"".equals(strTable)) {
        ResetAccounting.restoreErrorOnly(vars.getClient(), strOrg, strTable, strDateFrom,
            strDateTo);
      } else {
        ResetAccounting.restoreErrorOnly(vars.getClient(), strOrg, strDateFrom, strDateTo);
      }

    } else {
      // reinicilizar cuentas con error
      String myQuery = "select distinct e.organization.id from OrganizationAcctSchema e where client.id = :clientId and active= true";
      Query query = OBDal.getInstance().getSession().createQuery(myQuery);
      query.setString("clientId", vars.getClient());
      List<String> orgs = query.list();
      for (int i = 0; i < orgs.size(); i++) {
        if (!"".equals(strTable)) {
          ResetAccounting.restoreErrorOnly(vars.getClient(), orgs.get(i), strTable, strDateFrom,
              strDateTo);
        } else {
          ResetAccounting.restoreErrorOnly(vars.getClient(), orgs.get(i), strDateFrom, strDateTo);
        }
      }
    }
