...
public class DocInventory extends AcctServer {
  ...
  private DocLine[] loadLines(ConnectionProvider conn) {
    ...
    try {
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        BigDecimal QtySurplus = new BigDecimal(data[i].getField("emSwaQtysurplus"));
        docLine.setQty((QtyCount.add(QtySurplus).subtract(QtyBook)).toString(), conn);
  ...
  public Fact createFact(AcctSchema as, ConnectionProvider conn, Connection con,
    ...
    InventoryCount inv = OBDal.getInstance().get(InventoryCount.class, Record_ID);
    ...
    for (int i = 0; i < p_lines.length; i++) {
      ...
      if (invDiff == null) {
    	if(b_Costs.negate().compareTo(BigDecimal.ZERO)<0)  
    		invDiff = getAccount(AcctServer.ACCTTYPE_InvGain, as, conn);
    	else{
    		invDiff = getAccount(AcctServer.ACCTTYPE_InvDifferences, as, conn);
    		
    		OBCriteria<CostCenter> obCriteria = OBDal.getInstance().createCriteria(CostCenter.class);
			obCriteria.add(Restrictions.eq(CostCenter.PROPERTY_ORGANIZATION, inv.getTransactionDocument().getOrganization()));
			obCriteria.add(Restrictions.like(CostCenter.PROPERTY_NAME, "%ALMACEN%"));
			obCriteria.setFilterOnReadableClients(false);
			obCriteria.setFilterOnReadableOrganization(false);
			List<CostCenter> lines = obCriteria.list();
			
    		if(lines.size()>0) centrocostos=lines.get(0).getId();
    		line.m_C_Costcenter_ID = centrocostos;
    	}
  ...
  public boolean getDocumentConfirmation(ConnectionProvider conn, String strRecordId) {
	  ...
	  InventoryCount inv = OBDal.getInstance().get(InventoryCount.class, strRecordId);
	  if(inv.getPosted().equals("sco_M")){
