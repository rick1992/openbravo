...
OB.Layout.initialize = function() {
  ...
  OB.TopLayout.ConvRate = isc.Label.create({
      width: '550',
      height: '100%',
      padding: '0',
      align: 'right',
  ...
  OB.TestRegistry.register('org.openbravo.client.application.convrate', OB.TopLayout.ConvRate);    

  OB.TopLayout.ConvRateRefreshButton = isc.OBRefreshConvrate.create({});
  OB.TestRegistry.register('org.openbravo.client.application.convrateRefreshButton', OB.TopLayout.ConvRateRefreshButton);    
  OB.TopLayout.ConvRateRefreshButton.hide();
  
  OB.TopLayout.ConvRateContainer = isc.HLayout.create({
        members: [OB.TopLayout.ConvRate, OB.TopLayout.ConvRateRefreshButton]
    }, OB.Styles.TopLayout.ConvRateContainer);
  
  OB.TestRegistry.register('org.openbravo.client.application.convrateContainer', OB.TopLayout.ConvRateContainer);    
  ...
  OB.TopLayout.addMember(
    isc.HLayout.create({
        members: [OB.TopLayout.ConvRateContainer, OB.TopLayout.CompanyImageLogo, OB.TopLayout.OpenbravoLogo]
  ...
  (function _OB_checkHeartBeatRegistration() {
   ...
   var mlrcvcallback = function (rpcResponse, data, rpcRequest) {
        var status = rpcResponse.status;
        if(data.showconvrate && data.showconvrate == 'Y'){
          if (data.message) {
            OB.TopLayout.ConvRate.setContents(data.message.text);
            OB.TopLayout.ConvRateRefreshButton.show();
          }
        }
        else{
          OB.TopLayout.ConvRate.setContents('');
          //hide refresh button
          OB.TopLayout.ConvRateRefreshButton.hide();
        }
    };
    
    OB.RemoteCallManager.call('org.openbravo.client.application.RefreshConvRateButtonActionHandler', {}, {},  mlrcvcallback);
