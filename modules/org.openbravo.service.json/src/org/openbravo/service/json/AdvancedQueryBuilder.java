...
public class AdvancedQueryBuilder {
  ...
  public String getWhereClause() {
    ...
    if (subEntity != null) {
      ...
      String subEntityClientOrg = " and e.organization.id " + createInClause(OBContext.getOBContext().getReadableOrganizations());
      subEntityClientOrg += " and e.client.id " + createInClause(OBContext.getOBContext().getReadableClients());
      ...
      if (!distinctPropertyPath.contains(DalUtil.FIELDSEPARATOR) && subEntity.getProperty(distinctPropertyPath).allowNullValues()) {
        whereClause += " exists (select 1 from " + subEntity.getName() + " " + subEntityQueryBuilder.getJoinClause() + " left join " + subEntityQueryBuilder.getMainAlias() + DalUtil.DOT + distinctPropertyPath + " as i " + subentityWhere + " i = " + mainAlias + subEntityClientOrg + ") ";
      } else {
        whereClause += " exists (select 1 from " + subEntity.getName() + " " + subEntityQueryBuilder.getJoinClause() + subentityWhere + "e." + distinctPropertyPath.replace(DalUtil.FIELDSEPARATOR, DalUtil.DOT) + " = " + mainAlias + subEntityClientOrg + ") ";
      }
  ...
  private String addWhereOrgParameters(String where) {
    ...
    if (filterParameters.containsKey(JsonConstants.ORG_PARAMETER)) {
      ...
      if (entity.isOrganizationEnabled() && value != null && value.length() > 0) {
        final Set<String> orgs = OBContext.getOBContext().getOrganizationStructureProvider().getNaturalTree(value);
        if (orgs.size() > 0) {
      ...
      if (localWhereClause == null || localWhereClause.length() == 0) {
        ...
        localWhereClause = "(" + localWhereClause + ")" + (orgPart.length() > 0 ? " and " + orgPart.toString() : "");
      }
    }
  ...
  private String substituteParameters(String where) {
    ...
    filterParameters.put(JsonConstants.QUERY_PARAM_USER, OBContext.getOBContext().getUser().getId());
    if (!filterParameters.containsKey(JsonConstants.QUERY_PARAM_CLIENT)) {
      filterParameters.put(JsonConstants.QUERY_PARAM_CLIENT, OBContext.getOBContext().getCurrentClient().getId());
    }
    ...
    if (localWhereClause.contains(JsonConstants.QUERY_PARAM_TRANSACTIONAL_RANGE)) {
      ...
      final String range = Utility.getTransactionalDate(new DalConnectionProvider(false), RequestContext.get().getVariablesSecureApp(), windowId);
      final int rangeNum = Integer.parseInt(range);
      final Calendar cal = Calendar.getInstance();
      cal.add(Calendar.DAY_OF_MONTH, -1 * rangeNum);
      localWhereClause = localWhereClause.replace(JsonConstants.QUERY_PARAM_TRANSACTIONAL_RANGE, alias);
  ...
  private boolean hasOrAndOperator(JSONObject jsonCriteria) throws JSONException {
    ...
    return OPERATOR_OR.equals(jsonCriteria.get(OPERATOR_KEY)) || OPERATOR_AND.equals(jsonCriteria.get(OPERATOR_KEY));
  }
  ...
  private String parseSingleClause(JSONObject jsonCriteria) throws JSONException {
    ...
    if (operator.equals(OPERATOR_BETWEEN) || operator.equals(OPERATOR_BETWEENINCLUSIVE) || operator.equals(OPERATOR_IBETWEEN) || operator.equals(OPERATOR_IBETWEENINCLUSIVE)) {
    ...
    if (jsonCriteria.has("minutesTimezoneOffset")) {
      int clientMinutesTimezoneOffset = Integer.parseInt(jsonCriteria.get("minutesTimezoneOffset").toString());
      Calendar now = Calendar.getInstance();
      // Obtains the UTC time zone offset of the server
      int serverMinutesTimezoneOffset = (now.get(Calendar.ZONE_OFFSET) + now.get(Calendar.DST_OFFSET)) / (1000 * 60);
    ...
    if (operator.equals(OPERATOR_EQUALS) || operator.equals(OPERATOR_EQUALSFIELD)) {
      ...
      if (property.isDate() || property.isDatetime() || property.isAbsoluteDateTime()) {
        if (operator.equals(OPERATOR_EQUALS)) {
          return "(" + parseSimpleClause(fieldName, OPERATOR_GREATEROREQUAL, value) + " and " + parseSimpleClause(fieldName, OPERATOR_LESSOREQUAL, value) + ")";

        } else {
          return "(" + parseSimpleClause(fieldName, OPERATOR_GREATEROREQUALFIELD, value) + " and " + parseSimpleClause(fieldName, OPERATOR_LESSOREQUALFIElD, value) + ")";
        }
      }
    }
  ...
  private String parseBetween(JSONObject jsonCriteria, String operator, boolean inclusive) throws JSONException {
    ...
    final String leftClause = parseSimpleClause(fieldName, getBetweenOperator(operator, false), start);
  ...
  private String parseSimpleClause(String fieldName, String operator, Object value) throws JSONException {
  ...
  private String buildRightClause(Property property, String operator, Object value) throws JSONException {
    ...
    if (operator.equals(OPERATOR_EQUALSFIELD) || operator.equals(OPERATOR_NOTEQUALFIELD) || operator.equals(OPERATOR_GREATERTHANFIElD) || operator.equals(OPERATOR_LESSTHANFIELD) || operator.equals(OPERATOR_GREATEROREQUALFIELD) || operator.equals(OPERATOR_LESSOREQUALFIElD) || operator.equals(OPERATOR_CONTAINSFIELD) || operator.equals(OPERATOR_STARTSWITHFIELD) || operator.equals(OPERATOR_ENDSWITHFIELD)) {
      ...
      if (properties.isEmpty()) {
        ...
        throw new OBException(OBMessageUtils.getI18NMessage("OBJSON_InvalidProperty", new String[] { value.toString(), sb.toString() }));
      }
  ...
  private String buildFieldClause(List<Property> properties, Property property, String fieldName, String operator) {
    ...
    if (properties.size() >= 2) {
      ...
      if (tableReference) {
        ...
        final boolean isTable = property.getEntity() == ModelProvider.getInstance().getEntity(Table.ENTITY_NAME);
        if (isTable) {
          ...
          final Reference reference = OBDal.getInstance().get(Reference.class, refProperty.getDomainType().getReference().getId());
          for (ReferencedTable referencedTable : reference.getADReferencedTableList()) {
            if (referencedTable.isActive() && referencedTable.getDisplayedColumn() != null && referencedTable.getDisplayedColumn().isActive()) {
              useProperty = property.getEntity().getPropertyByColumnName(referencedTable.getDisplayedColumn().getDBColumnName());
    ...
    if (useFieldName.equals(JsonConstants.IDENTIFIER) || useFieldName.endsWith(DalUtil.DOT + JsonConstants.IDENTIFIER)) {
      if (useFieldName.endsWith(DalUtil.DOT + JsonConstants.IDENTIFIER) && (operator.equals(OPERATOR_ISNULL) || operator.equals(OPERATOR_NOTNULL))) {
        clause = getMainAlias() + DalUtil.DOT + useFieldName.replace(DalUtil.DOT + JsonConstants.IDENTIFIER, "");
      } else {
        clause = computeLeftWhereClauseForIdentifier(useProperty, useFieldName, clause);
      }
    } else if (!useProperty.isPrimitive()) {
      clause = clause + ".id";
    } else if (tableReference && useProperty.isTranslatable() && OBContext.hasTranslationInstalled()) {
  ...
  private String buildValueClause(Property property, String operator, Object value) throws JSONException {

    Object localValue = value;
    boolean isFromRelativeDate = false;
    // if the value is a RelativeDate process it:
    if (value.toString().indexOf("_constructor") != -1) {
      boolean isjsonobject = false;
      JSONObject jsonCriteria = null;
      try {
        jsonCriteria = (JSONObject) value;
        isjsonobject = true;
      } catch (Exception e) {
        isjsonobject = false;
      }
      ...
      if (isjsonobject) {
        String constructor = jsonCriteria.getString("_constructor");
        if (constructor != null && constructor.compareTo("RelativeDate") == 0) {
          // it is a relative date, process it
          String rvalue = jsonCriteria.getString("value");
          String rrangePosition = jsonCriteria.getString("rangePosition");
          boolean logicalDate = false;
          Date baseDate = new Date();

          baseDate = RelativeDate.getAbsoluteDate(rvalue, baseDate, rrangePosition, logicalDate);
          if (baseDate != null) {
            localValue = baseDate;
            isFromRelativeDate = true;
          }
        }
      }
    }
    ...
    if (!property.isPrimitive()) {
      ...
      if (localValue.toString().contains(JsonConstants.IN_PARAMETER_SEPARATOR)) {
        ...
        final String[] separatedValues = localValue.toString().split(JsonConstants.IN_PARAMETER_SEPARATOR);
    ...
    try {
      localValue = getTypeSafeValue(operator, property, localValue, isFromRelativeDate);
    } catch (IllegalArgumentException e) {
      throw new OBException(OBMessageUtils.getI18NMessage("OBJSON_InvalidFilterValue", new String[] { value != null ? value.toString() : "" }));
    }
    typedParameters.add(localValue);
    return clause;
  }

  private Object getTypeSafeValue(String operator, Property property, Object value, boolean isFromRelativeDate) throws JSONException {
    if (value == null) {
      return value;
    }

    if (isLike(operator)) {
      if (operator.equals(OPERATOR_INOTCONTAINS) || operator.equals(OPERATOR_ICONTAINS) || operator.equals(OPERATOR_CONTAINSFIELD)) {
        ...
        return "%" + escapeLike(value.toString()).replaceAll(" ", "%") + "%";
      } else if (operator.equals(OPERATOR_INOTSTARTSWITH) || operator.equals(OPERATOR_ISTARTSWITH) || operator.equals(OPERATOR_STARTSWITHFIELD)) {
        return escapeLike(value.toString()).replaceAll(" ", "%") + "%";
    ...
    if (operator.equals(OPERATOR_INSET) || operator.equals(OPERATOR_NOTINSET)) {
      ...
      final JSONArray values = (JSONArray) value;
      for (int i = 0; i < values.length(); i++) {
        typedValues.add(getTypeSafeValue(OPERATOR_EQUALS, property, values.get(i), isFromRelativeDate));
      }
      return typedValues;
    }
    ...
    if (Boolean.class == property.getPrimitiveObjectType()) {
      ...
      try {
        ...
        if (isFromRelativeDate) {
          // java converted relativedate return the value since it will be a date
        ...
        if (!isFromRelativeDate && (hasComeADateTime || property.isDatetime() || property.isDate())) {
  ...
  private boolean isGreaterOperator(String operator) {
    return operator != null && (operator.equals(OPERATOR_GREATERTHAN) || operator.equals(OPERATOR_GREATEROREQUAL) || operator.equals(OPERATOR_IGREATERTHAN) || operator.equals(OPERATOR_IGREATEROREQUAL) || operator.equals(OPERATOR_GREATERTHANFIElD) || operator.equals(OPERATOR_GREATEROREQUALFIELD));
  }

  private boolean isLesserOperator(String operator) {
    return operator != null && (operator.equals(OPERATOR_LESSTHAN) || operator.equals(OPERATOR_LESSOREQUAL) || operator.equals(OPERATOR_ILESSTHAN) || operator.equals(OPERATOR_ILESSOREQUAL) || operator.equals(OPERATOR_LESSTHANFIELD) || operator.equals(OPERATOR_LESSOREQUALFIElD));
  }

  private String computeLeftWhereClauseForIdentifier(Property property, String key, String leftWherePart) {
    ...
    Check.isTrue(identifierProperties.contains(property), "Property " + property + " not part of identifier of " + property.getEntity());
    String prefix;
    ...
    if (key.equals(JsonConstants.IDENTIFIER)) {
      ...
      final String join = resolveJoins(JsonUtils.getPropertiesOnPath(getEntity(), propPath), propPath);
  ...
  private String parseStructuredClause(JSONArray clauses, String hqlOperator) throws JSONException {
    ...
    for (int i = 0; i < clauses.length(); i++) {
      ...
      if (clause.has(VALUE_KEY) && clause.get(VALUE_KEY) != null && clause.getString(VALUE_KEY).equals("")) {
        continue;
      }
  ...
  private boolean isNot(String operator) {
    return operator.equals(OPERATOR_NOTCONTAINS) || operator.equals(OPERATOR_NOTENDSWITH) || operator.equals(OPERATOR_NOTSTARTSWITH) || operator.equals(OPERATOR_INOTCONTAINS) || operator.equals(OPERATOR_INOTENDSWITH) || operator.equals(OPERATOR_INOTSTARTSWITH) || operator.equals(OPERATOR_NOT) || operator.equals(OPERATOR_NOTINSET);
  }
  ...
  private String substituteContextParameters(String currentWhereClause) {
    ...
    while (localWhereClause.contains("@")) {
      ...
      String paramValue = Utility.getContext(new DalConnectionProvider(false), RequestContext.get().getVariablesSecureApp(), param, RequestContext.get().getRequestParameter("windowId") != null ? RequestContext.get().getRequestParameter("windowId") : "");
  ...
  private String setRequestParameters(String currentWhereClause) {
    ...
    for (String key : filterParameters.keySet()) {
      if (!key.startsWith(DataEntityQueryService.PARAM_DELIMITER) || !key.endsWith(DataEntityQueryService.PARAM_DELIMITER)) {
        continue;
      }
      ...
      if (index != -1) {
        while (index != -1) {
          ...
          localWhereClause = localWhereClause.substring(0, index) + getTypedParameterAlias() + " " + localWhereClause.substring(index + key.length());
  ...
  public String getJoinClause() {
    ...
    for (String additionalProperty : additionalProperties) {
      final List<Property> properties = JsonUtils.getPropertiesOnPath(getEntity(), additionalProperty);
      if (properties.isEmpty()) {
        continue;
      }
  ...
  protected String getOrderByClause() {
    ...
    for (String localOrderBy : orderBy.split(",")) {
      ...
      sb.append(getOrderByClausePart(localOrderBy.trim().replace(DalUtil.FIELDSEPARATOR, DalUtil.DOT)));
      firstElement = false;
    }
  ...
  protected String getOrderByClausePart(String orderByParam) {
    ...
    boolean isIdentifier = localOrderBy.equals(JsonConstants.IDENTIFIER) || localOrderBy.endsWith(DalUtil.DOT + JsonConstants.IDENTIFIER);
    if (isIdentifier) {
      ...
      if (!localOrderBy.equals(JsonConstants.IDENTIFIER)) {
        ...
        Check.isNotNull(prop, "Property path " + localOrderBy + " is not valid for entity " + searchEntity);
    ...
    } else if (localOrderBy.equals(JsonConstants.SORTINGIDENTIFIER) || localOrderBy.endsWith(DalUtil.DOT + JsonConstants.SORTINGIDENTIFIER)) {
      Entity searchEntity = getEntity();
      // a path to an entity, find the last entity
      String prefix;
      if (!localOrderBy.equals(JsonConstants.SORTINGIDENTIFIER)) {
        // be lazy get the last property, it belongs to the last entity
        final Property prop = DalUtil.getPropertyFromPath(searchEntity, localOrderBy);
        Check.isNotNull(prop, "Property path " + localOrderBy + " is not valid for entity " + searchEntity);
        searchEntity = prop.getEntity();
      ...
      for (Property prop : searchEntity.getSortingIdentifierProperties()) {
    ...
    for (String path : paths) {
      ...
      final String resolvedPath = resolveJoins(JsonUtils.getPropertiesOnPath(getEntity(), path), path);
    ...
    for (Property prop : identifierProperties) {
      ...
      if (prop.getReferencedProperty() == null) {
        if (prop.isTranslatable() && OBContext.hasTranslationInstalled()) {
          ...
          sb.append("COALESCE(to_char((select " + prop.getTranslationProperty().getName() + " from " + prop.getTranslationProperty().getEntity().getName() + " as t where t." + prop.getTrlParentProperty().getName() + " = " + prefix.substring(0, prefix.lastIndexOf('.')) + " and t.language.language='" + OBContext.getOBContext().getLanguage().getLanguage() + "')), to_char(" + replaceValueWithJoins(prefix + prop.getName()) + "), '')");
        } else {
          sb.append("COALESCE(to_char(" + replaceValueWithJoins(prefix + prop.getName()) + "),'')");
        }

      } else {
        final List<Property> newIdentifierProperties = prop.getReferencedProperty().getEntity().getIdentifierProperties();
        sb.append(createIdentifierLeftClause(newIdentifierProperties, prefix + prop.getName() + DalUtil.DOT));
      }
    }
    ...
    if (value.contains("join")) {
      ...
      for (JoinDefinition joinDefinition : joinDefinitions) {
        if (joinDefinition.joinAlias.equals(joinValue)) {
          ...
          query = getMainAlias() + DalUtil.DOT + string + DalUtil.DOT + value.substring(value.indexOf(".") + 1, value.length());
          break;
        }
      }
    }
  ...
  private class JoinDefinition {
    ...
    public String getJoinStatement() {
      ...
      if (orNesting > 0) {
        return " left outer join " + (fetchJoin ? "fetch " : "") + (ownerAlias != null ? ownerAlias + DalUtil.DOT : "") + propName + " as " + joinAlias;
      } else {
        return " left join " + (fetchJoin ? "fetch " : "") + (ownerAlias != null ? ownerAlias + DalUtil.DOT : "") + propName + " as " + joinAlias;
      }
    }
  ...
  public void setOrderBy(String orderBy) {
    ...
    if (orderBy.indexOf(DalUtil.DOT) != -1 && orderBy.indexOf(DalUtil.DOT) != orderBy.lastIndexOf(DalUtil.DOT)) {
      setMainAlias(JsonConstants.MAIN_ALIAS);
    }
  }
  ...
  public void addSelectClausePart(String selectClausePart) {
    ...
    final List<Property> properties = JsonUtils.getPropertiesOnPath(getEntity(), localSelectClausePart);
...
class RelativeDate {
  static public String mapRelativeDateShortcut(String relativeDate, String rangePosition) {
    if (relativeDate == null)
      return null;

    if (relativeDate.compareTo("$now") == 0) {
      return "+0MS";
    } else if (relativeDate.compareTo("$today") == 0) {
      if (rangePosition.compareTo("end") == 0) {
        return "+0D";
      } else {
        return "-0D";
      }
    } else if (relativeDate.compareTo("$startOfToday") == 0) {
      return "-0D";
    } else if (relativeDate.compareTo("$endOfToday") == 0) {
      return "+0D";
    } else if (relativeDate.compareTo("$yesterday") == 0) {
      if (rangePosition.compareTo("end") == 0) {
        return "-1d[+0D]";
      } else {
        return "-1D";
      }
    } else if (relativeDate.compareTo("$startOfYesterday") == 0) {
      return "-1D";
    } else if (relativeDate.compareTo("$endOfYesterday") == 0) {
      return "-1d[+0D]";
    } else if (relativeDate.compareTo("$tomorrow") == 0) {
      if (rangePosition.compareTo("end") == 0) {
        return "+1D";
      } else {
        return "+1d[-0D]";
      }
    } else if (relativeDate.compareTo("$startOfTomorrow") == 0) {
      return "+1d[-0D]";
    } else if (relativeDate.compareTo("$endOfTomorrow") == 0) {
      return "+1D";
    } else if (relativeDate.compareTo("$startOfWeek") == 0) {
      return "-0W";
    } else if (relativeDate.compareTo("$endOfWeek") == 0) {
      return "+0W";
    } else if (relativeDate.compareTo("$startOfMonth") == 0) {
      return "-0M";
    } else if (relativeDate.compareTo("$endOfMonth") == 0) {
      return "+0M";
    } else if (relativeDate.compareTo("$startOfYear") == 0) {
      return "-0Y";
    } else if (relativeDate.compareTo("$endOfYear") == 0) {
      return "+0Y";
    } else if (relativeDate.compareTo("$weekFromNow") == 0) {
      if (rangePosition.compareTo("end") == 0) {
        return "+1w[+0D]";
      } else {
        return "+1w[-0D]";
      }
    } else if (relativeDate.compareTo("$weekAgo") == 0) {
      if (rangePosition.compareTo("end") == 0) {
        return "-1w[+0D]";
      } else {
        return "-1w[-0D]";
      }
    } else if (relativeDate.compareTo("$monthFromNow") == 0) {
      if (rangePosition.compareTo("end") == 0) {
        return "+1m[+0D]";
      } else {
        return "+1m[-0D]";
      }
    } else if (relativeDate.compareTo("$monthAgo") == 0) {
      if (rangePosition.compareTo("end") == 0) {
        return "-1m[+0D]";
      } else {
        return "-1m[-0D]";
  ...
  static public RelativeDatePart getRelativeDateParts(String relativeDateString) {
    RelativeDatePart rdpart = new RelativeDatePart();
    ...
    int countValue = 0;
    try {
      countValue = NumberFormat.getInstance().parse(withoutQualifier).intValue();
    } catch (Exception e) {
    }
    String period = withoutQualifier.replace(String.valueOf(countValue), "");

    rdpart.direction = (direction.compareTo("+") == 0 || direction.compareTo("-") == 0 ? direction : "+");
    rdpart.qualifier = qualifier != null && !qualifier.isEmpty() ? qualifier.replace("[", "").replace("]", "").replace(",", "") : null;
    rdpart.countValue = countValue;
    rdpart.period = period != null && !period.isEmpty() ? period : direction;

    return rdpart;
  }

  public static String getTimeUnitKey(String timeUnitName) {
    if (timeUnitName.compareTo("millisecond") == 0)
      return "ms";
    else if (timeUnitName.compareTo("second") == 0)
      return "s";
    else if (timeUnitName.compareTo("minute") == 0)
      return "mn";
    else if (timeUnitName.compareTo("hour") == 0)
      return "h";
    else if (timeUnitName.compareTo("day") == 0)
      return "d";
    else if (timeUnitName.compareTo("week") == 0)
      return "w";
    else if (timeUnitName.compareTo("month") == 0)
      return "m";
    else if (timeUnitName.compareTo("quarter") == 0)
      return "q";
    else if (timeUnitName.compareTo("year") == 0)
      return "y";
    else if (timeUnitName.compareTo("decade") == 0)
      return "dc";
    else if (timeUnitName.compareTo("century") == 0)
      return "c";

    return timeUnitName;
  }

  public static boolean isDatetimeOnlyPeriod(String period) {
    if (period.compareTo("s") == 0 || period.compareTo("S") == 0 || period.compareTo("mn") == 0 || period.compareTo("MN") == 0 || period.compareTo("h") == 0 || period.compareTo("H") == 0 || period.compareTo("d") == 0 || period.compareTo("D") == 0) {
      return true;
    }
    return false;
  }

  public static Calendar getStartOf(Calendar calendar, String period, boolean logicalDate) {

    if (logicalDate && isDatetimeOnlyPeriod(period)) {
    ...
    if (period.compareTo("s") == 0 || period.compareTo("S") == 0) {
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, year);
      result.set(Calendar.MONTH, month);
      result.set(Calendar.DATE, dateVal);
      ...
      return result;
    } else if (period.compareTo("mn") == 0 || period.compareTo("MN") == 0) {
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, year);
      result.set(Calendar.MONTH, month);
      result.set(Calendar.DATE, dateVal);
      ...
      return result;
    } else if (period.compareTo("h") == 0 || period.compareTo("H") == 0) {
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, year);
      result.set(Calendar.MONTH, month);
      result.set(Calendar.DATE, dateVal);
      ...
      return result;
    } else if (period.compareTo("d") == 0 || period.compareTo("D") == 0) {
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, year);
      result.set(Calendar.MONTH, month);
      result.set(Calendar.DATE, dateVal);
      ...
      return result;
    } else if (period.compareTo("w") == 0 || period.compareTo("W") == 0) {
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, year);
      result.set(Calendar.MONTH, month);
      result.set(Calendar.DATE, (dateVal - (dayOfWeek - Calendar.SUNDAY)));
      result.set(Calendar.HOUR_OF_DAY, 0);
      result.set(Calendar.MINUTE, 0);
      result.set(Calendar.SECOND, 0);
      result.set(Calendar.MILLISECOND, 0);
      ...
      return result;
    } else if (period.compareTo("m") == 0 || period.compareTo("M") == 0) {
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, year);
      result.set(Calendar.MONTH, month);
      ...
      return result;
    } else if (period.compareTo("q") == 0 || period.compareTo("Q") == 0) {
      int quarterStart = month - (month % 3);
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, year);
      result.set(Calendar.MONTH, quarterStart);
      result.set(Calendar.DATE, 1);
      result.set(Calendar.HOUR_OF_DAY, 0);
      result.set(Calendar.MINUTE, 0);
      result.set(Calendar.SECOND, 0);
      ...
      return result;
    } else if (period.compareTo("y") == 0 || period.compareTo("Y") == 0) {
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, year);
      ...
      return result;
    } else if (period.compareTo("dc") == 0 || period.compareTo("DC") == 0) {
      int decade = year - (year % 10);
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, decade);
      result.set(Calendar.MONTH, 0);
      result.set(Calendar.DATE, 1);
      result.set(Calendar.HOUR_OF_DAY, 0);
      result.set(Calendar.MINUTE, 0);
      ...
      return result;
    } else if (period.compareTo("c") == 0 || period.compareTo("C") == 0) {
      int century = year - (year % 100);
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, century);
      result.set(Calendar.MONTH, 0);
      result.set(Calendar.DATE, 1);
      result.set(Calendar.HOUR_OF_DAY, 0);
      result.set(Calendar.MINUTE, 0);
  ...
  public static Calendar getEndOf(Calendar calendar, String period, boolean logicalDate) {
    if (logicalDate && isDatetimeOnlyPeriod(period)) {
      return (Calendar) calendar.clone();
    }
    ...
    if (period.compareTo("s") == 0 || period.compareTo("S") == 0) {
      ...
      result.set(Calendar.YEAR, year);
      result.set(Calendar.MONTH, month);
      result.set(Calendar.DATE, dateVal + (6 - (dayOfWeek - Calendar.SUNDAY)));
      result.set(Calendar.HOUR_OF_DAY, 23);
      result.set(Calendar.MINUTE, 59);
      result.set(Calendar.SECOND, 59);
      result.set(Calendar.MILLISECOND, 999);
      ...
      Calendar result = Calendar.getInstance();
      result.set(Calendar.YEAR, year);
      result.set(Calendar.MONTH, nextQ);
      result.set(Calendar.DATE, 1);
      result.set(Calendar.HOUR_OF_DAY, 0);
      result.set(Calendar.MINUTE, 0);
      result.set(Calendar.SECOND, 0);
  ...
  public static Calendar dateAdd(Calendar _calendar, String unit, int amount, int multiplier, boolean isLogicalDate) {
    ...
    if (unit.length() > 2)
      unit = getTimeUnitKey(unit);

    if (unit.compareTo("MS") == 0 || unit.compareTo("ms") == 0) {
      calendar.add(Calendar.MILLISECOND, (amount * multiplier));
    } else if (unit.compareTo("S") == 0 || unit.compareTo("s") == 0) {
      if (unit.compareTo("S") == 0)
        boundary = true;
      calendar.add(Calendar.SECOND, (amount * multiplier));
    } else if (unit.compareTo("MN") == 0 || unit.compareTo("mn") == 0) {
      if (unit.compareTo("MN") == 0)
        boundary = true;
      calendar.add(Calendar.MINUTE, (amount * multiplier));
    } else if (unit.compareTo("H") == 0 || unit.compareTo("h") == 0) {
      if (unit.compareTo("H") == 0)
        boundary = true;
      calendar.add(Calendar.HOUR_OF_DAY, (amount * multiplier));
    } else if (unit.compareTo("D") == 0 || unit.compareTo("d") == 0) {
      if (unit.compareTo("D") == 0)
        boundary = true;
      calendar.add(Calendar.DATE, (amount * multiplier));
    } else if (unit.compareTo("W") == 0 || unit.compareTo("w") == 0) {
      if (unit.compareTo("W") == 0)
        boundary = true;
      calendar.add(Calendar.DATE, ((amount * 7) * multiplier));
    } else if (unit.compareTo("M") == 0 || unit.compareTo("m") == 0) {
      if (unit.compareTo("M") == 0)
        boundary = true;
      calendar.add(Calendar.MONTH, (amount * multiplier));
    } else if (unit.compareTo("Q") == 0 || unit.compareTo("q") == 0) {
      if (unit.compareTo("Q") == 0)
        boundary = true;
      calendar.add(Calendar.MONTH, ((amount * 3) * multiplier));
    } else if (unit.compareTo("Y") == 0 || unit.compareTo("y") == 0) {
      if (unit.compareTo("Y") == 0)
        boundary = true;
      calendar.add(Calendar.YEAR, (amount * multiplier));
    } else if (unit.compareTo("DC") == 0 || unit.compareTo("dc") == 0) {
      if (unit.compareTo("DC") == 0)
        boundary = true;
      calendar.add(Calendar.YEAR, ((amount * 10) * multiplier));
    } else if (unit.compareTo("C") == 0 || unit.compareTo("c") == 0) {
      if (unit.compareTo("C") == 0)
        boundary = true;
      calendar.add(Calendar.YEAR, ((amount * 100) * multiplier));
    }

    if (boundary) {
      if (multiplier > 0) {
        calendar = getEndOf(calendar, unit, isLogicalDate);
      } else {
        calendar = getStartOf(calendar, unit, isLogicalDate);
      }
    }
  ...
  static public Date getAbsoluteDate(String _relativeDate, Date baseDate, String rangePosition, boolean isLogicalDate) {
    String relativeDate = _relativeDate;
    Calendar localBaseDate = Calendar.getInstance();
    localBaseDate.setTime(baseDate);

    if (relativeDate.startsWith("$")) {
      relativeDate = mapRelativeDateShortcut(relativeDate, rangePosition);
    }

    RelativeDatePart parts = getRelativeDateParts(relativeDate);

    if (parts.qualifier != null && !parts.qualifier.isEmpty()) {
      parts.qualifier = parts.qualifier.toUpperCase();

      RelativeDatePart qParts = getRelativeDateParts(parts.qualifier);

      String[] options = new String[] { "S", "MN", "H", "D", "W", "M", "Q", "Y" };
      if (ArrayUtils.contains(options, qParts.period)) {
        localBaseDate = dateAdd(localBaseDate, qParts.period, qParts.countValue, (qParts.direction.compareTo("+") == 0 ? 1 : -1), isLogicalDate);
      }
    }

    // perform the date calculation
    Calendar absoluteDate = dateAdd(localBaseDate, parts.period, parts.countValue, (parts.direction.compareTo("+") == 0 ? 1 : -1), isLogicalDate);
