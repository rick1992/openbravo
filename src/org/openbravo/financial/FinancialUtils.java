...
public class FinancialUtils {
  ...
  public static BigDecimal getProductStdPrice(Product product, Date date, boolean useSalesPriceList, PriceList pricelist, Currency currency, Organization organization) throws OBException {
    ...
    if (!DalUtil.getId(pp.getPriceListVersion().getPriceList().getCurrency()).equals(currency.getId())) {
      ...
      price = getConvertedAmount(price, pp.getPriceListVersion().getPriceList().getCurrency(), currency, date, organization, PRECISION_PRICE);
    }
  ...
  public static ProductPrice getProductPrice(Product product, Date date, boolean useSalesPriceList, PriceList priceList, boolean throwException) throws OBException {
    ...
    where.append(" order by pl." + PriceList.PROPERTY_DEFAULT + " desc, plv." + PriceListVersion.PROPERTY_VALIDFROMDATE + " desc");

    OBQuery<ProductPrice> ppQry = OBDal.getInstance().createQuery(ProductPrice.class, where.toString());
    ...
    if (ppList.isEmpty()) {
      ...
      if (throwException) {
        throw new OBException("@PriceListVersionNotFound@. @Product@: " + product.getIdentifier() + " @Date@: " + OBDateUtils.formatDate(date));
      } else {
        return null;
      }
    }
   ...
   */
  public static ConversionRate getConversionRate(Date _conversionDate, Currency fromCurrency, Currency toCurrency, Organization org, Client client) {
    ...
    try {
      ...
      if ("0".equals(org.getId())) {
        ...
        return getConversionRate(conversionDate, fromCurrency, toCurrency, OBContext.getOBContext().getOrganizationStructureProvider(client.getId()).getParentOrg(org), client);
      }
    } catch (Exception e) {
  ...
  public static BigDecimal getConvertedAmount(BigDecimal amount, Currency curFrom, Currency curTo, Date date, Organization org, String strPrecision) throws OBException {
    ...
    if (cr == null) {
      throw new OBException("@NoCurrencyConversion@ " + curFrom.getISOCode() + " @to@ " + curTo.getISOCode() + " @ForDate@ " + OBDateUtils.formatDate(date) + " @And@ @ACCS_AD_ORG_ID_D@ " + org.getIdentifier());
    }
    ...
    return amount.multiply(cr.getMultipleRateBy()).setScale(precision.intValue(), RoundingMode.HALF_UP);
  }
  ...
  public static Currency getLegalEntityCurrency(Organization organization) {
    Organization legalEntity = OBContext.getOBContext().getOrganizationStructureProvider(organization.getClient().getId()).getLegalEntity(organization);
    Currency currency = (legalEntity.getCurrency() != null) ? legalEntity.getCurrency() : organization.getClient().getCurrency();
    return currency;
  }
   ...
   */
  public static BigDecimal calculateNetFromGross(String strTaxId, BigDecimal grossAmount, int pricePrecision, BigDecimal alternateAmount, BigDecimal quantity) {
    ...
    final BigDecimal lineNetAmount = (BigDecimal) CallStoredProcedure.getInstance().call(procedureName, parameters, null);
