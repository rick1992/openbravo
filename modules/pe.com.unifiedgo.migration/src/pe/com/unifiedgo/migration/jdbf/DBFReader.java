...
package pe.com.unifiedgo.migration.jdbf;
...
public class DBFReader {
  ...
  public Object[] nextRecord() throws JDBFException {
    ...
    for (int j = 0; j < aobj.length; j++) {
      ...
      if(fields[j].getType()=='T')//timestamp
      {
    	  long idate = ((0xFF & nextRecord[i+3]) << 24) | ((0xFF & nextRecord[i+2]) << 16) |
  	            ((0xFF & nextRecord[i+1]) << 8) | (0xFF & nextRecord[i]);
    	  if(idate==0) 
    		  stringbuffer.append(new String(nextRecord, i, k));
    	  else{
    		  ByteBuffer bbTime = ByteBuffer.wrap(nextRecord, i+4, 4);
    	
    	  long itime = ((0xFF & nextRecord[i+7]) << 24) | ((0xFF & nextRecord[i+6]) << 16) |
  	            ((0xFF & nextRecord[i+5]) << 8) | (0xFF & nextRecord[i+4]);
    	  
    	  int hours = (int)(itime/3600000L);  itime = itime - hours*3600000L;
    	  int minutes = (int)(itime/60000L);  itime = itime - minutes*60000L;
    	  int seconds = (int)(itime/1000L);
    	  
    	  Timestamp ts = new Timestamp(-4713-1900+1, 0, 1, 00, 00, 00, 00);
    	  Date dtt = new Date(-4713-1900+1, 0, 1, 0, 0, 0);
    	  
    	  Calendar c = Calendar.getInstance();
    	  c.setTime(dtt);
    	  c.add(Calendar.DATE, (int)idate);  // number of days to add
    	  c.set(Calendar.HOUR_OF_DAY, hours);
    	  c.set(Calendar.MINUTE, minutes);
    	  c.set(Calendar.SECOND, seconds);
    	  c.set(Calendar.MILLISECOND,0);
    	  
    	  dtt = c.getTime();  // dt is now the new date
    	  SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    	  stringbuffer.append(sdf.format(dtt)); 
    	  }
      }else if(fields[j].getType()=='I')//Long
      {
    	  long ilong = ((0xFF & nextRecord[i+3]) << 24) | ((0xFF & nextRecord[i+2]) << 16) |
    	            ((0xFF & nextRecord[i+1]) << 8) | (0xFF & nextRecord[i]);
    	  stringbuffer.append(String.valueOf(ilong));
