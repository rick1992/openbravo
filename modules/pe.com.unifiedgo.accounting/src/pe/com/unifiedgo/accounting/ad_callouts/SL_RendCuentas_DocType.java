...
public class SL_RendCuentas_DocType extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strSCORendCuentasId = vars.getStringParameter("inpscoRendicioncuentasId");
      String strDescription = vars.getStringParameter("inpdescription");
      String strTabId = vars.getStringParameter("inpTabId");

      try {
        printPage(response, vars, strDocTypeTarget, strDocType, docNo, strSCORendCuentasId, strDescription, strTabId);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strDocTypeTarget, String strDocType, String docNo, String strSCORendCuentasId, String strDescription, String strTabId) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    if (strDocTypeTarget.equals(""))
      ...
      resultado.append("var calloutName='SL_RendCuentas_DocType';\n\n");
      ...
      SCORendCuentasDocTypeData[] data = null;

      if (!newDocNo && !"0".equals(strDocType)) {
        data = SCORendCuentasDocTypeData.select(this, strDocType);
      ...
      SCORendCuentasDocTypeData[] dataNew = SCORendCuentasDocTypeData.select(this, strDocTypeTarget);
      if (dataNew != null && dataNew.length > 0) {
        DocSubTypeSO = dataNew[0].docsubtypeso;
        if (DocSubTypeSO == null)
          DocSubTypeSO = "--";
        String strOldDocTypeTarget = SCORendCuentasDocTypeData.selectOldDocSubType(this, strSCORendCuentasId);
        if (!DocSubTypeSO.equals("OB") && strOldDocTypeTarget.equals("OB")) {
          String strOldDocNo = SCORendCuentasDocTypeData.selectOldDocNo(this, strSCORendCuentasId);
        ...
        if (dataNew[0].isdocnocontrolled.equals("Y")) {
          if (!newDocNo && !AD_Sequence_ID.equals(dataNew[0].adSequenceId) && !SCORendCuentasDocTypeData.selectOldDocTypeTargetId(this, strSCORendCuentasId).equalsIgnoreCase(strDocTypeTarget))
            newDocNo = true;
          if (newDocNo) {
