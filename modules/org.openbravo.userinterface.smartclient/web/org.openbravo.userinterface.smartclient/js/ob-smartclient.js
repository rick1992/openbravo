...
isc.ListGrid.addProperties({
	...
	sortingswapped : false,
	
	swapSortingField : function(){
		var fields = this.getFields();
		for(i=0; i<fields.length; i++){
			var field = fields[i];
			if(field.sortingfield){
				var tmp = field.displayField;
				field.displayField = field.sortingfield;
				field.sortingfield = tmp;
			}
		}
		this.sortingswapped = !this.sortingswapped;
	},
	
	_original_toggleSort: isc.ListGrid.getPrototype().toggleSort,
	toggleSort : function (fieldName, direction) {
		
		var swapped = false;
		if(!this.sortingswapped){
			this.swapSortingField();
			swapped = true;
		}
		
		this._original_toggleSort(fieldName, direction);
		if(this.sortingswapped && swapped) this.swapSortingField();
	},
	
	_original_getSortSpecifier: isc.ListGrid.getPrototype().getSortSpecifier,
	getSortSpecifier : function (fieldName) {
		...
		var item = this._original_getSortSpecifier(fieldName);
		if(this.sortingswapped && swapped) this.swapSortingField();
		return item;
	},
	
	_original_isSortField: isc.ListGrid.getPrototype().isSortField,
	isSortField : function (fieldName) {
	    ...
	    if (!result) {
	        ...
	        if(!result && field && field.sortingfield && !field.optionDataSource){
	        	result = this.getSortSpecifier(field.sortingfield) ? true : false;
	        }
	    }
	    return result;
	},
	...
	_original_setSort: isc.ListGrid.getPrototype().setSort,
	setSort : function (sortSpecifiers) {
		...
		var returnvalue = this._original_setSort(sortSpecifiers);
		if(this.sortingswapped && swapped) this.swapSortingField();
		return returnvalue;
	}
});
...
isc.ComboBoxItem.addProperties({
  ...
  _original_handleKeyDown: isc.ComboBoxItem.getPrototype().handleKeyDown,
  handleKeyDown: function (event, eventInfo) {
    if(isc.EH.lastEvent.keyName == null){
    	isc.EH.lastEvent.keyName = event.keyName;
    }
