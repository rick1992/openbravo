...
OB.SSA.execBOEUnmarkAsReceived = {
  execute: function (params, view) {
    ...
    for (i = 0; i < selection.length; i++) {
     ...
     isc.SSA_BOEUnmarkAsReceivedPopup.create({
  ...
  execFunction: function (params, view) {
    params.actionHandler = 'pe.com.unifiedgo.sales.BOEUnmarkAsReceivedActionHandler';
    params.adTabId = view.activeView.tabId;
    OB.SSA.execBOEUnmarkAsReceived.execute(params, view);
  }

};

isc.defineClass('SSA_BOEUnmarkAsReceivedPopup', isc.OBPopup);

isc.SSA_BOEUnmarkAsReceivedPopup.addProperties({
  ...
  initWidget: function () {
	...
	this.confirmation = isc.Label.create({
		    ...
		    wrap: false,
		    showEdges: false,
		    contents: OB.I18N.getLabel('SSA_BOEUnmarkAsReceived_confirmation')
    });
