...
//    wrapTitle:null,
    ...
    // ------
        ...
        // the icon is disabled, and "Focused".
        // @visibility external
        //<
    ...
    // form item's cell will have the <code>"formCellFocused"</code> style applied to it.
    // @visibility external
    // @group appearance
    //<
...
isc.FormItem.addMethods({
    ...
    _getValueIconHTML : function (value) {
        ...
        var isRTL = this.containerWidget != null && this.containerWidget.isRTL(),
    ...
    _getCellStyle : function (baseStyle) {
        ...
        if (hasErrors && this.shouldShowErrorStyle() && this.form.showInlineErrors) {
            return this.showFocusedErrorState && this.hasFocus && !this.isInactiveHTML() ? 
                baseStyle + "ErrorFocused" : cacheObject.Error;
        } else {
    ...
    redrawn : function () {
        ...
        if (this._hasRedrawFocus(true)) {
            ...
            if (isc.Browser.isIE) {
                this.delayCall("_refocusAfterRedraw", [true], 100);
            } else {
    ...
    // focus handlers) now.
    _refocusAfterRedraw : function (delayed) {
    ...
    _getIconImgHTML : function (imgID, width, height, vAlign, vMargin, lMargin, backgroundColor,
        ...
        if (lMargin != null) {
            ...
            template[7] = (this.containerWidget.isRTL() ? "margin-right:" : "margin-left:") + lMargin + "px;";
        } else {
            template[7] = null;
        }
                ...
                if (isDate || isTime) {
                    if (this._formatAsTime()) {
                        if (isEmptyString && this.allowEmptyValue) {
                            ...
                            var timeVal = isc.Time.parseInput(value, false, false, !isTime, baseDate);
...
//JMD: handle null value in isc.getKeyForValue instead?
    ...
    getOptionDataSource : function () {
        ...
        if (ods == null) {
            var formDS = this.form ? this.form.dataSource : null;
    ...
    _checkForValueFieldValue : function (displayValue) {
        ...
        return this._checkForTargetFieldValue(displayValue, this.getDisplayFieldName(), this._fetchingMissingValueFieldValues);
    },
    ...
    _checkForTargetFieldValue : function (newValue, targetField, fetchingMissingValues) {
        ...
        if (isc.isAn.Array(newValue)) {
            for (var i = 0, len = newValue.length; i < len; ++i) {
                ...
                if (!fetchingMissingValues[val]) {
                    fetchingMissingValues[val] = needFetch = true;
                }
            }
        } else if (!fetchingMissingValues[newValue]) {
            fetchingMissingValues[newValue] = needFetch = true;
        }
        ...
        if (needFetch) {
            ...
            var context = isc.addProperties(
                ...
                {showPrompt:false, 
                 clientContext:{newValue:newValue,
    ...
    fetchMissingValueReply : function (response, data, request) {
        ...
        if (fetchedDisplayFieldValues) {
            this._fetchMissingDisplayFieldValueReply(response, data, request, notFoundCount);
        } else if (this._fetchingMissingValueFieldValues === fetchingMissingValues) {
            this._fetchMissingValueFieldValueReply(response, data, request, notFoundCount);
        }
    },

    _fetchMissingDisplayFieldValueReply : function (dsResponse, data, dsRequest, notFoundCount) {
        ...
        if (dsResponse.clientContext.filterLocally) this._gotAllOptions = true;
    ...
    _fetchMissingValueFieldValueReply : function (dsResponse, data, dsRequest, notFoundCount) {
        this._fetchMissingDisplayFieldValueReply(dsResponse, data, dsRequest, notFoundCount);

        // Call _updateValue() to fire change events in case this item's value changed as a result
        // of fetching a missing data value.
        this._updateValue(dsResponse.clientContext.newValue);
    },
   ...
   _updateValueIcon : function (value) {
    ...
    _showInFieldHint : function () {
        if (!this._showingInFieldHint && !this.hasFocus) {
    ...
    _getInFieldHintStyle : function() {
        if (this.showDisabled && this.isDisabled()) return this.textBoxStyle + "DisabledHint";
    ...
    hasAdvancedCriteria : function () {
        ...
        return value != null && (this.operator != null || this._shouldAllowExpressions());
    },
    ...
    setSelectionRange : function (start, end) {
        ...
        if (isc.Browser.isIE) {
            ...
            return isc.EH._settingTextSelection;
        }
    ...
    //<
	 getLeft : function () {
        ...
        var left = this._getElementLeft(tableElement);
        return left;
    ...
    getPageLeft : function () {
        ...
        var adjustForRTL = (cw.isRTL() && cw.overflow != isc.Canvas.VISIBLE);
        if (!adjustForRTL) {
            return this.getLeft() + 
            ...
            var maxScroll = cw.getScrollWidth()-cw.getViewportWidth();
            return this.getLeft() +
                (this.containerWidget.getPageLeft() 
                        + this.containerWidget.getLeftMargin() 
                        + this.containerWidget.getLeftBorderSize()
    ...
    // and the corresponding +link{OperatorId,operator} that will be used.  Note that in the
    ...
    // <P>
    // <P>
    // Note that, like +link{formItem.operator}, enabling <code>allowExpressions:true</code>
    // causes +link{dynamicForm.getValuesAsCriteria,form.getValuesAsCriteria()}) to return
            ...
            for (var key in opIndex) {
                        ...
                        || wildCard))
                {
                    ...
                    if (op.wildCard && isc.isA.String(valuePart) && valuePart.contains(op.wildCard)) {
                        ...
                        result.criteria.add({
                            operator: insensitive ? "iMatchesPattern" : "matchesPattern",
                            fieldName: fieldName,
