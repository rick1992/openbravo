...
public class DocPayment extends AcctServer {
  ...
  public String convertAmount(String Amount, boolean isReceipt, String DateAcct,
    ...
    if ((isReceipt && AmtDiff.compareTo(new BigDecimal("0.00")) == 1)
      ...
      FactLine fl = fact.createLine(line, getAccount(AcctServer.ACCTTYPE_ConvertGainDefaultAmt, as, conn),
          C_Currency_ID, "", AmtDiff.abs().toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo),
          DocumentType, conn);
      fl.descripcion = "GANANCIA POR DIFERENCIA DE CAMBIO";
    } else {
      FactLine fl = fact.createLine(line, getAccount(AcctServer.ACCTTYPE_ConvertChargeDefaultAmt, as, conn),
          C_Currency_ID, AmtDiff.abs().toString(), "", Fact_Acct_Group_ID, nextSeqNo(SeqNo),
          DocumentType, conn);
