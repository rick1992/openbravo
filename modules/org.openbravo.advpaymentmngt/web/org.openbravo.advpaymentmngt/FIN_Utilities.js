  ...
  if (actualConverted && expectedConverted && exchangeRate) {
    if (recalcExchangeRate) {
      if (actualConverted.value && actualPayment.value) {
        if (compare(exchangeRate.value, '!=', 0)) {
        //  exchangeRate.value = formattedNumberOpTemp(actualConverted.value, '/', actualPayment.value, exchangeRateRoundedMask, globalDecSeparator, globalGroupSeparator, globalGroupInterval);
        	actualPayment.value = formattedNumberOpTemp(actualConverted.value, '/', exchangeRate.value, roundedMask, globalDecSeparator, globalGroupSeparator, globalGroupInterval);
        }
...
function updateDifference() {
  ...
  displayLogicElement('overpayment', /*isCreditAllowed &&*/ isReceipt && compareWithSign(amount, '>', total));
  ...
  if (!(compare(expected, '!=', total) || (isCreditAllowed && compareWithSign(amount, '>', total)))) {
    ...
    selectDifferenceAction('overpayment');
...
function updateTotal() {
  ...
  if (!chk) {
	// SUG IMPL for payment in = payment out
  ...
  if (typeof callbackUpdateTotal == 'function') { callbackUpdateTotal(); }
}
...
function updateReadOnly(key, mark) {
  ...
  //for disablePaymentAmountEdit
  var disablePaymentAmountEdit = frm.elements['inpPaymentAmount'+key].getAttribute("disablepaymentamountedit");
  if(disablePaymentAmountEdit === null){
	  disablePaymentAmountEdit = 'N';
  }
  if(disablePaymentAmountEdit == 'Y'){
    console.log("disabling");
    frm.elements['inpPaymentAmount'+key].readOnly = true;
	frm.elements['inpPaymentAmount'+key].className = frm.elements['inpPaymentAmount'+key].className.replace(' readonly', '');
    frm.elements['inpPaymentAmount'+key].className += " readonly";
  }
...
function calculateRatePolarity(finaccCurrId, paymentCurrId) {
    if (finaccCurrId === "308") {
      return "D";
    } else if (paymentCurrId === "308") {
      return "E";
    } else if (finaccCurrId === "100") {
      return "D";
    } else if (paymentCurrId === "100") {
      return "E";
    } else if (finaccCurrId === "102") {
      return "D";
    } else if (paymentCurrId === "102") {
      return "E";
    } else {
      return "E";
    }
...
function calculateAltConvRate(finaccCurrId, paymentCurrId, convertRate) {
    var altconverRate = 1;
    var ratePolarity = calculateRatePolarity(finaccCurrId, paymentCurrId);
    if (ratePolarity === "E") {
      altconverRate = convertRate;
    } else {
      altconverRate = Math.round((1/convertRate) * 1000)/1000;
    }
    return altconverRate;
}

function calculateNormalConvRate(finaccCurrId, paymentCurrId, altConvertRate) {
    var converRate = 1;
    var ratePolarity = calculateRatePolarity(finaccCurrId, paymentCurrId);
    if (ratePolarity === "E") {
      converRate = altConvertRate;
    } else {
      converRate = Math.round((1/altConvertRate) * 1000000)/1000000;
    }
    return converRate;
}