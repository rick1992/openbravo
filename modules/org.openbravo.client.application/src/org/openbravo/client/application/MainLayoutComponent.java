...
public class MainLayoutComponent extends BaseTemplateComponent {
  ...
  public Collection<NBComponent> getNavigationBarComponents() {
    ...
    for (NavBarComponent nbc : obc.list()) {
      ...
      String jsCode = "";
      try {
        final Class<BaseTemplateComponent> clz = (Class<BaseTemplateComponent>) OBClassLoader.getInstance().loadClass(nbc.getJavaClassName());
  ...
  public String getStartPage() {
    try {
      return Preferences.getPreferenceValue(getContextUrl() + ApplicationConstants.START_PAGE_PROPERTY, true, OBContext.getOBContext().getCurrentClient(), OBContext.getOBContext().getCurrentOrganization(), OBContext.getOBContext().getUser(), OBContext.getOBContext().getRole(), null);
  ...
  public String getETag() {
    ...
    if (getModule().isInDevelopment() != null && getModule().isInDevelopment()) {
      ...
      return OBContext.getOBContext().getLanguage().getId() + "_" + OBContext.getOBContext().getRole().getId() + "_" + getModule().getVersion();
    }
  }
