...
public class Column extends ModelObject {
  ...
  public DomainType getDomainType() {
    if (referenceValue != null) {
      ...
      if (reference.getDomainType() instanceof ButtonDomainType && !(refValueDomainType instanceof EnumerateDomainType || refValueDomainType instanceof StringDomainType)) {
  ...
  protected void setReferenceType() {
    ...
    try {
      setReferenceType(((ForeignKeyDomainType) getDomainType()).getForeignKeyColumn(getColumnName()));
    } catch (final Exception e) {
      if (!OBPropertiesProvider.isFriendlyWarnings()) {
        log.error("No referenced column found: error >> tableName: " + table.getTableName() + " - columnName: " + getColumnName(), e);
      }
    }
  }
