...
public class SCO_Sperception_Receipt_DocType extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strSCOPercepSalesId = vars.getStringParameter("inpscoPercepSalesId");
      String strDescription = vars.getStringParameter("inpdescription");
      String strTabId = vars.getStringParameter("inpTabId");

      try {
        printPage(response, vars, strDocTypeTarget, strDocType, docNo, strSCOPercepSalesId, strDescription, strTabId);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strDocTypeTarget, String strDocType, String docNo, String strSCOPercepSalesId, String strDescription, String strTabId) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    if (strDocTypeTarget.equals(""))
      ...
      resultado.append("var calloutName='SCO_Sperception_Receipt_DocType';\n\n");
      ...
      SCOSperceptionReceiptDocTypeData[] data = null;

      if (!newDocNo && !"0".equals(strDocType)) {
        data = SCOSperceptionReceiptDocTypeData.select(this, strDocType);
      ...
      SCOSperceptionReceiptDocTypeData[] dataNew = SCOSperceptionReceiptDocTypeData.select(this, strDocTypeTarget);
      if (dataNew != null && dataNew.length > 0) {
        DocSubTypeSO = dataNew[0].docsubtypeso;
        if (DocSubTypeSO == null)
          DocSubTypeSO = "--";
        String strOldDocTypeTarget = SCOSperceptionReceiptDocTypeData.selectOldDocSubType(this, strSCOPercepSalesId);
        if (!DocSubTypeSO.equals("OB") && strOldDocTypeTarget.equals("OB")) {
          String strOldDocNo = SCOSperceptionReceiptDocTypeData.selectOldDocNo(this, strSCOPercepSalesId);
        ...
        if (dataNew[0].isdocnocontrolled.equals("Y")) {
          if (!newDocNo && !AD_Sequence_ID.equals(dataNew[0].adSequenceId) && !SCOSperceptionReceiptDocTypeData.selectOldDocTypeTargetId(this, strSCOPercepSalesId).equalsIgnoreCase(strDocTypeTarget))
            newDocNo = true;
          if (newDocNo) {
