...
public class DocFINReconciliation extends AcctServer {
  ...
  public FieldProviderFactory[] loadLinesPaymentDetailsFieldProvider(
    ...
    try {
      for (int i = 0; i < data.length; i++) {
        ...
        FieldProviderFactory.setField(data[i], "cGlItemId", transaction.getGLItem() != null
            ? transaction.getGLItem().getId() : data[i].getField("cGlItemId"));
            ...
            paymentDetails.get(i).getFINPaymentScheduleDetailList().get(0)
                ...
                && paymentDetails.get(i).getFINPaymentScheduleDetailList().get(0)
                    .getInvoicePaymentSchedule().getInvoice().getProject() != null
                        ? paymentDetails
                            .get(i).getFINPaymentScheduleDetailList().get(0)
                            .getInvoicePaymentSchedule()
                            .getInvoice().getProject().getId()
                        : (paymentDetails.get(i).getFINPaymentScheduleDetailList().get(0)
                            .getOrderPaymentSchedule() != null
                            && paymentDetails.get(i).getFINPaymentScheduleDetailList().get(0)
        ...
        FieldProviderFactory.setField(data[i], "cCostcenterId",
            paymentDetails.get(i).getFINPaymentScheduleDetailList().get(0).getCostCenter() != null
                ? paymentDetails.get(i).getFINPaymentScheduleDetailList().get(0).getCostCenter()
                    .getId()
                : "");
        FieldProviderFactory.setField(data[i], "lineno", transaction.getLineNo().toString());

        FieldProviderFactory.setField(data[i], "user1Id",
            paymentDetails.get(i).getFINPaymentScheduleDetailList().get(0).getStDimension() != null
                ? paymentDetails.get(i).getFINPaymentScheduleDetailList().get(0).getStDimension()
                    .getId()
                : "");

        FieldProviderFactory.setField(data[i], "user2Id",
            paymentDetails.get(i).getFINPaymentScheduleDetailList().get(0).getNdDimension() != null
                ? paymentDetails.get(i).getFINPaymentScheduleDetailList().get(0).getNdDimension()
                    .getId()
                : "");
  ...
  private DocLine[] loadLines() {
    ...
    try {
      for (int i = 0; i < data.length; i++) {
        ...
        docLine.m_C_Costcenter_ID = data[i].getField("cCostcenterId");
  ...
  public Fact createFactFee(DocLine_FINReconciliation line, AcctSchema as, ConnectionProvider conn,
    ...
    if (getDocumentTransactionConfirmation(transaction)) {
      ...
      transactionPaymentAmount = convertAmount(transactionPaymentAmount, false/* compra */,
          ...
          conn, vTcDummy);
      transactionDepositedAmount = convertAmount(transactionDepositedAmount, false/* compra */,
          dateFormat.format(transaction.getDateAcct()), TABLEID_Transaction, transaction.getId(),
          C_Currency_ID, as.m_C_Currency_ID, line, as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo),
          conn, vTcDummy);
      FactLine fl = fact.createLine(line, getWithdrawalAccount(as, transaction.getAccount(), conn),
    ...
    FactLine fl2 = fact.createLine(line, getClearOutAccount(as, transaction.getAccount(), conn),
    ...
    if (transaction.getAccount().getScoInternalDoc() != null)
      fl2.setRecordId3(transaction.getAccount().getScoInternalDoc().getId());
  ...
  public Fact createFactPayment(DocLine_FINReconciliation line, AcctSchema as,
    ...
    /*
     * List<SCOPaymentCostcenter> costcenters =
     * transaction.getFinPayment().getSCOPaymentCostcenterList();
     * 
    ...
    if (getDocumentTransactionConfirmation(transaction)) {
      ...
      Vector<BigDecimal> vTcDummy = new Vector<BigDecimal>();
      // Should map with the amount booked in the transaction and adjust with currency gain/loss
      transactionAmount = convertAmount(new BigDecimal(line.getAmount()), false /* compra */,
          dateFormat.format(transaction.getDateAcct()), TABLEID_Transaction, transaction.getId(),
          C_Currency_ID, as.m_C_Currency_ID, line, as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo),
          conn, vTcDummy);

      FactLine fl = fact.createLine(line, getAccountTransactionPayment(conn, payment, as),
          C_Currency_ID, !payment.isReceipt() ? transactionAmount.toString() : "",
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        detail.m_C_Currency_ID = data[i].getField("cCurrencyId");
        detail.setCurrency(data[i].getField("cCurrencyId"));
        ...
        detail.setWriteOffAmt(data[i].getField("WriteOffAmt"));
        final String finPaymentDetailID = data[i].getField("FIN_Payment_Detail_ID");
        detail.setInvoiceTaxCashVAT_V(finPaymentDetailID);

        String internalDoc = data[i].getField("internalDoc");
        if (internalDoc != null && !internalDoc.equals("")) {
          detail.m_Record_Id3 = internalDoc;
          detail.m_AD_Table3_Id = "A64BF5FB928C4EC1BACC023D6DC87F3C";
        }

        String invoiceRef = data[i].getField("invoiceRef");
        if (invoiceRef != null && !invoiceRef.equals("")) {
          detail.m_Record_Id3 = invoiceRef;
          detail.m_AD_Table3_Id = "318";
        }
      ...
      Vector<BigDecimal> vTcDummy = new Vector<BigDecimal>();
      paymentAmount = convertAmount(new BigDecimal(line.getAmount()), false/* compra */,
          dateFormat.format(transaction.getDateAcct()), TABLEID_Payment, line.getFinPaymentId(),
          C_Currency_ID, as.m_C_Currency_ID, line, as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo),
          conn, vTcDummy);
      FactLine fl = fact.createLine(line, getAccountPayment(conn, payment, as), C_Currency_ID,
    ...
    FactLine fl3 = fact.createLine(line, getAccountReconciliation(conn, payment, as), C_Currency_ID,
    ...
    if (transaction.getAccount().getScoInternalDoc() != null)
      fl3.setRecordId3(transaction.getAccount().getScoInternalDoc().getId());
      ...
      if (payment.getUsedCredit().compareTo(ZERO) != 0
        ...
        for (FIN_Payment_Credit creditPayment : creditPayments) {
          ...
          fact.createLine(line,
              ...
              DocumentType, line.m_DateAcct, vTcCreditOut.get(0), conn);

          // amtDiff = amtDiff.add(creditPayment.getAmount()).subtract(new
    ...
    if (getDocumentTransactionConfirmation(transaction))
      ...
      fact.createLine(line,
          getAccountBPartner((line.m_C_BPartner_ID == null || line.m_C_BPartner_ID.equals(""))
              ? this.C_BPartner_ID : line.m_C_BPartner_ID, as, isReceipt, isPrepayment, conn),
  ...
  public Fact createFactPaymentDetails(DocLine_FINReconciliation line,
    ...
    Invoice invoice = paymentDetail.getFINPaymentScheduleDetailList().get(0)
        .getInvoicePaymentSchedule() != null
            ...
            : null;
    if (invoice != null) {
      line.m_Record_Id3 = invoice.getId();
      line.m_AD_Table3_Id = "318";
    }

    line.m_Record_Id2 = paymentDetail.getId();
    ...
    BigDecimal bpAmount = new BigDecimal(line.getAmount());
    Currency paymentCurrency = paymentDetail.getFinPayment().getAccount().getCurrency();
    String bpartnerId = (line.m_C_BPartner_ID == null || line.m_C_BPartner_ID.equals(""))
        ? this.C_BPartner_ID : line.m_C_BPartner_ID;
    if (paymentDetail.getWriteoffAmount() != null
      ...
      // Vector<BigDecimal> vTcDummy = new Vector<BigDecimal>();
      BigDecimal writeOffAmt = paymentDetail.getWriteoffAmount();// , !isReceipt, line.m_DateAcct,
                                                                 // TABLEID_Payment, strcCurrencyId,
                                                                 // paymentCurrency.getId(),
                                                                 // as.m_C_Currency_ID, line, as,
                                                                 // fact, Fact_Acct_Group_ID,
    ...
    if (bpartnerId == null || bpartnerId.equals("")) {
      bpartnerId = paymentDetail.getFINPaymentScheduleDetailList().get(0)
          .getInvoicePaymentSchedule() != null
              ? paymentDetail.getFINPaymentScheduleDetailList().get(0).getInvoicePaymentSchedule()
                  .getInvoice().getBusinessPartner().getId()
    ...
    if (line.cGlItemId != null && !"".equals(line.cGlItemId)) {
      FactLine fl = fact.createLine(line,
          getAccountGLItem(OBDal.getInstance().get(GLItem.class, line.cGlItemId), as, isReceipt,
              conn),
          strcCurrencyId, (isReceipt ? "" : bpAmount.toString()),
      ...
      String docOrgInvoiceId = "";
      String docCurrInvoiceId = "";
      String docDoctypeInvoiceId = "";
      if (invoice != null) {
        Invoice invref = AcctServer.getInvoicerefForMapping(invoice);
        docOrgInvoiceId = invref.getOrganization().getId();
        docCurrInvoiceId = invref.getCurrency().getId();
        docDoctypeInvoiceId = invref.getTransactionDocument().getId();
      }
      FactLine fl = null;
      ...
      boolean tc = true;
      if (isReceipt)
        tc = AcctServer.getTCenVigencia(invoice, line.m_DateAcct);
      ...
      if (!isPrepayment && invoice != null) {
        ...
        bpAmountConverted = convertAmount(bpAmountConverted, !isReceipt, line.m_DateAcct,
            TABLEID_Invoice, invoice.getId(), strcCurrencyId, as.m_C_Currency_ID, line, as, fact,
            Fact_Acct_Group_ID, nextSeqNo(SeqNo), conn, true, tc, false, vTcOut);
        ...
        convertAmount(new BigDecimal(100), isReceipt, DateAcct, TABLEID_Invoice, invoice.getId(),
            strcCurrencyId, as.m_C_Currency_ID, line, as, fact, Fact_Acct_Group_ID,
            nextSeqNo(SeqNo), conn, false, isReceipt, isReceipt, vCashvatTc);
      ...
      if (isPrepayment) {
        ...
        bpAmountConverted = convertAmount(bpAmountConverted, !isReceipt, line.m_DateAcct,
            TABLEID_Payment, paymentDetail.getFinPayment().getId(), strcCurrencyId,
      ...
      if (line.getDoubtFulDebtAmount().signum() != 0) {
        ...
        BigDecimal doubtFulDebtAmount = convertAmount(line.getDoubtFulDebtAmount(), isReceipt,
            ...
            as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo), conn, false, tc, false, vDebtTcOut);
        ...
        if (fl != null) {
          fl.doc_Ad_Org_ID = docOrgInvoiceId;
          fl.doc_C_Currency_ID = docCurrInvoiceId;
          fl.doc_C_Doctype_ID = docDoctypeInvoiceId;
        }

        bpAmountConverted = bpAmountConverted.subtract(doubtFulDebtAmount);
        fl = fact.createLine(line, getAccountBPartnerAllowanceForDoubtfulDebt(bpartnerId, as, conn),
      ...
      fl = fact.createLine(line,
          (diverseAccount == null || isPrepayment) ? getAccountBPartner(bpartnerId, as, isReceipt,
              (isPrepayment || isPaymentDatePriorToInvoiceDate), conn) : diverseAccount,
          ...
          DocumentType, line.m_DateAcct, vTcOut.get(0), conn);
      if (fl != null) {
        fl.doc_Ad_Org_ID = docOrgInvoiceId;
        fl.doc_C_Currency_ID = docCurrInvoiceId;
        fl.doc_C_Doctype_ID = docDoctypeInvoiceId;
      ...
      if (isPaymentDatePriorToInvoiceDate) {
        ...
        if (!getAccountBPartner(bpartnerId, as, isReceipt, false, conn).Account_ID
          ...
          fl = fact.createLine(line2,
              (diverseAccount == null) ? getAccountBPartner(bpartnerId, as, isReceipt, false, conn)
                  : diverseAccount,
              strcCurrencyId, !isReceipt ? bpAmountConverted.toString() : "",
              isReceipt ? bpAmountConverted.toString() : "", Fact_Acct_Group_ID2, nextSeqNo(SeqNo),
              DocumentType, line2.m_DateAcct, vTcOut.get(0), conn);
          if (fl != null) {
            fl.doc_Ad_Org_ID = docOrgInvoiceId;
            fl.doc_C_Currency_ID = docCurrInvoiceId;
            fl.doc_C_Doctype_ID = docDoctypeInvoiceId;
          ...
          fl = fact.createLine(line2,
              (diverseAccount == null) ? getAccountBPartner(bpartnerId, as, isReceipt, true, conn)
                  : diverseAccount,
              strcCurrencyId, isReceipt ? bpAmountConverted.toString() : "",
              !isReceipt ? bpAmountConverted.toString() : "", Fact_Acct_Group_ID2, nextSeqNo(SeqNo),
              DocumentType, line2.m_DateAcct, vTcOut.get(0), conn);
  ...
  public Fact createFactGLItem(DocLine_FINReconciliation line, AcctSchema as,
    ...
    if (getDocumentTransactionConfirmation(transaction)) {
      ...
      Vector<BigDecimal> vTcDummy = new Vector<BigDecimal>();
      transactionPaymentAmount = convertAmount(transactionPaymentAmount, false/* compra */,
          DateAcct, TABLEID_Transaction, transaction.getId(), C_Currency_ID, as.m_C_Currency_ID,
          line, as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo), conn, vTcDummy);
      transactionDepositedAmount = convertAmount(transactionDepositedAmount, false/* compra */,
          DateAcct, TABLEID_Transaction, transaction.getId(), C_Currency_ID, as.m_C_Currency_ID,
          line, as, fact, Fact_Acct_Group_ID, nextSeqNo(SeqNo), conn, vTcDummy);
      FactLine fl2 = fact.createLine(line,
          getAccountTransaction(conn, transaction.getAccount(), as, isReceipt), C_Currency_ID,
    ...
    FactLine fl = fact.createLine(line, getAccount(conn, transaction.getAccount(), as, isReceipt),
        C_Currency_ID, line.getDepositAmount(), line.getPaymentAmount(), Fact_Acct_Group_ID,
        "999999", DocumentType, line.m_DateAcct, null, conn);
    if (transaction.getAccount().getScoInternalDoc() != null)
      fl.setRecordId3(transaction.getAccount().getScoInternalDoc().getId());
  ...
  public BigDecimal getBalance() {
    ...
    for (int i = 0; i < p_lines.length; i++) {
      ...
      lineBalance = lineBalance
          .subtract(new BigDecimal(((DocLine_FINReconciliation) p_lines[i]).PaymentAmount));
