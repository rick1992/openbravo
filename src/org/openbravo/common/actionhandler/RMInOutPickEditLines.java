...
public class RMInOutPickEditLines extends BaseProcessActionHandler {
  ...
  protected JSONObject doExecute(Map<String, Object> parameters, String content) {
    ...
    try {
      ...
      if (inOut != null) {
        List<String> idList = OBDao.getIDListFromOBObject(inOut.getMaterialMgmtShipmentInOutLineList());
  ...
  private void createInOutLines(JSONObject jsonRequest, List<String> idList) throws JSONException {
    ...
    Order orderref = null;
    boolean gotorderref = false;

    if (selectedLines.length() == 0) {
      inOut.setSalesOrder(orderref);
    ...
    String strShipment = "GR: ";
    String strInvoice = "FV: ";
    String strDescription = "";
    
    for (long i = 0; i < selectedLines.length(); i++) {
    	JSONObject selectedLine = selectedLines.getJSONObject((int) i);
        log.debug(selectedLine);
        OrderLine orderLine = OBDal.getInstance().get(OrderLine.class, selectedLine.getString("orderLine"));
       
        	
        strShipment = strShipment + " " + orderLine.getGoodsShipmentLine().
	                   getShipmentReceipt().
	                   getSCRPhysicalDocumentNo(); 
       
        strInvoice = strInvoice + " " + 
        ((orderLine.getSalesOrder().getSsaInvoiceref() != null) ? orderLine.getSalesOrder().getSsaInvoiceref().getScrPhysicalDocumentno():"");    
        strDescription = orderLine.getSalesOrder().getDescription();
       break;
    }
    ...
    for (long i = 0; i < selectedLines.length(); i++) {
      ...
      if (notExistsShipmentLine) {
        ...
        newInOutLine = OBDal.getInstance().get(ShipmentInOutLine.class, selectedLine.get("goodsShipmentLine"));
      ...
      newInOutLine.setStorageBin(OBDal.getInstance().get(Locator.class, selectedLine.getString("storageBin")));
      ...
      if (selectedLine.getString("conditionGoods") != null && !selectedLine.getString("conditionGoods").equals("null")) {
        newInOutLine.setConditionGoods(OBDal.getInstance().get(ConditionGoods.class, selectedLine.getString("conditionGoods")));
      } else {
      ...
      // Get the referenced order, only if all the lines have the same order
      if (selectedLine.getString("orderLine") != null) {
        OrderLine orderline = OBDal.getInstance().get(OrderLine.class, selectedLine.getString("orderLine"));

        if (gotorderref == false) {
          orderref = orderline.getSalesOrder();
          gotorderref = true;
        } else {
          if (orderref != null) {
            if (!orderref.getId().equals(orderline.getSalesOrder().getId())) {
              orderref = null;
            }
          }
        }
    ...
    inOut.setSalesOrder(orderref);
    inOut.setSwaInvoiceReference(strInvoice + " - "   + strShipment);
    inOut.setDescription(strDescription);
    OBDal.getInstance().save(inOut);
