...
public class SCO_Pperception_Receipt_DocType extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strSCOPercepPurchId = vars.getStringParameter("inpscoPercepPurchId");
      String strDescription = vars.getStringParameter("inpdescription");
      String strTabId = vars.getStringParameter("inpTabId");

      try {
        printPage(response, vars, strDocTypeTarget, strDocType, docNo, strSCOPercepPurchId, strDescription, strTabId);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strDocTypeTarget, String strDocType, String docNo, String strSCOPercepPurchId, String strDescription, String strTabId) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    if (strDocTypeTarget.equals(""))
      ...
      resultado.append("var calloutName='SCO_Pperception_Receipt_DocType';\n\n");
      ...
      SCOPperceptionReceiptDocTypeData[] data = null;

      if (!newDocNo && !"0".equals(strDocType)) {
        data = SCOPperceptionReceiptDocTypeData.select(this, strDocType);
      ...
      SCOPperceptionReceiptDocTypeData[] dataNew = SCOPperceptionReceiptDocTypeData.select(this, strDocTypeTarget);
      if (dataNew != null && dataNew.length > 0) {
        DocSubTypeSO = dataNew[0].docsubtypeso;
        if (DocSubTypeSO == null)
          DocSubTypeSO = "--";
        String strOldDocTypeTarget = SCOPperceptionReceiptDocTypeData.selectOldDocSubType(this, strSCOPercepPurchId);
        if (!DocSubTypeSO.equals("OB") && strOldDocTypeTarget.equals("OB")) {
          String strOldDocNo = SCOPperceptionReceiptDocTypeData.selectOldDocNo(this, strSCOPercepPurchId);
        ...
        if (dataNew[0].isdocnocontrolled.equals("Y")) {
          if (!newDocNo && !AD_Sequence_ID.equals(dataNew[0].adSequenceId) && !SCOPperceptionReceiptDocTypeData.selectOldDocTypeTargetId(this, strSCOPercepPurchId).equalsIgnoreCase(strDocTypeTarget))
            newDocNo = true;
          if (newDocNo) {
