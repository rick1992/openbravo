...
public class ModelProvider implements OBSingleton {
  ...
  private void initialize() {
    ...
    try {
      ...
      final Map<String, Boolean> colMandatories = getColumnMandatories(initsession, sessionFactoryController);
      ...
      for (final Entity e : model) {
        for (final Property p : e.getProperties()) {
          if (!p.isOneToMany()) {
            ...
            // tables
            if (!e.isView() && p.getColumnName() != null && !e.isDataSourceBased() && !e.isVirtualEntity()) {
              final Boolean mandatory = colMandatories.get(createColumnMandatoryKey(e.getTableName(), p.getColumnName()));
              if (mandatory != null) {
                ...
                log.warn("Column " + p + " mandatory setting not found in the database metadata. " + "A cause can be that the column does not exist in the database schema");
              }
            }
          }
        }
      ...
      // set fkHasspecialSorting for the entities UGO MODIFIED
      for (final Entity e : model) {
        e.setfkHasspecialSorting(false);
        List<Property> properties = e.getProperties();
        for (int i = 0; i < properties.size(); i++) {
          Property p_ = properties.get(i);
          if (p_.isIdentifier()) {
            if (p_.isScrIgnoreforsorting()) {
              e.setfkHasspecialSorting(true);
              break;
            }
          }
        }
  ...
  private void initializeReferenceClasses(ModelSessionFactoryController sessionFactoryController) {
    ...
    try {
      ...
      try {
        ps = connection.prepareStatement("select distinct model_impl from ad_reference where model_impl is not null");
  ...
  private <T extends ModelObject> long getLastUpdated(Class<T> clz, long currentLastTime, Session session) {
  ...
  private <T extends ModelObject> T queryLastUpdateObject(Session session, Class<T> clazz) {
    ...
    if (list.size() == 0) {
      throw new OBException("No instances of " + clazz.getName() + " in the database, has the database been created and filled with data?");
    }
    return (T) list.get(0);
  ...
  private List<Column> setReferenceProperties() {
    ...
    for (final Entity entity : model) {
      ...
      Entity baseEntity = entitiesByTableId.get(entity.getTableId().substring(0, entity.getTableId().indexOf("_CC")));
      if (baseEntity == null) {
      ...
      if (!c.isPrimitiveType()) {
        ...
        if (thatColumn == null) {
          if (!OBPropertiesProvider.isFriendlyWarnings()) {
            log.error("Property " + thisProp + " is mapped incorrectly, there is no referenced column for it, removing from the mapping");
          }
        ...
        if (thatColumn.getProperty() == null) {
          ...
          Check.isNotNull(entity, "No entity found using tablename " + thatColumn.getTable().getTableName() + " for column " + thatColumn);
  ...
  private Map<String, Boolean> getColumnMandatories(Session session, SessionFactoryController sfController) {
  ...
  private void buildUniqueConstraints(Session session, SessionFactoryController sessionFactoryController) {
    final List<UniqueConstraintColumn> uniqueConstraintColumns = getUniqueConstraintColumns(session, sessionFactoryController);
    Entity entity = null;
    UniqueConstraint uniqueConstraint = null;
    for (final UniqueConstraintColumn uniqueConstraintColumn : uniqueConstraintColumns) {
      // get the entity
      if (entity == null || !entity.getTableName().equalsIgnoreCase(uniqueConstraintColumn.getTableName())) {
      ...
      if (uniqueConstraint == null || !uniqueConstraint.getName().equalsIgnoreCase(uniqueConstraintColumn.getUniqueConstraintName())) {
  ...
  private List<UniqueConstraintColumn> getUniqueConstraintColumns(Session session, SessionFactoryController sessionFactoryController) {
    final List<UniqueConstraintColumn> result = new ArrayList<UniqueConstraintColumn>();
    final SQLQuery sqlQuery = session.createSQLQuery(sessionFactoryController.getUniqueConstraintQuery());
    for (final Object row : sqlQuery.list()) {
      ...
      Check.isTrue(values.length == 3, "Unexpected value length for constraint query, should be 3, but is " + values.length);
  ...
  private void createIdReferenceProperty(Entity e) {
    ...
    final String propName = idProperty.getSimpleTypeName().substring(0, 1).toLowerCase() + idProperty.getSimpleTypeName().substring(1);
  ...
  private void createPropertyInParentEntity(Entity e) {
    try {
      ...
      for (final Property p : props) {
        if (!p.isParent() && (p.isOneToMany() || p.isId() || p.getColumnName().equalsIgnoreCase("createdby") || p.getColumnName().equalsIgnoreCase("updatedby") || p.getReferencedProperty() == null || entitiesByClassName.get("org.openbravo.model.ad.system.Client").equals(p.getReferencedProperty().getEntity()) || entitiesByClassName.get("org.openbravo.model.common.enterprise.Organization").equals(p.getReferencedProperty().getEntity()) || entitiesByClassName.get("org.openbravo.model.ad.module.Module").equals(p.getReferencedProperty().getEntity()) || entitiesByClassName.get("org.openbravo.model.ad.system.Language").equals(p.getReferencedProperty().getEntity()))) {
          continue;
        }
  ...
  public Table getTable(String tableName) throws CheckException {
    ...
    if (table == null) {
      if (OBPropertiesProvider.isFriendlyWarnings()) {
        ...
        throw new IllegalArgumentException("Table: " + tableName + " not found in runtime model, is it maybe inactive?");
      } else {
   ...
   */
  protected Column getColumnByReference(String reference, String referenceValue, char validationType, String columnName) throws CheckException {
    Column c = null;
    ...
    if (reference.equals(Reference.TABLEDIR) || (reference.equals(Reference.SEARCH) && referenceValue.equals(Reference.NO_REFERENCE)) || reference.equals(Reference.IMAGE) || reference.equals(Reference.PRODUCT_ATTRIBUTE) || reference.equals(Reference.RESOURCE_ASSIGNMENT)) {
      ...
      try {
        ...
        e.printStackTrace();
        Check.fail("Reference column for " + columnName + " not found in runtime model [ref: " + reference + ", refval: " + referenceValue + "], encountered exception " + e.getMessage());
      }
