...
public class FIN_Utility {
   ...
   */
  public static FieldProvider[] getShownScheduledPayments(VariablesSecureApp vars, List<FIN_PaymentSchedule> selectedScheduledPayments, List<FIN_PaymentSchedule> filteredScheduledPayments) {
    ...
    try {
      ...
      for (int i = 0; i < data.length; i++) {
        FieldProviderFactory.setField(data[i], "finSelectedPaymentId", (selectedScheduledPayments.contains(FIN_PaymentSchedules[i])) ? FIN_PaymentSchedules[i].getId() : "");
        ...
        if (FIN_PaymentSchedules[i].getOrder() != null)
          FieldProviderFactory.setField(data[i], "orderNr", FIN_PaymentSchedules[i].getOrder().getDocumentNo());
        if (FIN_PaymentSchedules[i].getInvoice() != null) {
          FieldProviderFactory.setField(data[i], "invoiceNr", FIN_PaymentSchedules[i].getInvoice().getDocumentNo());
          FieldProviderFactory.setField(data[i], "invoicedAmount", FIN_PaymentSchedules[i].getInvoice().getGrandTotalAmount().toString());
        }
        FieldProviderFactory.setField(data[i], "dueDate", dateFormater.format(FIN_PaymentSchedules[i].getDueDate()).toString());
        FieldProviderFactory.setField(data[i], "expectedAmount", FIN_PaymentSchedules[i].getAmount().toString());
  ...
  public static String getExceptionMessage(Throwable t) {
    if (t.getCause() instanceof BatchUpdateException && ((BatchUpdateException) t.getCause()).getNextException() != null) {
  ...
  public static DocumentType getDocumentType(Organization org, String docCategory) {
    ...
    obcDoc.add(Restrictions.in("organization.id", OBContext.getOBContext().getOrganizationStructureProvider(org.getClient().getId()).getParentTree(org.getId(), true)));
  ...
  public static String getDocumentNo(DocumentType docType, String tableName, boolean updateNext) {
    if (docType != null) {
      ...
      if (seq == null && tableName != null) {
        ...
        obcSeq.add(Restrictions.eq(Sequence.PROPERTY_NAME, "DocumentNo_" + tableName));
   ...
   */
  @Deprecated
  public static String getPaymentMethodList(String strPaymentMethodId, String strFinancialAccountId, String strOrgId, boolean isMandatory, boolean excludePaymentMethodWithoutAccount) {
    dao = new AdvPaymentMngtDao();
    List<FIN_PaymentMethod> paymentMethods = dao.getFilteredPaymentMethods(strFinancialAccountId, strOrgId, excludePaymentMethodWithoutAccount, AdvPaymentMngtDao.PaymentDirection.EITHER);
   ...
   * @return a String with the html code with the options to fill the drop-down of Payment Methods.
   */
  public static String getPaymentMethodList(String strPaymentMethodId, String strFinancialAccountId, String strOrgId, boolean isMandatory, boolean excludePaymentMethodWithoutAccount, boolean isInPayment) {
    dao = new AdvPaymentMngtDao();
    String selectedPaymentMethodId = strPaymentMethodId;
    List<FIN_PaymentMethod> paymentMethods = dao.getFilteredPaymentMethods(strFinancialAccountId, strOrgId, excludePaymentMethodWithoutAccount, isInPayment ? AdvPaymentMngtDao.PaymentDirection.IN : AdvPaymentMngtDao.PaymentDirection.OUT);
    if ("".equals(selectedPaymentMethodId) && !"".equals(strFinancialAccountId)) {
      selectedPaymentMethodId = dao.getDefaultPaymentMethodId(OBDal.getInstance().get(FIN_FinancialAccount.class, strFinancialAccountId), isInPayment);
    }
   ...
   */
  @Deprecated
  public static String getFinancialAccountList(String strPaymentMethodId, String strFinancialAccountId, String strOrgId, boolean isMandatory, String strCurrencyId) {
    List<FIN_FinancialAccount> financialAccounts = dao.getFilteredFinancialAccounts(strPaymentMethodId, strOrgId, strCurrencyId, AdvPaymentMngtDao.PaymentDirection.EITHER);
   ...
   *         Accounts.
   */
  public static String getFinancialAccountList(String strPaymentMethodId, String strFinancialAccountId, String strOrgId, boolean isMandatory, String strCurrencyId, boolean isInPayment) {

    List<FIN_FinancialAccount> financialAccounts = dao.getFilteredFinancialAccounts(strPaymentMethodId, strOrgId, strCurrencyId, isInPayment ? AdvPaymentMngtDao.PaymentDirection.IN : AdvPaymentMngtDao.PaymentDirection.OUT);
    String options = getOptionsList(financialAccounts, strFinancialAccountId, isMandatory);
    return options;
  }
   ...
   */
  public static <T extends BaseOBObject> String getOptionsList(List<T> obObjectList, String selectedValue, boolean isMandatory) {
   ...
   * @return a String containing the html code with the options. *
   */
  public static <T extends BaseOBObject> String getOptionsList(List<T> obObjectList, String selectedValue, boolean isMandatory, boolean isRefList) {
    StringBuilder strOptions = new StringBuilder();
    if (!isMandatory)
      strOptions.append("<option value=\"\"></option>");

    for (T obObject : obObjectList) {
      strOptions.append("<option value=\"").append((isRefList) ? obObject.getValue("searchKey") : obObject.getId()).append("\"");
  ...
  public static <T extends BaseOBObject> String getOptionsListFromFieldProvider(FieldProvider[] fieldProvider, String selectedValue, boolean isMandatory) {
    StringBuilder strOptions = new StringBuilder();
    if (!isMandatory)
      strOptions.append("<option value=\"\"></option>");
  ...
  public static boolean isAutomaticDepositWithdrawn(FIN_FinancialAccount account, FIN_PaymentMethod paymentMethod, boolean isReceipt) {
    FinAccPaymentMethod financialAccountPaymentMethod = new AdvPaymentMngtDao().getFinancialAccountPaymentMethod(account, paymentMethod);
    if (financialAccountPaymentMethod == null)
      return false;
    return isReceipt ? financialAccountPaymentMethod.isAutomaticDeposit() : financialAccountPaymentMethod.isAutomaticWithdrawn();
  }

  public static boolean isAutomaticRevertDepositWithdrawn(FIN_FinancialAccount account, FIN_PaymentMethod paymentMethod, boolean isReceipt) {
    FinAccPaymentMethod financialAccountPaymentMethod = new AdvPaymentMngtDao().getFinancialAccountPaymentMethod(account, paymentMethod);
    if (financialAccountPaymentMethod == null)
      return false;
    return isReceipt ? financialAccountPaymentMethod.isScoAutoRevertDeposit() : financialAccountPaymentMethod.isScoAutoRevertWithdrawn();
  }
  ...
  public static boolean isAutomaticDepositWithdrawn(FIN_PaymentProposal paymentProposal) {
    return isAutomaticDepositWithdrawn(paymentProposal.getAccount(), paymentProposal.getPaymentMethod(), paymentProposal.isReceipt());
  }

  public static boolean isAutomaticRevertDepositWithdrawn(FIN_Payment payment) {
    return isAutomaticRevertDepositWithdrawn(payment.getAccount(), payment.getPaymentMethod(), payment.isReceipt());
  }

  public static boolean isAutomaticRevertDepositWithdrawn(FIN_PaymentProposal paymentProposal) {
    return isAutomaticRevertDepositWithdrawn(paymentProposal.getAccount(), paymentProposal.getPaymentMethod(), paymentProposal.isReceipt());
  }
   ...
   */
  public static <T extends BaseOBObject> List<T> getAllInstances(Class<T> clazz, boolean setFilterClient, boolean setFilterOrg, Value... values) {
   ...
   */
  public static String multiCurrencyAmountToDisplay(BigDecimal amt, Currency currency, BigDecimal convertedAmt, Currency convertedCurrency) {
    StringBuffer out = new StringBuffer();
    final UIDefinitionController.FormatDefinition formatDef = UIDefinitionController.getInstance().getFormatDefinition("euro", "Edition");
     ...
     * amountFormatter.setMaximumFractionDigits(convertedCurrency
     * .getStandardPrecision().intValue());
     * out.append(" (").append(amountFormatter.format(convertedAmt
     * )).append(" ").append(convertedCurrency.getISOCode()).append(")"); }
     */
   ...
   */
  // UGO MODIFIED CONVERSIONRATE
  public static ConversionRate getConversionRate(Currency fromCurrency, Currency toCurrency, Date _conversionDate, Organization org) {
   ...
   * @return A valid conversion rate for the parameters, or null if no conversion rate can be found
   */
  public static ConversionRateDoc getConversionRateDoc(Currency fromCurrency, Currency toCurrency, String documentId, Entity entity) {

    final OBCriteria<ConversionRateDoc> obcConvRateDoc = OBDal.getInstance().createCriteria(ConversionRateDoc.class);

    if (entity.equals(ModelProvider.getInstance().getEntity("Invoice"))) {
      obcConvRateDoc.add(Restrictions.eq(ConversionRateDoc.PROPERTY_INVOICE, OBDal.getInstance().get(Invoice.class, documentId)));
    } else if (entity.equals(ModelProvider.getInstance().getEntity("FIN_Payment"))) {
      obcConvRateDoc.add(Restrictions.eq(ConversionRateDoc.PROPERTY_PAYMENT, OBDal.getInstance().get(FIN_Payment.class, documentId)));
    } else if (entity.equals(ModelProvider.getInstance().getEntity("FIN_Finacc_Transaction"))) {
      obcConvRateDoc.add(Restrictions.eq(ConversionRateDoc.PROPERTY_FINANCIALACCOUNTTRANSACTION, OBDal.getInstance().get(FIN_FinaccTransaction.class, documentId)));
    }
  ...
  public static int getConversionRatePrecision(VariablesSecureApp vars) {
    try {
      ...
      if (formatOutput.contains(decimalSeparator)) {
        formatOutput = formatOutput.substring(formatOutput.indexOf(decimalSeparator), formatOutput.length());
   ...
   */
  public static String formatNumber(BigDecimal number, String javaFormat, String _decimalSeparator, String _groupingSeparator) {
  ...
  public static String getDesiredDocumentNo(final Organization organization, final Invoice invoice) {
    ...
    try {
      ...
      final String paymentDescription = OBDal.getInstance().get(OrganizationInformation.class, ((String) DalUtil.getId(organization))).getAPRMPaymentDescription();
  ...
  private static List<String> getListPaymentConfirmedOrNot(Boolean isConfirmed) {
    ...
    OBContext.setAdminMode(true);
    try {
      final OBCriteria<org.openbravo.model.ad.domain.List> obCriteria = OBDal.getInstance().createCriteria(org.openbravo.model.ad.domain.List.class);
      obCriteria.add(Restrictions.eq(org.openbravo.model.ad.domain.List.PROPERTY_REFERENCE + ".id", "575BCB88A4694C27BC013DE9C73E6FE7"));
  ...
  public static Organization getLegalEntityOrg(final Organization org) {
    try {
      OBContext.setAdminMode(true);
      final OrganizationStructureProvider osp = OBContext.getOBContext().getOrganizationStructureProvider(org.getClient().getId());
  ...
  public static boolean periodControlOpened(String tableName, String recordId, String idColumnName, String orgType) {
    final Session session = OBDal.getInstance().getSession();
  ...
  public static boolean isBlockedBusinessPartner(String strBPartnerId, boolean issotrx, int docType) {
    try {
      ...
      case 1: {
        // Order
        return ((issotrx && bPartner.isCustomerBlocking() && bPartner.isSalesOrder()) || (!issotrx && bPartner.isVendorBlocking() && bPartner.isPurchaseOrder()));

      }
      case 2: {
        // Goods Shipment / Receipt
        return ((issotrx && bPartner.isCustomerBlocking() && bPartner.isGoodsShipment()) || (!issotrx && bPartner.isVendorBlocking() && bPartner.isGoodsReceipt()));

      }
      case 3: {
        // Invoice
        return ((issotrx && bPartner.isCustomerBlocking() && bPartner.isSalesInvoice()) || (!issotrx && bPartner.isVendorBlocking() && bPartner.isPurchaseInvoice()));
      }
      case 4: {
        // Payment
        return ((issotrx && bPartner.isCustomerBlocking() && bPartner.isPaymentIn()) || (!issotrx && bPartner.isVendorBlocking() && bPartner.isPaymentOut()));
  ...
  public static List<FIN_PaymentDetail> getOrderedPaymentDetailList(FIN_Payment payment) {
    ...
    try {
      ...
      whereClause.append(" left join pd." + FIN_PaymentDetail.PROPERTY_FINPAYMENTSCHEDULEDETAILLIST + " as psd");
      whereClause.append(" where pd." + FIN_PaymentDetail.PROPERTY_FINPAYMENT + ".id = '" + payment.getId() + "'");
      whereClause.append(" order by psd." + FIN_PaymentScheduleDetail.PROPERTY_INVOICEPAYMENTSCHEDULE);
      whereClause.append(", coalesce(psd." + FIN_PaymentScheduleDetail.PROPERTY_ORDERPAYMENTSCHEDULE + ",'0')");

      OBQuery<FIN_PaymentDetail> query = OBDal.getInstance().createQuery(FIN_PaymentDetail.class, whereClause.toString());
   ...
   * account for a payment
   */
  public static String invoicePaymentStatus(FIN_PaymentMethod paymentMethod, FIN_FinancialAccount financialAccount, boolean isReceipt) {
    String status = null;
   ...
   */
  public static void updatePaymentAmounts(FIN_PaymentDetail paymentDetail, FIN_PaymentScheduleDetail psd) {
    ...
    if (psd.getInvoicePaymentSchedule() != null) {
      ...
      BigDecimal amountWithSign = psd.getInvoicePaymentSchedule().getInvoice().isSalesTransaction() ? psd.getAmount() : psd.getAmount().negate();
      creditUsed = creditUsed.subtract(amountWithSign);
      bPartner.setCreditUsed(creditUsed);
      OBDal.getInstance().save(bPartner);
      FIN_AddPayment.updatePaymentScheduleAmounts(paymentDetail,psd.getInvoicePaymentSchedule(), psd.getAmount(), psd.getWriteoffAmount(), "--");
    }
    if (psd.getOrderPaymentSchedule() != null) {
      FIN_AddPayment.updatePaymentScheduleAmounts(psd.getOrderPaymentSchedule(), psd.getAmount(), psd.getWriteoffAmount(), "--");
    }
    if (psd.getPaymentDetails().isPrepayment() && psd.getOrderPaymentSchedule() == null && psd.getInvoicePaymentSchedule() == null) {
      // This PSD is credit
      BusinessPartner bPartner = psd.getPaymentDetails().getFinPayment().getBusinessPartner();
      BigDecimal creditUsed = bPartner.getCreditUsed();
      BigDecimal amountWithSign = psd.getPaymentDetails().getFinPayment().isReceipt() ? psd.getAmount() : psd.getAmount().negate();
      creditUsed = creditUsed.subtract(amountWithSign);
      bPartner.setCreditUsed(creditUsed);
      OBDal.getInstance().save(bPartner);
    }
  ...
  public static void updateBusinessPartnerCredit(FIN_Payment payment) {
    ...
    if (payment.getGeneratedCredit().compareTo(BigDecimal.ZERO) == 0 && payment.getUsedCredit().compareTo(BigDecimal.ZERO) != 0) {
  ...
  public static int seqnumberpaymentstatus(String status) {
    List<Object> parameters = new ArrayList<Object>();
    parameters.add(status);
    int result = Integer.parseInt((String) CallStoredProcedure.getInstance().call("aprm_seqnumberpaymentstatus", parameters, null));
   ...
   * This function should only be called when it should update the payment amounts
   */
  public static void restorePaidAmounts(FIN_PaymentDetail paymentDetail, FIN_PaymentScheduleDetail paymentScheduleDetail) {
    ...
    BusinessPartner businessPartner = paymentScheduleDetail.getPaymentDetails().getFinPayment().getBusinessPartner();
    if (paymentScheduleDetail.getInvoicePaymentSchedule() != null) {

      FIN_AddPayment.updatePaymentScheduleAmounts(paymentDetail, paymentScheduleDetail.getInvoicePaymentSchedule(), psdAmount.negate(), psdWriteoffAmount.negate(), "--");
      // BP SO_CreditUsed
    ...
    if (paymentScheduleDetail.getOrderPaymentSchedule() != null) {
      FIN_AddPayment.updatePaymentScheduleAmounts(paymentScheduleDetail.getOrderPaymentSchedule(), psdAmount.negate(), psdWriteoffAmount.negate(), "--");
    }
    // when generating credit for a BP SO_CreditUsed is also updated
    if (paymentScheduleDetail.getInvoicePaymentSchedule() == null && paymentScheduleDetail.getOrderPaymentSchedule() == null && paymentScheduleDetail.getPaymentDetails().getGLItem() == null && !paymentScheduleDetail.getPaymentDetails().isRefund()) {
      // BP SO_CreditUsed
      if (paymentScheduleDetail.getPaymentDetails().getFinPayment().isReceipt()) {
        increaseCustomerCredit(businessPartner, amount);
      } else {
