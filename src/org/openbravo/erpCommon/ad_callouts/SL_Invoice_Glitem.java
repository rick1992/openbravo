...
public class SL_Invoice_Glitem extends HttpSecureAppServlet {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      try {
        printPage(response, vars, strChanged, strAccountID, strADOrgID, strCInvoiceID, strIsSOTrx,
            strWharehouse, strCCostcenterID);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strChanged,
      ...
      String strWharehouse, String strCCostcenterID) throws IOException, ServletException {
    ...
    if (strChanged.equalsIgnoreCase("inpaccountId")) {
      ...
      if (inv != null) {
        if (inv.getDocumentStatus().compareTo("CO") != 0) {
        ...
        if (inv != null && !inv.isSalesTransaction() && inv.getProject() != null) {
          if (strCCostcenterID == null || "".compareTo(strCCostcenterID) == 0) {
            result.append("new Array(\"inpcCostcenterId\", \""
                + inv.getProject().getSprCostcenter().getId() + "\"),");
          }
        }
      }
