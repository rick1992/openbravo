...
public class PaymentReportDao {
  ...
  public FieldProvider[] getPaymentReport(VariablesSecureApp vars, String strOrg, String strInclSubOrg, String strDueDateFrom, String strDueDateTo, String strAmountFrom, String strAmountTo, String strDocumentDateFrom, String strDocumentDateTo, String strcBPartnerIdIN, String strcBPGroupIdIN, String strcProjectIdIN, String strfinPaymSt, String strPaymentMethodId, String strFinancialAccountId, String strcCurrency, String strConvertCurrency, String strConversionDate, String strPaymType, String strOverdue, String strGroupCrit, String strOrdCrit) {

    try {
      return getPaymentReport(vars, strOrg, strInclSubOrg, strDueDateFrom, strDueDateTo, strAmountFrom, strAmountTo, strDocumentDateFrom, strDocumentDateTo, strcBPartnerIdIN, strcBPGroupIdIN, "include", strcProjectIdIN, strfinPaymSt, strPaymentMethodId, strFinancialAccountId, strcCurrency, strConvertCurrency, strConversionDate, strPaymType, strOverdue, strGroupCrit, strOrdCrit, "Y", "", "");
    } catch (OBException e) {
  ...
  public FieldProvider[] getPaymentReport(VariablesSecureApp vars, String strOrg, String strInclSubOrg, String strDueDateFrom, String strDueDateTo, String strAmountFrom, String strAmountTo, String strDocumentDateFrom, String strDocumentDateTo, String strcBPartnerIdIN, String strcBPGroupIdIN, String strcNoBusinessPartner, String strcProjectIdIN, String strfinPaymSt, String strPaymentMethodId, String strFinancialAccountId, String strcCurrency, String strConvertCurrency, String strConversionDate, String strPaymType, String strOverdue, String strGroupCrit, String strOrdCrit, String strInclPaymentUsingCredit, String strPaymentDateFrom, String strPaymentDateTo, String strExpectedDateFrom, String strExpectedDateTo) throws OBException {
    ...
    try {
      ...
      hsqlScript.append("select fpsd.id, (select a.sequenceNumber from ADList a where a.reference.id = '575BCB88A4694C27BC013DE9C73E6FE7' and a.searchKey = coalesce(pay.status, 'RPAP')) as a,");
      hsqlScript.append(" (select trans.id from FIN_Finacc_Transaction trans left outer join trans.finPayment payment where payment.id=pay.id) as trans ");
      ...
      if (!strOrg.isEmpty()) {
        if (!strInclSubOrg.equalsIgnoreCase("include")) {
          ...
          hsqlScript.append(FIN_PaymentScheduleDetail.PROPERTY_ORGANIZATION);
          hsqlScript.append(".id in ('");
          Set<String> orgChildTree = OBContext.getOBContext().getOrganizationStructureProvider().getChildTree(strOrg, true);
      ...
      if (!strFinancialAccountId.isEmpty()) {
        ...
        hsqlScript.append("(select case when trans is not null then trans.account.id else payment.account.id end from FIN_Finacc_Transaction trans right outer join trans.finPayment payment where payment=pay)");
      ...
      if (strGroupCrit.equalsIgnoreCase("APRM_FATS_BPARTNER")) {
        ...
        hsqlScript.append(" (select trans.account.name from FIN_Finacc_Transaction trans left outer join trans.finPayment payment where payment.id=pay.id),");
        hsqlScript.append(" pay.");
      ...
      for (Object resultObject : query.list()) {
        if (resultObject.getClass().isArray()) {
          ...
          for (Object value : values) {
            if (index == 0) {
              obqPSDList.add(OBDal.getInstance().get(FIN_PaymentScheduleDetail.class, (String) value));
              StringPSDId = (String) value;
            } else if (index == 2) {
              if (value != null) {
                hashMapTransactions.put(StringPSDId, OBDal.getInstance().get(FIN_FinaccTransaction.class, value));
              }
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        if (FIN_PaymentScheduleDetail[i].getPaymentDetails() != null) {
          ...
          if (bp == null) {
            ...
            FieldProviderFactory.setField(data[i], "BP_GROUP", bp.getBusinessPartnerCategory().getName());
            // bpartner
            FieldProviderFactory.setField(data[i], "BPARTNER", bp.getName());
          }

          // transCurrency
          transCurrency = FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getCurrency();
          FieldProviderFactory.setField(data[i], "TRANS_CURRENCY", transCurrency.getISOCode());
          // paymentMethod
          FieldProviderFactory.setField(data[i], "PAYMENT_METHOD", FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getPaymentMethod().getIdentifier());

          // payment
          FieldProviderFactory.setField(data[i], "PAYMENT", ((FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getPaymentDate() != null) ? dateFormat.format(FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getPaymentDate()) : "Null") + " - " + FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getDocumentNo());
          // payment description
          FieldProviderFactory.setField(data[i], "PAYMENT_DESC", FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getDescription());
          // payment_id
          FieldProviderFactory.setField(data[i], "PAYMENT_ID", FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getId().toString());
          // payment_date
          FieldProviderFactory.setField(data[i], "PAYMENT_DATE", (FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getPaymentDate() != null) ? dateFormat.format(FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getPaymentDate()) : "Null");
          // payment_docNo
          FieldProviderFactory.setField(data[i], "PAYMENT_DOCNO", FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getDocumentNo());
          // payment yes / no
          FieldProviderFactory.setField(data[i], "PAYMENT_Y_N", "");
          // financialAccount
          FieldProviderFactory.setField(data[i], "FINANCIAL_ACCOUNT", FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getFINFinaccTransactionList().size() != 0 ? FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getFINFinaccTransactionList().get(0).getAccount().getName() : FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getAccount().getName());
          // status
          FieldProviderFactory.setField(data[i], "STATUS", translateRefList(FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getStatus()));
          FieldProviderFactory.setField(data[i], "STATUS_CODE", FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getStatus());
          // is receipt
          ...
          if (hashMapTransactions.containsKey(FIN_PaymentScheduleDetail[i].getId().toString())) {
            FieldProviderFactory.setField(data[i], "DEPOSIT_WITHDRAW_DATE", dateFormat.format(hashMapTransactions.get(FIN_PaymentScheduleDetail[i].getId()).getTransactionDate()));
          } else {
          ...
          // bp_group -- bp_category
          FieldProviderFactory.setField(data[i], "BP_GROUP", FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule().getInvoice().getBusinessPartner().getBusinessPartnerCategory().getName());
          // bpartner
          FieldProviderFactory.setField(data[i], "BPARTNER", FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule().getInvoice().getBusinessPartner().getName());
          // transCurrency
          transCurrency = FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule().getInvoice().getCurrency();
          FieldProviderFactory.setField(data[i], "TRANS_CURRENCY", transCurrency.getISOCode());
          // paymentMethod
          FieldProviderFactory.setField(data[i], "PAYMENT_METHOD", FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule().getFinPaymentmethod().getIdentifier());
          // payment
          ...
          // is receipt
          if (FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule().getInvoice().isSalesTransaction()) {
            FieldProviderFactory.setField(data[i], "ISRECEIPT", "Y");
            isReceipt = true;
          } else {
            FieldProviderFactory.setField(data[i], "ISRECEIPT", "N");
        ...
        if (FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule() != null) {
          fillLine(dateFormat, data[i], FIN_PaymentScheduleDetail[i], FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule(), false);
        } else if (FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment() != null) {
          java.util.List<Invoice> invoices = getInvoicesUsingCredit(FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment());
          if (invoices.size() == 1) {
            java.util.List<FIN_PaymentSchedule> ps = getInvoicePaymentSchedules(FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment());
            ...
            // invoiceNumber.
            FieldProviderFactory.setField(data[i], "INVOICE_NUMBER", invoices.size() > 1 ? "**" + getInvoicesDocNos(invoices) : "");
            // payment plan id
            FieldProviderFactory.setField(data[i], "PAYMENT_PLAN_ID", "");
            // payment plan yes / no
            FieldProviderFactory.setField(data[i], "PAYMENT_PLAN_Y_N", invoices.size() != 1 ? "Display:none" : "");
            // payment plan yes / no
            FieldProviderFactory.setField(data[i], "NOT_PAYMENT_PLAN_Y_N", invoices.size() > 1 ? "" : "Display:none");
            // invoiceDate
        ...
        if (!sameCurrency) {
          ...
          if (convRate != null) {
            ...
            if (isReceipt) {
              ...
              FieldProviderFactory.setField(data[i], "BASE_AMOUNT", transAmount.multiply(convRate.getMultipleRateBy()).setScale(stdPrecission, BigDecimal.ROUND_HALF_UP).toString());
            } else {
              FieldProviderFactory.setField(data[i], "TRANS_AMOUNT", transAmount.negate().toString());
              FieldProviderFactory.setField(data[i], "BASE_AMOUNT", transAmount.multiply(convRate.getMultipleRateBy()).setScale(stdPrecission, BigDecimal.ROUND_HALF_UP).negate().toString());
            }
          } else {
            String message = transCurrency.getISOCode() + " -> " + baseCurrency.getISOCode() + " " + strConversionDate;
        ...
        final boolean isCreditPayment = FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule() == null && FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment() != null;

        BigDecimal balance = BigDecimal.ZERO;
        if (isCreditPayment && status != null && "PWNC RPR RPPC PPM RDNC".indexOf(status) >= 0) {
          balance = FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getGeneratedCredit().subtract(FIN_PaymentScheduleDetail[i].getPaymentDetails().getFinPayment().getUsedCredit());
          if (isReceipt) {
            balance = balance.negate();
          }
        } else if (!isCreditPayment && status != null && "PWNC RPR RPPC PPM RDNC RPVOID".indexOf(status) == -1) {
          balance = isReceipt ? transAmount : transAmount.negate();
        }
        if (convRate != null) {
          final int stdPrecission = convRate.getToCurrency().getStandardPrecision().intValue();
          balance = balance.multiply(convRate.getMultipleRateBy()).setScale(stdPrecission, BigDecimal.ROUND_HALF_UP);
        }
        ...
        if (finPaymDetail != null && FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule() != null) {
          mustGroup = finPaymDetail.getFinPayment().getId().equalsIgnoreCase(previousPaymentId) && FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule().getId().equalsIgnoreCase(previousFPSDInvoiceId);
        ...
        } else if (finPaymDetail != null && FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule() == null) {
          mustGroup = finPaymDetail.getFinPayment().getId().equalsIgnoreCase(previousPaymentId) && previousFPSDInvoiceId == null;
          previousPaymentId = finPaymDetail.getFinPayment().getId();
          previousFPSDInvoiceId = null;
        } else if (finPaymDetail == null && FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule() != null) {
          mustGroup = previousPaymentId == null && FIN_PaymentScheduleDetail[i].getInvoicePaymentSchedule().getId().equalsIgnoreCase(previousFPSDInvoiceId);
        ...
        if (mustGroup) {
          ...
          if (previousRow != null) {
            ...
            if (previousConvRate == null) {
              ...
              if (previousRow.getField("ISRECEIPT").equalsIgnoreCase("Y"))
                FieldProviderFactory.setField(previousRow, "BASE_AMOUNT", amountSum.multiply(previousConvRate.getMultipleRateBy()).setScale(stdPrecission, BigDecimal.ROUND_HALF_UP).toString());
              else
                FieldProviderFactory.setField(previousRow, "BASE_AMOUNT", amountSum.multiply(previousConvRate.getMultipleRateBy()).setScale(stdPrecission, BigDecimal.ROUND_HALF_UP).negate().toString());
            }

            if (strAmountFrom.isEmpty() && strAmountTo.isEmpty()) {
              isAmtInLimit = true;
            } else if (!strAmountFrom.isEmpty() && strAmountTo.isEmpty()) {
              isAmtInLimit = Double.parseDouble(previousRow.getField("TRANS_AMOUNT")) >= Double.parseDouble(strAmountFrom);
            } else if (strAmountFrom.isEmpty() && !strAmountTo.isEmpty()) {
              isAmtInLimit = Double.parseDouble(previousRow.getField("TRANS_AMOUNT")) <= Double.parseDouble(strAmountTo);
            } else {
              isAmtInLimit = Double.parseDouble(previousRow.getField("TRANS_AMOUNT")) >= Double.parseDouble(strAmountFrom) && Double.parseDouble(previousRow.getField("TRANS_AMOUNT")) <= Double.parseDouble(strAmountTo);
            }
            if (isAmtInLimit) {
        ...
        if (strGroupCrit.equalsIgnoreCase("APRM_FATS_BPARTNER")) {
          ...
          FieldProviderFactory.setField(previousRow, "GROUP_CRIT_ID", previousRow.getField("TRANS_CURRENCY"));
          FieldProviderFactory.setField(previousRow, "GROUP_CRIT", "Currency");
        } else if (strGroupCrit.equalsIgnoreCase("ACCS_ACCOUNT_ID_D")) {
          FieldProviderFactory.setField(previousRow, "GROUP_CRIT_ID", previousRow.getField("FINANCIAL_ACCOUNT"));
        ...
        if (lastElement != numberOfElements) {
          if (transactionsList.size() > 0) {
            try {
              existsConvRate = insertIntoTotal(groupedData.get(lastElement), transactionsList, totalData, strGroupCrit, strOrdCrit, transactionData, totalTransElements, strConvertCurrency, strConversionDate);
            } catch (OBException e) {
      ...
      while (transactionsList.size() > 0) {
        try {
          transactionData[totalTransElements - transactionsList.size()] = createFieldProviderForTransaction(transactionsList.get(0), transactionData[totalTransElements - transactionsList.size()], strGroupCrit, strConvertCurrency, strConversionDate);
        } catch (OBException e) {
          // If there is no conversion rate
          throw e;
        }
   ...
   * @throws OBException
   */
  private boolean insertIntoTotal(FieldProvider data, java.util.List<FIN_FinaccTransaction> transactionsList, ArrayList<FieldProvider> totalData, String strGroupCrit, String strOrdCrit, FieldProvider[] transactionData, int totalTransElements, String strConvertCurrency, String strConversionDate) throws OBException {

    while (transactionsList.size() > 0 && transactionIsBefore(transactionsList.get(0), data, strGroupCrit, strOrdCrit)) {
      try {
        transactionData[totalTransElements - transactionsList.size()] = createFieldProviderForTransaction(transactionsList.get(0), transactionData[totalTransElements - transactionsList.size()], strGroupCrit, strConvertCurrency, strConversionDate);
      } catch (OBException e) {
        // If there is no conversion rate
   ...
   * @throws OBException
   */
  private FieldProvider createFieldProviderForTransaction(FIN_FinaccTransaction transaction, FieldProvider transactionData, String strGroupCrit, String strConvertCurrency, String strConversionDate) throws OBException {
    String dateFormatString = OBPropertiesProvider.getInstance().getOpenbravoProperties().getProperty("dateFormat.java");
    SimpleDateFormat dateFormat = new SimpleDateFormat(dateFormatString);
    BigDecimal transAmount = null;
    ConversionRate convRate = null;
    ...
    if (transaction.getBusinessPartner() != null) {
      FieldProviderFactory.setField(transactionData, "BP_GROUP", transaction.getBusinessPartner().getBusinessPartnerCategory().getName());
      // bpartner
    ...
    // payment_date
    FieldProviderFactory.setField(transactionData, "PAYMENT_DATE", dateFormat.format(transaction.getDateAcct()));
    // payment_docNo
    ...
    // deposit/withdraw date
    FieldProviderFactory.setField(transactionData, "DEPOSIT_WITHDRAW_DATE", dateFormat.format(transaction.getDateAcct()));
    // project
    ...
    // expectedDate.
    FieldProviderFactory.setField(transactionData, "EXPECTED_DATE", dateFormat.format(transaction.getDateAcct()));
    // plannedDSO
    ...
    if (!sameCurrency) {
      ...
      if (convRate != null) {
        ...
        FieldProviderFactory.setField(transactionData, "BASE_AMOUNT", transAmount.multiply(convRate.getMultipleRateBy()).setScale(stdPrecission, BigDecimal.ROUND_HALF_UP).toString());
      } else {
        String message = transaction.getCurrency().getISOCode() + " -> " + baseCurrency.getISOCode() + " " + strConversionDate;
    ...
    if (strGroupCrit.equalsIgnoreCase("APRM_FATS_BPARTNER")) {
      ...
      FieldProviderFactory.setField(transactionData, "GROUP_CRIT_ID", transactionData.getField("TRANS_CURRENCY"));
      FieldProviderFactory.setField(transactionData, "GROUP_CRIT", "Currency");
    } else if (strGroupCrit.equalsIgnoreCase("ACCS_ACCOUNT_ID_D")) {
      FieldProviderFactory.setField(transactionData, "GROUP_CRIT_ID", transactionData.getField("FINANCIAL_ACCOUNT"));
   ...
   */
  private boolean transactionIsBefore(FIN_FinaccTransaction transaction, FieldProvider data, String strGroupCrit, String strOrdCrit) {
    ...
    if (transaction.getBusinessPartner() != null) {
      ...
      BPCategory = transaction.getBusinessPartner().getBusinessPartnerCategory().getName().toString();
    }
    ...
    if (!strGroupCrit.equals("")) {
      ...
      if (strGroupCrit.equalsIgnoreCase("APRM_FATS_BPARTNER")) {
        ...
        if (BPName.equals(data.getField("BPARTNER"))) {
          isBefore = isBeforeStatusAndOrder(transaction, data, strOrdCrit, BPName, BPCategory, strProject);
        ...
        if (BPCategory.equals(data.getField("BP_GROUP"))) {
          isBefore = isBeforeStatusAndOrder(transaction, data, strOrdCrit, BPName, BPCategory, strProject);
        } else if ((pos < posData || data.getField("BP_GROUP").equals("")) && !BPCategory.equals("")) {
          isBefore = true;
        }
      } else if (strGroupCrit.equalsIgnoreCase("INS_CURRENCY")) {
        if (transaction.getCurrency().getISOCode().toString().compareTo(data.getField("TRANS_CURRENCY")) == 0) {
          isBefore = isBeforeStatusAndOrder(transaction, data, strOrdCrit, BPName, BPCategory, strProject);
        } else if (transaction.getCurrency().getISOCode().toString().compareTo(data.getField("TRANS_CURRENCY")) < 0) {
          isBefore = true;
        }
      } else if (strGroupCrit.equalsIgnoreCase("ACCS_ACCOUNT_ID_D")) {
        if (acctList == null) {
        ...
        if (transaction.getAccount().getName().equals(data.getField("FINANCIAL_ACCOUNT"))) {
          isBefore = isBeforeStatusAndOrder(transaction, data, strOrdCrit, BPName, BPCategory, strProject);
        } else if ((pos < posData || data.getField("FINANCIAL_ACCOUNT").equals("")) && !transaction.getAccount().getName().equals("")) {
          isBefore = true;
        }
      }
   ...
   * @return
   */
  private boolean isBeforeStatusAndOrder(FIN_FinaccTransaction transaction, FieldProvider data, String strOrdCrit, String BPName, String BPCategory, String strProject) {
    boolean isBefore = false;

    if (transaction.getStatus().toString().equals(data.getField("STATUS_CODE"))) {
      if (!strOrdCrit.isEmpty()) {
        String[] strOrdCritList = strOrdCrit.substring(2, strOrdCrit.length() - 2).split("', '");
        isBefore = isBeforeOrder(transaction, data, strOrdCritList, 0, BPName, BPCategory, strProject);
      }
   ...
   * @return
   */
  private boolean isBeforeOrder(FIN_FinaccTransaction transaction, FieldProvider data, String[] strOrdCritList, int i, String BPName, String BPCategory, String strProject) {
    boolean isBefore = false;
    ...
    if (i == strOrdCritList.length - 1) {
      if (strOrdCritList[i].contains("Project")) {
        ...
        isBefore = isBefore || (((pos < posData) || data.getField("PROJECT").equals("")) && !strProject.equals(""));
      }
      if (strOrdCritList[i].contains("FINPR_BPartner_Category")) {
        if (bpCategoryList == null) {
          createBPCategoryList();
        ...
        isBefore = isBefore || (((pos < posData) || data.getField("BP_GROUP").equals("")) && !BPCategory.equals(""));
      }
      if (strOrdCritList[i].contains("APRM_FATS_BPARTNER")) {
        if (bpList == null) {
          createBPList();
        ...
        isBefore = isBefore || (((pos < posData) || data.getField("BPARTNER").equals("")) && !BPName.equals(""));
      }
      if (strOrdCritList[i].contains("INS_CURRENCY")) {
        isBefore = isBefore || (transaction.getCurrency().getISOCode().toString().compareTo(data.getField("TRANS_CURRENCY")) < 0);
      }
      if (strOrdCritList[i].equalsIgnoreCase("DueDate")) {
      ...
      if (strOrdCritList[i].equalsIgnoreCase("ACCS_ACCOUNT_ID_D")) {
        ...
        int posData = acctList.indexOf(data.getField("FINANCIAL_ACCOUNT"));
        int pos = acctList.indexOf(transaction.getAccount().getName());
        isBefore = isBefore || (((pos < posData) || data.getField("FINANCIAL_ACCOUNT").equals("")) && !transaction.getAccount().getName().equals(""));
      }
      return isBefore;
    } else {
      if (strOrdCritList[i].contains("Project")) {
        ...
        if ((pos < posData || data.getField("PROJECT").equals("")) && !strProject.equals("")) {
          ...
          isBefore = isBeforeOrder(transaction, data, strOrdCritList, i + 1, BPName, BPCategory, strProject);
        }
        ...
        if ((pos < posData || data.getField("FINANCIAL_ACCOUNT").equals("")) && !transaction.getAccount().getName().equals("")) {
          isBefore = true;
  ...
  private boolean isBeforeStatus(String firstValue, String secondValue) {
    ...
    try {
      ...
      obCriteria.add(Restrictions.ilike("r." + org.openbravo.model.ad.domain.Reference.PROPERTY_NAME, "FIN_Payment status"));
  ...
  private java.util.List<FIN_FinaccTransaction> getTransactionsList(String strInclSubOrg, String strOrg, String strcBPartnerIdIN, String strFinancialAccountId, String strDocumentDateFrom, String strDocumentDateTo, String strPaymentDateFrom, String strPaymentDateTo, String strAmountFrom, String strAmountTo, String strcBPGroupIdIN, String strcProjectIdIN, String strfinPaymSt, String strcCurrency, String strPaymType, String strGroupCrit, String strOrdCrit, String strcNoBusinessPartner, String strDueDateFrom, String strDueDateTo, String strExpectedDateFrom, String strExpectedDateTo) {
    ...
    java.util.List<BusinessPartner> bPartners = OBDao.getOBObjectListFromString(BusinessPartner.class, strcBPartnerIdIN);
    java.util.List<Project> projects = OBDao.getOBObjectListFromString(Project.class, strcProjectIdIN);
    OBContext.setAdminMode(true);
    try {
      OBCriteria<FIN_FinaccTransaction> obCriteriaTrans = OBDal.getInstance().createCriteria(FIN_FinaccTransaction.class);
      obCriteriaTrans.createAlias(FIN_FinaccTransaction.PROPERTY_BUSINESSPARTNER, "bp", OBCriteria.LEFT_JOIN);
      obCriteriaTrans.createAlias("bp." + BusinessPartner.PROPERTY_BUSINESSPARTNERCATEGORY, "bpc", OBCriteria.LEFT_JOIN);
      obCriteriaTrans.createAlias(FIN_FinaccTransaction.PROPERTY_PROJECT, "p", OBCriteria.LEFT_JOIN);
      obCriteriaTrans.createAlias(FIN_FinaccTransaction.PROPERTY_CURRENCY, "c", OBCriteria.LEFT_JOIN);
      obCriteriaTrans.createAlias(FIN_FinaccTransaction.PROPERTY_ACCOUNT, "acc", OBCriteria.LEFT_JOIN);
      obCriteriaTrans.add(Restrictions.isNull(FIN_FinaccTransaction.PROPERTY_FINPAYMENT));
      obCriteriaTrans.add(Restrictions.in(FIN_FinaccTransaction.PROPERTY_ORGANIZATION, organizations));
      ...
      if (strcNoBusinessPartner.equals("include")) {
        ...
        if (!bPartners.isEmpty()) {
          obCriteriaTrans.add(Restrictions.or(Restrictions.in(FIN_FinaccTransaction.PROPERTY_BUSINESSPARTNER, bPartners), Restrictions.isNull(FIN_FinaccTransaction.PROPERTY_BUSINESSPARTNER)));
        }

        // BPartner Category
        if (!strcBPGroupIdIN.equals("")) {
          obCriteriaTrans.add(Restrictions.or(Restrictions.eq("bp." + BusinessPartner.PROPERTY_BUSINESSPARTNERCATEGORY, strcBPGroupIdIN), Restrictions.isNull(FIN_FinaccTransaction.PROPERTY_BUSINESSPARTNER)));
        }
        ...
        // BPartners
        if (!bPartners.isEmpty()) {
          obCriteriaTrans.add(Restrictions.in(FIN_FinaccTransaction.PROPERTY_BUSINESSPARTNER, bPartners));
        }

        // BPartner Category
        if (!strcBPGroupIdIN.equals("")) {
          obCriteriaTrans.add(Restrictions.eq("bp." + BusinessPartner.PROPERTY_BUSINESSPARTNERCATEGORY, strcBPGroupIdIN));
        }

        if (bPartners.isEmpty() && strcBPGroupIdIN.equals("")) {
          obCriteriaTrans.add(Restrictions.isNotNull(FIN_FinaccTransaction.PROPERTY_BUSINESSPARTNER));
        }
      ...
      if (!strFinancialAccountId.equals("")) {
        obCriteriaTrans.add(Restrictions.eq(FIN_FinaccTransaction.PROPERTY_ACCOUNT, OBDal.getInstance().get(FIN_FinancialAccount.class, strFinancialAccountId)));
      }

      // Document Date, Payment Date & Due Date
      if (!strDocumentDateFrom.equals("")) {
        obCriteriaTrans.add(Restrictions.ge(FIN_FinaccTransaction.PROPERTY_DATEACCT, FIN_Utility.getDate(strDocumentDateFrom)));
      }
      if (!strDocumentDateTo.equals("")) {
        obCriteriaTrans.add(Restrictions.le(FIN_FinaccTransaction.PROPERTY_DATEACCT, FIN_Utility.getDate(strDocumentDateTo)));
      }
      if (!strPaymentDateFrom.equals("")) {
        obCriteriaTrans.add(Restrictions.ge(FIN_FinaccTransaction.PROPERTY_DATEACCT, FIN_Utility.getDate(strPaymentDateFrom)));
      }
      if (!strPaymentDateTo.equals("")) {
        obCriteriaTrans.add(Restrictions.le(FIN_FinaccTransaction.PROPERTY_DATEACCT, FIN_Utility.getDate(strPaymentDateTo)));
      }
      if (!strDueDateFrom.equals("")) {
        obCriteriaTrans.add(Restrictions.ge(FIN_FinaccTransaction.PROPERTY_DATEACCT, FIN_Utility.getDate(strDueDateFrom)));
      }
      if (!strExpectedDateTo.equals("")) {
        obCriteriaTrans.add(Restrictions.le(FIN_FinaccTransaction.PROPERTY_DATEACCT, FIN_Utility.getDate(strExpectedDateFrom)));
      }

      // Amount
      if (!strAmountFrom.equals("")) {
        obCriteriaTrans.add(Restrictions.or(Restrictions.ge(FIN_FinaccTransaction.PROPERTY_DEPOSITAMOUNT, new BigDecimal(strAmountFrom)), Restrictions.ge(FIN_FinaccTransaction.PROPERTY_PAYMENTAMOUNT, new BigDecimal(strAmountFrom))));
      }
      if (!strAmountTo.equals("")) {
        obCriteriaTrans.add(Restrictions.or(Restrictions.le(FIN_FinaccTransaction.PROPERTY_DEPOSITAMOUNT, new BigDecimal(strAmountTo)), Restrictions.le(FIN_FinaccTransaction.PROPERTY_PAYMENTAMOUNT, new BigDecimal(strAmountTo))));
      }
      ...
      if (!strcCurrency.equals("")) {
        obCriteriaTrans.add(Restrictions.eq(FIN_FinaccTransaction.PROPERTY_CURRENCY, OBDal.getInstance().get(Currency.class, strcCurrency)));
      }
  ...
  private void fillLine(SimpleDateFormat dateFormat, FieldProvider data, FIN_PaymentScheduleDetail fIN_PaymentScheduleDetail, FIN_PaymentSchedule paymentSchedule, boolean creditPaysInvoice) {
    Date invoicedDate;
    long plannedDSO = 0;
    long currentDSO = 0;
    long currentTime = 0;
    ...
    if (paymentSchedule.getInvoice().getProject() != null)
      FieldProviderFactory.setField(data, "PROJECT", paymentSchedule.getInvoice().getProject().getIdentifier());
    else
      FieldProviderFactory.setField(data, "PROJECT", "");
    // salesPerson
    if (paymentSchedule.getInvoice().getSalesRepresentative() != null) {
      FieldProviderFactory.setField(data, "SALES_PERSON", paymentSchedule.getInvoice().getSalesRepresentative().getIdentifier());
    } else {
      FieldProviderFactory.setField(data, "SALES_PERSON", "");
    }
    // invoiceNumber
    FieldProviderFactory.setField(data, "INVOICE_NUMBER", (creditPaysInvoice ? "*" : "") + paymentSchedule.getInvoice().getDocumentNo());
    // payment plan id
    ...
    // dueDate
    FieldProviderFactory.setField(data, "DUE_DATE", dateFormat.format(paymentSchedule.getDueDate()).toString());
    // expectedDate
    FieldProviderFactory.setField(data, "EXPECTED_DATE", dateFormat.format(paymentSchedule.getExpectedDate()).toString());
    // plannedDSO
    ...
    if (fIN_PaymentScheduleDetail.getPaymentDetails() != null) {
      currentDSO = (((fIN_PaymentScheduleDetail.getPaymentDetails().getFinPayment().getPaymentDate() != null) ? fIN_PaymentScheduleDetail.getPaymentDetails().getFinPayment().getPaymentDate().getTime() : 0) - invoicedDate.getTime()) / milisecDayConv;
    } else {
  ...
  public ConversionRate getConversionRate(Currency transCurrency, Currency baseCurrency, String _conversionDate) {

    java.util.List<ConversionRate> convRateList;
    ConversionRate convRate;
    Date conversionDateObj = FIN_Utility.getDate(_conversionDate);

    // TRUNCATE DATE
    Date conversionDate = null;
    try {
      SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
      conversionDate = sdf.parse(sdf.format(conversionDateObj));
    } catch (Exception e) {
      return null;
    }
  ...
  public static String translateRefList(String strCode) {
    ...
    try {
      ...
      if ("en_US".equals(language.getLanguage()) || strMessage == null) {
        ...
        strMessage = (obc.list() != null && obc.list().size() > 0) ? obc.list().get(0).getName() : null;
      }
  ...
  private void createBPList() {
    ...
    OBCriteria<BusinessPartner> critBPartner = OBDal.getInstance().createCriteria(BusinessPartner.class);
  ...
  private void createAcctList() {
    ...
    OBCriteria<FIN_FinancialAccount> critAcct = OBDal.getInstance().createCriteria(FIN_FinancialAccount.class);
