...
public class ProcessInvoice extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strVoidInOutDocument = vars.getGlobalVariable("inpVoidInOutDocument",
          "ProcessInvoice|VoidInOutDocument", "");
      ...
      if ((org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(),
        ...
        printPageDocAction(response, vars, strC_Invoice_ID, strdocaction, strProcessing,
            strdocstatus, stradTableId, strWindowId, strTabId, strVoidInOutDocument);
      }
      ...
      String strVoidInOutDocument = vars.getStringParameter("inpVoidInOutDocument");
      ...
      try {
        ...
        boolean voidInOutDocument = "Y".equals(strVoidInOutDocument) ? true : false;

        if ("CO".equals(strdocaction) && ("SCOARINVOICE".equals(invoice.getSCOSpecialDocType())
            || "SCOARTICKET".equals(invoice.getSCOSpecialDocType()))) {
          if ("SCONOTDEFINED".equals(invoice.getPaymentMethod().getScoSpecialmethod())) {
            throw new Exception("@SSA_NotDefinedPaymentMethod@");
          }
        }

        if ("VO".equals(strdocaction) && ("SCOARINVOICE".equals(invoice.getSCOSpecialDocType())
            || "SCOARTICKET".equals(invoice.getSCOSpecialDocType()))) {
          if (voidInOutDocument) {
            List<org.openbravo.model.common.invoice.InvoiceLine> invoicelines = invoice
                .getInvoiceLineList();

            List<ShipmentInOut> shipments = new ArrayList<ShipmentInOut>();
            ShipmentInOut shipment = null;
            for (int i = 0; i < invoicelines.size(); i++) {
              if (invoicelines.get(i).getGoodsShipmentLine() != null) {
                if (shipment != invoicelines.get(i).getGoodsShipmentLine().getShipmentReceipt()) {
                  shipment = invoicelines.get(i).getGoodsShipmentLine().getShipmentReceipt();

                  if (!"VO".equals(shipment.getDocumentStatus())) {
            ...
            if (shipments.size() > 1) {
              throw new Exception("@SSA_InvWithMoreOneShipment@");
            }

            if (shipments.size() > 0) {
              OBError myError = doVistoPorAnulacion(shipments.get(0));
              if ("Error".equals(myError.getType()))
                throw new OBException("@SSA_CannotVoidShipment@" + myError.getMessage());
              else if (!"Success".equals(myError.getType()))
                throw new OBException("@SSA_CannotVoidShipment@");
            }
          }
        }

        if (strdocaction.equals("RE") || strdocaction.equals("VO")) {
          boolean isAutomaticTrx = false;
          if (invoice.isScoIsautomatictrx() != null) {
            isAutomaticTrx = invoice.isScoIsautomatictrx();
          }

          if (isAutomaticTrx) {
            // we will reactivate the payment
            FIN_Payment payment = invoice.getScoAutopayment();
            if (payment != null) {

              if (!payment.getStatus().equals("RPAP")) {
                OBError message = FIN_AddPayment.processPayment(vars, new DalConnectionProvider(),
                    "R", payment);
                if (message.getType().equals("Error")) {
                  throw new OBException("@SCO_InvREAutomaticTrxError@:" + message.getMessage());
                }
              }
              ...
              if (payment.getStatus().compareTo("RPAP") != 0) {
                throw new OBException("@SCO_InvREAutomaticTrxError@");
              }

              invoice.setScoAutopayment(null);
              OBDal.getInstance().save(invoice);
              OBDal.getInstance().flush();
        ...
        if ("CO".equals(strdocaction)) {
          ...
          try {
            ...
            if (isAutomaticTrx) {
              ...
              DocumentType doctype = SCO_Utils.getDocTypeFromSpecial(invoice.getOrganization(),
                  "SCOAPPAYMENT");
              if (doctype == null) {
                throw new OBException("@SCO_InvCOAutomaticTrxError@");
              }

              // this is for Invoice Date
              /*
               * Date paymentdate = invoice.getScoNewdateinvoiced(); if (paymentdate == null) {
               * paymentdate = invoice.getInvoiceDate(); }
               */
              Date paymentdate = invoice.getInvoiceDate(); // this
              // is
              // for
              // Receipt
              // Date
              if (invoice.getScoAutotrxpaymentdate() != null)
                paymentdate = invoice.getScoAutotrxpaymentdate();

              Currency currencyfrom = invoice.getCurrency();
              Currency currencyto = invoice.getScrFinFinaccount().getCurrency();
              BigDecimal rate = new BigDecimal(0);
              if (currencyfrom.getId().compareTo(currencyto.getId()) == 0) {
                rate = new BigDecimal(1);
              } else {
                ConversionRate conv = SCO_Utils.getConversionRate(currencyfrom, currencyto,
                    paymentdate);
                if (conv == null) {
                  throw new OBException("@SCO_InvCOAutomaticTrxCurrConvError@");

                }
                rate = conv.getDivideRateBy();
              }
              ...
              payment.setDocumentNo(SCO_Utils.getDocumentNo(doctype, "FIN_Payment"));
              payment.setStatus("RPAP");
              payment.setProcessed(false);
              payment.setProcessNow(false);
              payment.setAccount(invoice.getScrFinFinaccount());
              ...
              String description = "DOC/" + invoice.getScrPhysicalDocumentno();

              payment.setDescription(description);

              OBDal.getInstance().save(payment);
              invoice.setScoAutopayment(payment);
              OBDal.getInstance().save(invoice);
              OBDal.getInstance().flush();

              payment = dao.getObject(FIN_Payment.class, payment.getId());
              invoice = dao.getObject(Invoice.class, invoice.getId());
              List<Invoice> invoices = new ArrayList<Invoice>();
              List<BigDecimal> amts = new ArrayList<BigDecimal>();

              invoices.add(invoice);
              BigDecimal amt = invoice.getOutstandingAmount().divide(rate,
                  invoice.getCurrency().getStandardPrecision().intValue(), RoundingMode.HALF_UP);
              amts.add(amt);
              BigDecimal totalamt = amt;

              payment = SMG_Utils.execSimplePaymentMultiple(vars, new DalConnectionProvider(),
                  payment, invoices, amts, totalamt, false, payment.getCurrency(), rate, totalamt,
                  "D");
        ...
        if ("CO".equals(strdocaction)) {
          ...
          // UGO Commented. The credit windows will not be used
          /*
           * OBContext.setAdminMode(false); String invoiceDocCategory = ""; try { invoiceDocCategory
           * = invoice.getDocumentType().getDocumentCategory();
           * 
           * // // Print a grid popup in case of credit payment // // If the invoice grand total is
           * ZERO or already has payments (due to // payment method automation) or the business
           * partner does not have a default financial // account defined or invoice's payment
           * method is not inside BP's financial // account do not cancel credit if
           ...
           * AcctServer.DOCTYPE_APInvoice .equals(invoiceDocCategory))) { creditPayments = dao
           * .getCustomerPaymentsWithCredit(invoice.getOrganization(), invoice.getBusinessPartner(),
           * invoice.isSalesTransaction(), invoice.getCurrency()); if (creditPayments != null &&
           * !creditPayments.isEmpty()) { printPageCreditPaymentGrid(response, vars,
           * strC_Invoice_ID, strdocaction, strTabId, strC_Invoice_ID, strdocaction, strWindowId,
           * strTabId, invoice.getInvoiceDate(), strOrg); } } } finally {
           * OBContext.restorePreviousMode(); }
           */
        ...
        if (message.contains("@"))
          message = Utility.parseTranslation(this, vars, vars.getLanguage(), message);

        OBError errmsg = new OBError();
        errmsg
            .setTitle(Utility.parseTranslation(this, vars, vars.getLanguage(), "@OBUIAPP_Error@"));
      ...
      if (strCreditPaymentIds != null && !strCreditPaymentIds.isEmpty()) {
        ...
        try {
          ...
          for (final FIN_Payment creditPayment : selectedCreditPayment) {
            ...
            // Set Used Credit = Amount + Previous used credit
          ...
          final FIN_Payment newPayment = FIN_AddPayment.savePayment(null, null, isSalesTransaction,
          ...
          for (final FIN_Payment creditPayment : selectedCreditPayment) {
            final BigDecimal usedCreditAmt = selectedCreditPaymentAmounts
                .get(creditPayment.getId());
            // FIX-ME Added by Pedro, getting index 0 from
            // scheduledetails, it is ok?
  ...
  public OBError doVistoPorAnulacion(ShipmentInOut shipment) {
    OBError myMessage = null;
    ...
    try {
      ...
      Process process = null;
      try {
        process = OBDal.getInstance().get(Process.class, "43C34D847BB34127A1A6B5363C371C26");
      } finally {
        OBContext.restorePreviousMode();
      }

      Map<String, String> parameters = null;
      final ProcessInstance pinstance = CallProcess.getInstance().call(process, shipment.getId(),
          parameters);
      OBDal.getInstance().flush();
  ...
  private void executePayments(HttpServletResponse response, VariablesSecureApp vars,
    ...
    if (payments != null && payments.size() > 0) {
      ...
      response.sendRedirect(
          getDireccion() + "/org.openbravo.advpaymentmngt.ad_actionbutton/ExecutePayments.html");
    } else {
      String strWindowPath = Utility.getTabURL(strTabId, "R", true, getDireccion());
      if (strWindowPath.equals(""))
        strWindowPath = strDefaultServlet;
  ...
  void printPageDocAction(HttpServletResponse response, VariablesSecureApp vars,
      ...
      String stradTableId, String strWindowId, String strTabId, String strVoidInOutDocument)
    ...
    xmlDocument.setParameter("voidInOutDocument", strVoidInOutDocument);
    ...
    Invoice invoice = (Invoice) OBDal.getInstance().getProxy(Invoice.ENTITY_NAME, strC_Invoice_ID);
    if (strWindowId.equals(Purchase_Invoice_Window)) {
      // VOID action: Reverse sales/purchase invoice by default takes
      // today as document date and
      // accounting date.
    ...
    FieldProvider[] dataDocAction = ActionButtonUtility.docAction(this, vars, strdocaction, "135",
        strdocstatus, strProcessing, stradTableId, strTabId);
    ...
    xmlDocument.setParameter("strSpecialDocType",
        "strSpecialDocType = \"" + invoice.getSCOSpecialDocType() + "\";\r\n");

    String deliveryViaRule = "";
    if (invoice.getSalesOrder() != null) {
      deliveryViaRule = invoice.getSalesOrder().getDeliveryMethod();
    }
    xmlDocument.setParameter("strDeliveryViaRule",
        "strDeliveryViaRule = \"" + deliveryViaRule + "\";\r\n");
  ...
  void returnError(HttpServletResponse response, VariablesSecureApp vars, String strTabId,
      String error) throws IOException, ServletException {
    OBError msg = Utility.translateError(this, vars, vars.getLanguage(), error);
    vars.setMessage(strTabId, msg);
    String strWindowPath = Utility.getTabURL(strTabId, "R", true, getDireccion());
    if (strWindowPath.equals(""))
      strWindowPath = strDefaultServlet;
  ...
  // UGO Commented. FIN_PaymentDetailV.PROPERTY_INVOICEPAYMENTPLAN column was
  // deleted because
  // fin_payment_sched_inv_v view is broken(takes too much time)
  /*
   ...
   * Restrictions.eq(FIN_PaymentSchedule.PROPERTY_INVOICE, invoice)).list()) { for
   * (FIN_PaymentDetailV pdv : OBDao.getFilteredCriteria(FIN_PaymentDetailV.class,
   * Restrictions.eq(FIN_PaymentDetailV.PROPERTY_INVOICEPAYMENTPLAN, ps)).list()) { if
   * (pdv.getPayment() != null && !"RPVOID".equals(pdv.getPayment().getStatus())) { return true; } }
   * }
   * 
   * return false; }
   */
