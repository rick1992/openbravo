...
public class SelectorComponent extends BaseTemplateComponent {
  ...
  public static final String TAB_FIX_PARAMETER = "WithTabFix";
  ...
  public static String getAdditionalProperties(Selector selector, boolean onlyDisplayField) {
    if (onlyDisplayField && (selector.getDisplayfield() == null || !selector.getDisplayfield().isActive())) {
      return "";
    }
  ...
  public boolean getTabFix() {
    return hasParameter(TAB_FIX_PARAMETER);
  }
  ...
  public String getDefaultPopupFilterField() {
    if (getSelector().isCustomQuery()) {
      if (getSelector().getDisplayfield() != null && getSelector().getDisplayfield().getDisplayColumnAlias() != null) {
        return getSelector().getDisplayfield().getDisplayColumnAlias().replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR);
      }
    ...
    if (getSelector().getDisplayfield() != null && getSelector().getDisplayfield().isShowingrid()) {
      if (getSelector().getDisplayfield().getProperty() != null) {
        return getSelector().getDisplayfield().getProperty().replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR);
      } else {
        return getSelector().getDisplayfield().getObserdsDatasourceField().getName().replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR);
      }
    }
    // a very common case, return the first selector field which is part of the
    // identifier
    if (getSelector().getDisplayfield() == null || getSelector().getDisplayfield().getProperty().equals(JsonConstants.IDENTIFIER)) {
      final Entity entity = getEntity();
      if (entity != null) {
        for (Property prop : entity.getIdentifierProperties()) {
          for (SelectorField selectorField : getActiveSelectorFields()) {
            if (selectorField.getProperty() != null && selectorField.getProperty().equals(prop.getName())) {
  ...
  public org.openbravo.userinterface.selector.Selector getSelector() {
    if (selector == null) {
      selector = OBDal.getInstance().get(org.openbravo.userinterface.selector.Selector.class, getId());
  ...
  private List<SelectorField> getActiveSelectorFields() {
    if (selectorFields == null) {
      selectorFields = OBDao.getActiveOBObjectList(getSelector(), Selector.PROPERTY_OBUISELSELECTORFIELDLIST);
    }
    return selectorFields;
  }

  public String getColumnName() {
    Check.isTrue(hasParameter(SelectorConstants.PARAM_COLUMN_NAME), "The " + SelectorConstants.PARAM_COLUMN_NAME + " parameter must be set");
  ...
  public String getComboReload() {
    ...
    Check.isTrue(hasParameter(SelectorConstants.PARAM_TAB_ID), "The " + SelectorConstants.PARAM_TAB_ID + " parameter must be set");
  ...
  public String getShowSelectorGrid() {
    if (OBDao.getFilteredCriteria(SelectorField.class, Restrictions.eq(SelectorField.PROPERTY_OBUISELSELECTOR, getSelector()), Restrictions.eq(SelectorField.PROPERTY_SHOWINGRID, true)).count() > 0) {
  ...
  public String getDataSourceJavascript() {
    ...
    if (getSelector().getObserdsDatasource() != null) {
      ...
      Check.isNotNull(getSelector().getTable(), "Both the datasource and table are null for this selector: " + selector);
  ...
  public List<OutSelectorField> getOutFields() {
    ...
    try {
      ...
      for (SelectorField selectorField : sortedFields) {
        if (Boolean.TRUE.equals(selectorField.isOutfield())) {
          if (tabId.equals("")) {
            ...
            List<Field> outFieldTargetFields = getOutFieldListForSelectorField(tabId, selectorField.getId());

            if (outFieldTargetFields.size() == 0 && selectorField.getSuffix() != null && !selectorField.getSuffix().equals("")) {
              ...
              outField.setOutSuffix((selectorField.getSuffix() == null ? "" : selectorField.getSuffix()));
              outFields.add(outField);
            }
            for (Field associatedField : outFieldTargetFields) {
              ...
              if (associatedField.getTab().getId().equals(tabId)) {
                ...
                if (getParameter("isSelectorItem") != null) {
                  final Property property = KernelUtils.getInstance().getPropertyFromColumn(associatedField.getColumn(), false);
                  outField.setTabFieldName(property.getName());
                  UIDefinition uiDef = UIDefinitionController.getInstance().getUIDefinition(associatedField.getColumn().getId());
      ...
      log.error("Error gettting out fields for selector " + getParameter(SelectorConstants.PARAM_TAB_ID) + ": " + e.getMessage(), e);
    } finally {
  ...
  public Map<String, String> getHiddenInputs() {
    ...
    OBContext.setAdminMode();
    try {
      final Criterion selectorConstraint = Restrictions.eq(SelectorField.PROPERTY_OBUISELSELECTOR, getSelector());
      final Criterion isOutFieldConstraint = Restrictions.eq(SelectorField.PROPERTY_ISOUTFIELD, true);
      final Criterion hasSuffixConstraint = Restrictions.isNotNull(SelectorField.PROPERTY_SUFFIX);
      List<SelectorField> fields = OBDao.getFilteredCriteria(SelectorField.class, selectorConstraint, isOutFieldConstraint, hasSuffixConstraint).list();
      for (final SelectorField field : fields) {
        hiddenInputs.put(columnName + field.getSuffix(), getElementString.replaceAll("@id@", columnName + field.getSuffix()));
      }
    } catch (Exception e) {
      log.error("Error getting hidden input for selector " + getParameter(SelectorConstants.PARAM_TAB_ID) + ": " + e.getMessage(), e);
    } finally {
      OBContext.restorePreviousMode();
    }
  ...
  private List<LocalSelectorField> getSelectorFields(boolean pickList, boolean popupGrid) {
    ...
    for (SelectorField selectorField : sortedFields) {
      ...
      if (pickList && (!selectorField.isShowInPicklist() || selectorField.equals(getSelector().getDisplayfield()))) {
        continue;
      }
      final LocalSelectorField localSelectorField = new LocalSelectorField();
      ...
      final DomainType domainType = getDomainType(selectorField);
      if (domainType instanceof ForeignKeyDomainType) {
        String displayField = fieldName.replace(".", DalUtil.FIELDSEPARATOR) + DalUtil.FIELDSEPARATOR + JsonConstants.IDENTIFIER;
  ...
  private List<SelectorFieldTrl> getTranslatedFields() {
    ...
    final String whereClause = " where " + SelectorFieldTrl.PROPERTY_OBUISELSELECTORFIELD + "." + SelectorField.PROPERTY_OBUISELSELECTOR + "=:selector and " + SelectorFieldTrl.PROPERTY_LANGUAGEID + ".id=:userLanguageId ";

    final OBQuery<SelectorFieldTrl> qry = OBDal.getInstance().createQuery(SelectorFieldTrl.class, whereClause);
  ...
  public static class OutSelectorField {
    ...
    public void setTabFieldName(String tabFieldName) {
      this.tabFieldName = tabFieldName != null ? tabFieldName.replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR) : tabFieldName;
    }
    ...
    public void setOutFieldName(String outFieldName) {
      this.outFieldName = outFieldName != null ? outFieldName.replace(DalUtil.DOT, DalUtil.FIELDSEPARATOR) : outFieldName;
    }
  ...
  public static class LocalSelectorField {
    ...
    public String getFilterEditorProperties() {
      if (getUIDefinition() != null) {
        if (getUIDefinition() instanceof FKComboUIDefinition && isSelectorItem) {
          return ", canFilter:true, required: false, filterEditorType: 'OBSelectorFilterSelectItem', filterEditorProperties: {entity: '" + getEntityName() + "'}";
        }
    ...
    public List<LocalSelectorFieldProperty> getProperties() {
      ...
      if (domainType instanceof StringEnumerateDomainType) {
        ...
        if (reference != null) {
          ...
          List<RefListEntry> entries = DataSourceProperty.createValueMap(allowedValues, reference.getId());
    ...
    private LocalSelectorFieldProperty createLocalSelectorFieldProperty(String propName, Object propValue) {
  ...
  private UIDefinition getUIDefinition(SelectorField selectorField) {
    if (selectorField.getObuiselSelector().getTable() != null && selectorField.getProperty() != null) {
      ...
      return UIDefinitionController.getInstance().getUIDefinition(selectorField.getObserdsDatasourceField().getReference());
    }
    return null;
  }
