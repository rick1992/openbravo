...
public class FIN_ExecutePayment {
	...
	public void init(String sourceType, PaymentExecutionProcess _executionProcess, List<FIN_Payment> payments, HashMap<String, String> _parameters, Organization organization) throws NoExecutionProcessFoundException {
		...
		final List<PaymentExecutionProcessParameter> allParameters = executionProcess.getFinancialMgmtPaymentExecutionProcessParameterList();
		for (PaymentExecutionProcessParameter parameter : allParameters)
			if ("IN".equals(parameter.getParameterType()))
				dao.getNewPaymentRunParameter(paymentRun, parameter, parameters.get(parameter.getSearchKey()));
		...
		try {
			this.paymentExecutionProcess = (FIN_PaymentExecutionProcess) Class.forName(executionProcess.getJavaClassName()).newInstance();
	...
	public OBError execute() {
		...
		try {
			...
			String strmessage2 = OBMessageUtils.messageBD("SSA_BusinessPartnerBlocked");
			for (PaymentRunPayment payRunPayment : paymentRun.getFinancialMgmtPaymentRunPaymentList()) {
				...
				if (payment.getBusinessPartner() != null) {
					if (FIN_Utility.isBlockedBusinessPartner(payment.getBusinessPartner().getId(), payment.isReceipt(), 4)) {
						// If the Business Partner is blocked, the Payment can
						// not be completed.
						OBError error = new OBError();
						error.setType("Error");
						error.setMessage(strmessage1 + " " + payment.getBusinessPartner().getIdentifier() + " " + strmessage2);
					...
					try {
						for (FIN_PaymentDetail pd : payment.getFINPaymentDetailList()) {
							for (FIN_PaymentScheduleDetail psd : pd.getFINPaymentScheduleDetailList()) {
								...
								if (bPartner != null && FIN_Utility.isBlockedBusinessPartner(bPartner.getId(), payment.isReceipt(), 4)) {
									// If the Business Partner is blocked for
									// Payments, the Payment will not be
									// completed.
									OBError error = new OBError();
									error.setType("Error");
			...
			if (paymentExecutionProcess != null) {
				...
				for (PaymentRunPayment paymentRunPayment : paymentRun.getFinancialMgmtPaymentRunPaymentList()) {
					...
					if ("S".equals(paymentRunPayment.getResult())) {
						...
						try {
							// UGO CHANGE. WHY ONLY CREATE TRX FOR INVOICED
							// PAYMENTS????
							...
							if (FIN_Utility.isAutomaticDepositWithdrawn(paymentRunPayment.getPayment()) && paymentRunPayment.getPayment().getAmount().compareTo(BigDecimal.ZERO) != 0 && ismigrationpayment == false) {
								FIN_FinaccTransaction transaction = TransactionsDao.createFinAccTransaction(paymentRunPayment.getPayment());

								VariablesSecureApp vars = new VariablesSecureApp(RequestContext.get().getRequest());
								OBError processTransactionError = processTransaction(vars, new DalConnectionProvider(), "P", transaction);
							 ...
							 * System.out.println
							 * ("enter FIN_ExecutePayment execute 8");
							 * 
							 * if (pd.getGLItem() != null) { continue; }
							 * System.out
							 * .println("enter FIN_ExecutePayment execute 9");
							 * 
							 * for (FIN_PaymentScheduleDetail psd :
							 * pd.getFINPaymentScheduleDetailList()) { if
							 * (psd.isInvoicePaid() &&
							 * FIN_Utility.isAutomaticDepositWithdrawn
							 * (paymentRunPayment.getPayment()) &&
							 * paymentRunPayment
							 * .getPayment().getAmount().compareTo
							 * (BigDecimal.ZERO) != 0) { FIN_FinaccTransaction
							 * transaction = TransactionsDao
							 * .createFinAccTransaction
							 * (paymentRunPayment.getPayment());
							 * System.out.println
							 * ("enter FIN_ExecutePayment execute 10");
							 * 
							 * VariablesSecureApp vars = new
							 * VariablesSecureApp(RequestContext.get()
							 * .getRequest()); OBError processTransactionError =
							 * processTransaction(vars, new
							 * DalConnectionProvider(), "P", transaction); if
							 * (processTransactionError != null &&
							 * "Error".equals
							 * (processTransactionError.getType())) { return
							 * processTransactionError; } System.out.println(
							 * "enter FIN_ExecutePayment execute 11");
							 * 
							 * } } break; }
							 */
	...
	FIN_PaymentProposal getPaymentProposalFromPayment(FIN_Payment payment) {
		FIN_PaymentPropDetailV ppv = FIN_Utility.getOneInstance(FIN_PaymentPropDetailV.class, new Value(FIN_PaymentPropDetailV.PROPERTY_PAYMENT, payment));
		...
		for (FIN_PaymentDetail pDetail : payment.getFINPaymentDetailList()) {
			for (FIN_PaymentScheduleDetail psd : pDetail.getFINPaymentScheduleDetailList()) {
				if (psd.getInvoicePaymentSchedule() != null) {
					...
					FIN_AddPayment.updatePaymentScheduleAmounts(pDetail, psd.getInvoicePaymentSchedule(), psd.getAmount(), psd.getWriteoffAmount(), "P");
				}
				if (psd.getOrderPaymentSchedule() != null) {
					FIN_AddPayment.updatePaymentScheduleAmounts(psd.getOrderPaymentSchedule(), psd.getAmount(), psd.getWriteoffAmount(), "P");
				}
				if (pDetail.isPrepayment() && psd.getOrderPaymentSchedule() == null && psd.getInvoicePaymentSchedule() == null) {
					// This PSD is credit
