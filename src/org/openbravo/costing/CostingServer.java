...
public class CostingServer {
   ...
   * 
   */
  public void process() throws OBException, ImportException {
    if (trxCost != null) {
    ...
    //  System.out.println(transaction.getId());
     ...
     // System.out.println("PRIMERO");
      ...
      trxCost = trxCost.setScale(costingAlgorithm.getCostCurrency().getCostingPrecision().intValue(), RoundingMode.HALF_UP);
      
      System.out.println("trxCost: " + trxCost);
  ...
  private void setNotPostedTransaction() {
    ...
    case ReceiptNegative: {
      org.openbravo.model.materialmgmt.transaction.ShipmentInOut inout = transaction.getGoodsShipmentLine().getShipmentReceipt();
    ...
    case InternalConsVoid: {
      InternalConsumption consumption = transaction.getInternalConsumptionLine().getInternalConsumption();
    ...
    case ReceiptChangeCode:
      SwaMovementCode movement = transaction.getSwaMovcodeProduct().getSWAMovementcodePlan().getSWAMovementcode();
      if (!"N".equals(movement.getPosted()) || !"Y".equals(movement.getPosted())) {
        movement.setPosted("N");
        OBDal.getInstance().save(movement);
      }
  ...
  private CostingAlgorithm getCostingAlgorithm() {
    ...
    org.openbravo.model.materialmgmt.cost.CostingAlgorithm costAlgorithm = costingRule.getCostingAlgorithm();
    ...
    if (transaction.getProduct().isProduction() && !"org.openbravo.costing.StandardAlgorithm".equals(costAlgorithm.getJavaClassName())) {
      OBQuery<org.openbravo.model.materialmgmt.cost.CostingAlgorithm> caQry = OBDal.getInstance().createQuery(org.openbravo.model.materialmgmt.cost.CostingAlgorithm.class, org.openbravo.model.materialmgmt.cost.CostingAlgorithm.PROPERTY_JAVACLASSNAME + " = 'org.openbravo.costing.AverageAlgorithm'");
    ...
    try {
      ...
      log4j.error("Exception loading Algorithm class: " + costAlgorithm.getJavaClassName() + " algorithm: " + costAlgorithm.getIdentifier());
  ...
  private CostingRule getCostDimensionRule() {
    ...
    where.append(" order by case when " + CostingRule.PROPERTY_STARTINGDATE + " is null then 1 else 0 end, " + CostingRule.PROPERTY_STARTINGDATE + " desc");
    ...
    if (costRules.size() == 0) {
      throw new OBException("@NoCostingRuleFoundForOrganizationAndDate@ @Organization@: " + organization.getName() + ", @Date@: " + OBDateUtils.formatDate(transaction.getTransactionProcessDate()));
    }
  ...
  public Organization getOrganization() {
    ...
    Organization org = OBContext.getOBContext().getOrganizationStructureProvider(transaction.getClient().getId()).getLegalEntity(transaction.getOrganization());
    if (org == null) {
   ...
   */
  public enum TrxType {
    Shipment, ShipmentReturn, ShipmentVoid, ShipmentNegative, Receipt, ReceiptReturn, ReceiptVoid, ReceiptNegative, InventoryIncrease, InventoryDecrease, IntMovementFrom, IntMovementTo, InternalCons, InternalConsNegative, InternalConsVoid, BOMPart, BOMProduct, ManufacturingConsumed, ManufacturingProduced, ShipmentChangeCode, ReceiptChangeCode, Unknown;
    /**
    ...
    static TrxType getTrxType(MaterialTransaction transaction) {
      if (transaction.getGoodsShipmentLine() != null) {
        ...
        if (inout.isSalesTransaction()) {
          // Shipment
          if (inout.getDocumentStatus().equals("VO") && transaction.getGoodsShipmentLine().getCanceledInoutLine() != null) {
          ...
          } else if (transaction.getGoodsShipmentLine().getMovementQuantity().compareTo(BigDecimal.ZERO) < 0) {
        ...
        if (transaction.getMovementQuantity().compareTo(BigDecimal.ZERO) > 0) {
          log4j.debug("Physical inventory, increments stock: " + transaction.getPhysicalInventoryLine().getIdentifier());
          return InventoryIncrease;
        } else {
          log4j.debug("Physical inventory, decreases stock " + transaction.getPhysicalInventoryLine().getIdentifier());
        ...
        if (transaction.getInternalConsumptionLine().getVoidedInternalConsumptionLine() != null) {
          ...
          log4j.debug("Negative Internal Consumption: " + transaction.getInternalConsumptionLine().getIdentifier());
          return InternalConsNegative;
        } else {
          log4j.debug("Internal Consumption: " + transaction.getInternalConsumptionLine().getIdentifier());
          return InternalCons;
        }
      } else if (transaction.getProductionLine() != null) {
        // Production Line
        ProductionTransaction production = transaction.getProductionLine().getProductionPlan().getProduction();
        if (production.isSalesTransaction()) {
          // BOM Production
          if ((!production.isSwaDesarmado() && transaction.getMovementQuantity().compareTo(BigDecimal.ZERO) > 0) || (production.isSwaDesarmado() && transaction.getMovementQuantity().compareTo(BigDecimal.ZERO) < 0)) {
      ...
      } else if (transaction.getSwaMovcodeProduct() != null) {

        if (transaction.getMovementQuantity().compareTo(BigDecimal.ZERO) < 0) {
          return ShipmentChangeCode;
        } else {
          return ReceiptChangeCode;
        }
