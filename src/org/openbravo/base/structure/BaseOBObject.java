...
public abstract class BaseOBObject implements BaseOBObjectDef, Identifiable, DynamicEnabled, OBNotSingleton, Serializable {
  private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(BaseOBObject.class);
    ...
    if (p.isTranslatable() && OBContext.hasTranslationInstalled()) {
      if (!hasLookedForTrl) {
        ...
        try {
          ...
          List<BaseOBObject> trl = OBDal.getInstance().getSession().createFilter(this.get(p.getTrlOneToManyProperty().getName()), "where language = ?").setParameter(0, language).list();
          ...
          try {
            if (id != null) {
              if (p.getTrlParentProperty() != null) {
                ...
                OBCriteria<BaseOBObject> trlList = OBDal.getInstance().createCriteria(p.getEntity().getName() + "Trl");
                trlList.add(Restrictions.eq(p.getTrlParentProperty().getName(), OBDal.getInstance().get(p.getEntity().toString(), id)));
  ...
  private void setDataValue(String propName, Object value) {
    ...
    if (p.getIndexInEntity() >= data.length) {
      throw new IllegalArgumentException("Property index (" + p.getIndexInEntity() + ") is larger than or equal to property list size (" + data.length + "). " + "This happens when setting property " + propName + " " + p + " with value " + value + " in entity " + getEntity());
    }
  ...
  public String getSortingIdentifier() {
    return SortingIdentifierProvider.getInstance().getSortingIdentifier(this);
  }
  ...
  protected void checkDerivedReadable(Property p) {
    ...
    if (!isAllowRead() && obContext != null && obContext.isInitialized() && !obContext.isInAdministratorMode()) {
      ...
      if (isDerivedReadable && !p.allowDerivedRead()) {
        throw new OBSecurityException("Entity " + getEntity() + " is not directly readable, only id and identifier properties are readable, property " + p + " is neither of these.");
      }
    }
  }
