...
public class TemplateInfo {
  ...
  public TemplateInfo(ConnectionProvider connectionProvider, String docTypeId, String orgId,
      String strLanguage, String templateId, DocumentType documentType, String docStatus, String isebill)
    ...
    final TemplateData template = getSelectedTemplate(templates, templateId, documentType,
        docStatus);
  ...
  public TemplateInfo(ConnectionProvider connectionProvider, String docTypeId, String orgId,
      String strLanguage, String templateId, String isebill) throws ServletException, ReportingException {
    templates = TemplateData.getDocumentTemplates(connectionProvider, docTypeId, orgId, isebill);
  ...
  private TemplateData getSelectedTemplate(TemplateData[] templates, String templateId) {
    return getSelectedTemplate(templates, templateId, null, "");
  }

  private TemplateData getSelectedTemplate(TemplateData[] templates, String templateId,
      DocumentType documentType, String docStatus) {
    if (documentType != null) {
      TemplateData tplTemplate = null;

      switch (documentType) {
      case ACCOUNTABILITY:
        tplTemplate = getAccountabilityTemplate(templates, templateId, docStatus);
        break;
      }
  ...
  TemplateData getTemplate(TemplateData[] templates, String templateFilename) {
    for (int i = 0; i < templates.length; i++) {
      if (templates[i].templateFilename.equals(templateFilename))
        return templates[i];
    }
    return null;
  }

  // Rpt_Entregas_Rendir_ugo.jrxml -> CO
  // Rpt_CEntrega_Rendir_ugo.jrxml -> DR y otros docStatus
  TemplateData getAccountabilityTemplate(TemplateData[] templates, String templateFilename,
      String docStatus) {
    if ("CO".equals(docStatus)) {
      return getTemplate(templates, "Rpt_Entregas_Rendir_ugo.jrxml");
    } else {
      return getTemplate(templates, "Rpt_CEntrega_Rendir_ugo.jrxml");
    }
  }
