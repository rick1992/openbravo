...
import pe.com.unifiedgo.core.data.SCRTabredirect;
...
public class ApplicationDynamicComponent extends BaseTemplateComponent {
  ...
  public Set<Entity> getAccessibleEntities() {
    final Set<Entity> entities = OBContext.getOBContext().getEntityAccessChecker().getReadableEntities();
  ...
  private Set<Entity> removeInaccessibleEntities(Set<Entity> entities) {
    ...
    for (Entity entity : entities) {
      ...
      if (windowAccessible(windowAccessList, window) || windowAccessible(windowAccessList, poWindow)) {
        accessibleEntities.add(entity);
      } else {
        // check in scr_tabredirect table too
        List<SCRTabredirect> tabredirects = table.getSCRTabredirectList();
        if (windowAccessible(windowAccessList, tabredirects)) {
          accessibleEntities.add(entity);
        }
      }
    }
  ...
  private boolean windowAccessible(List<WindowAccess> windowAccessList, List<SCRTabredirect> tabredirects) {

    for (int i = 0; i < tabredirects.size(); i++) {
      Window window = tabredirects.get(i).getWindow();
      if (window == null) {
        continue;
      }
      for (WindowAccess wa : windowAccessList) {
  ...
  public String getETag() {
    ...
    return super.getETag() + "_" + c.getRole().getId() + "_" + c.getUser().getId() + "_" + c.getCurrentClient().getId() + "_" + c.getCurrentOrganization().getId();
  }
}
