...
public class SCO_Swithholding_Receipt_DocType extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strSCOSwithholdingReceiptId = vars.getStringParameter("inpscoSwithholdingReceiptId");
      String strDescription = vars.getStringParameter("inpdescription");
      String strTabId = vars.getStringParameter("inpTabId");

      try {
        printPage(response, vars, strDocTypeTarget, strDocType, docNo, strSCOSwithholdingReceiptId, strDescription, strTabId);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strDocTypeTarget, String strDocType, String docNo, String strSCOSwithholdingReceiptId, String strDescription, String strTabId) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    if (strDocTypeTarget.equals(""))
      ...
      resultado.append("var calloutName='SCO_Swithholding_Receipt_DocType';\n\n");
      ...
      SCOSwithholdingReceiptDocTypeData[] data = null;

      if (!newDocNo && !"0".equals(strDocType)) {
        data = SCOSwithholdingReceiptDocTypeData.select(this, strDocType);
      ...
      SCOSwithholdingReceiptDocTypeData[] dataNew = SCOSwithholdingReceiptDocTypeData.select(this, strDocTypeTarget);
      if (dataNew != null && dataNew.length > 0) {
        DocSubTypeSO = dataNew[0].docsubtypeso;
        if (DocSubTypeSO == null)
          DocSubTypeSO = "--";
        String strOldDocTypeTarget = SCOSwithholdingReceiptDocTypeData.selectOldDocSubType(this, strSCOSwithholdingReceiptId);
        if (!DocSubTypeSO.equals("OB") && strOldDocTypeTarget.equals("OB")) {
          String strOldDocNo = SCOSwithholdingReceiptDocTypeData.selectOldDocNo(this, strSCOSwithholdingReceiptId);
        ...
        if (dataNew[0].isdocnocontrolled.equals("Y")) {
          if (!newDocNo && !AD_Sequence_ID.equals(dataNew[0].adSequenceId) && !SCOSwithholdingReceiptDocTypeData.selectOldDocTypeTargetId(this, strSCOSwithholdingReceiptId).equalsIgnoreCase(strDocTypeTarget))
            newDocNo = true;
          if (newDocNo) {
