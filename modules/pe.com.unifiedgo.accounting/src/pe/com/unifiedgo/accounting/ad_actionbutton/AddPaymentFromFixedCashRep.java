...
public class AddPaymentFromFixedCashRep extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strDocumentId = vars.getGlobalVariable("inpscoFixedcashRepositionId", "");
      ...
      String strGlitemId = org.getScoFcashrepGlitem().getId();
      ...
      String strDocumentId = vars.getRequestGlobalVariable("inpscoFixedcashRepositionId", "");
      ...
      boolean isReceipt = !(strDocstatus.equals("DR")
          || (!strDocstatus.equals("DR") && number.doubleValue() < 0));
      ...
      try {
        ...
        if (strPaymentDocumentNo.startsWith("<")) {
          // get DocumentNo
          strPaymentDocumentNo = Utility.getDocumentNo(this, vars, "AddPaymentFromFixedCashRep",
        ...
        FIN_FinancialAccount financialacc = dao.getObject(FIN_FinancialAccount.class,
            strFinancialAccountId);
        // FIN_Payment payment = dao.getNewPayment(isReceipt, dao.getObject(Organization.class,
        // strOrgId), dao.getObject(DocumentType.class, strDocTypeId), strPaymentDocumentNo,
        // businessPartner, dao.getObject(FIN_PaymentMethod.class, strPaymentMethodId),
        // financialacc, strPaymentAmount, FIN_Utility.getDate(strPaymentDate), strReferenceNo,
        ...
        SCOFixedcashReposition fixedcash_rep = OBDal.getInstance().get(SCOFixedcashReposition.class,
            strDocumentId);
        if (fixedcash_rep == null) {
          final OBError invmsg = new OBError();
          String invmessage = Utility.parseTranslation(this, vars, vars.getLanguage(),
              "@SCO_PaymentFixedCashInvFC@");

          invmsg.setType("Error");
          invmsg.setMessage(invmessage);
          invmsg.setTitle(Utility.parseTranslation(this, vars, vars.getLanguage(), "@Error@"));
          OBDal.getInstance().rollbackAndClose();
          vars.setMessage(strTabId, invmsg);
        ...
        if (addedGLITemsArray != null) {
          for (int i = 0; i < addedGLITemsArray.length(); i++) {
            ...
            /*
             * if (isReceipt) { glItemAmt = glItemInAmt.subtract(glItemOutAmt); } else { glItemAmt =
             * glItemOutAmt.subtract(glItemInAmt); } final String strGLItemId =
             * glItem.getString("glitemId");
             * 
             * FIN_AddPayment.saveGLItem(payment, glItemAmt, dao.getObject(GLItem.class,
             * strGLItemId), businessPartner, null, null, null, null, null, null, null, null,
             * "Reposicion de fondo fijo: "+fixedcash_rep.getDescription(),null);
             */
            ...
            FIN_AddPayment.saveGLItem(payment, glItemAmtReal,
                dao.getObject(GLItem.class, strGLItemId), businessPartner, null, null, null, null,
                null, null, null, null,
                "Reposicion de fondo fijo: " + fixedcash_rep.getDescription(), glItemAmt,
                exchangeRate, OBDal.getInstance().get(Currency.class, docCurrencyId), null, null);
        ...
        // payment = FIN_AddPayment.savePayment(null, payment, isReceipt, null, null, null, null,
        // null, strPaymentAmount, null, null, null, selectedPaymentDetails,
        // selectedPaymentDetailAmounts, false, false, dao.getObject(Currency.class,
        // paymentCurrencyId), exchangeRate, convertedAmount);
        payment = FIN_AddPayment.savePayment(null, payment, isReceipt, null, null, null, null, null,
            strPaymentAmount.toString(), null, null, null, selectedPaymentDetails,
            selectedPaymentDetailAmounts, false, false, finacc.getCurrency(), exchangeRate,
            strConvertedAmount);

        if (!fixedcash_rep.getDocumentStatus().equals("DR")) {
          final OBError invmsg = new OBError();
          String invmessage = Utility.parseTranslation(this, vars, vars.getLanguage(),
              "@SCO_PaymentFixedCashNotDR@");
        ...
        if (fixedcash_rep.getFinancialAccount().getId().equals(financialacc.getId())) {
          final OBError invmsg = new OBError();
          String invmessage = Utility.parseTranslation(this, vars, vars.getLanguage(),
              "@SCO_PaymentFixedCashSameFinAcc@");
        ...
        /*
         * if (!fixedcash_rep.getCurrency().getId().equals(financialacc.getCurrency().getId())) {
         * final OBError invmsg = new OBError(); String invmessage = Utility.parseTranslation(this,
         * vars, vars.getLanguage(), "@SCO_PaymentFixedCashDiffCurrency@");
         * 
         * invmsg.setType("Error"); invmsg.setMessage(invmessage);
         * invmsg.setTitle(Utility.parseTranslation(this, vars, vars.getLanguage(), "@Error@"));
         * OBDal.getInstance().rollbackAndClose(); vars.setMessage(strTabId, invmsg);
         * printPageClosePopUpAndRefreshParent(response, vars); return; }
         */

        fixedcash_rep.setDocumentStatus("CO");
        fixedcash_rep.setProcessed(true);
        fixedcash_rep.setProcessNow(true);
        fixedcash_rep.setPayment(payment);
        fixedcash_rep.setFINFinancialAccountFrom(financialacc);
        fixedcash_rep.setDocumentType(fixedcash_rep.getTransactionDocument());

        payment.getSCOFixedcashRepositionList().add(fixedcash_rep);

        OBDal.getInstance().save(fixedcash_rep);
        OBDal.getInstance().save(payment);
        OBDal.getInstance().flush();

        FIN_Payment preliqpayment = fixedcash_rep.getPreliqpayment();
        if (preliqpayment != null) {
          preliqpayment.setScoFixedcashrepStatus("SCO_REP");
          OBDal.getInstance().save(preliqpayment);
          OBDal.getInstance().flush();
        }
          ...
          try {
            ...
            if (payment.getFINPaymentDetailList().size() > 0) {
              ...
              if (strDocstatus.equals("DR")) {
                // add the desc to the payment
                List<FIN_PaymentDetail> pds = payment.getFINPaymentDetailList();
                for (int i = 0; i < pds.size(); i++) {
                  FIN_PaymentDetail pd = pds.get(i);
                  pd.setScoDescription(fixedcash_rep.getDescription());
                  pd.getSCOFixedcashRepositionList().add(fixedcash_rep);
                  fixedcash_rep.setPaymentDetails(pd);
              ...
              if (strDocstatus.equals("DR") && fixedcash_rep.getRespbpartner() != null) {
                // add the resp to the payment
                payment.setScoRespbpartner(fixedcash_rep.getRespbpartner());
                OBDal.getInstance().save(payment);
              }
      ...
      final OBError invmsg = new OBError();
      String invmessage = Utility.parseTranslation(this, vars, vars.getLanguage(), "@Success@");
      invmsg.setType("Success");
      invmsg.setMessage(invmessage);
      invmsg.setTitle(Utility.parseTranslation(this, vars, vars.getLanguage(), "@Success@"));
      vars.setMessage(strTabId, invmsg);
      printPageClosePopUpAndRefreshParent(response, vars);
      return;
    }
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars,
    ...
    XmlDocument xmlDocument = xmlEngine
        .readXmlTemplate("pe/com/unifiedgo/accounting/ad_actionbutton/AddPaymentFromFixedCashRep")
        .createXmlDocument();
    ...
    xmlDocument.setParameter("title",
        Utility.messageBD(this, "SCO_AddPaymentFromFixedCashRepTitle", vars.getLanguage()));
    ...
    String strDocTypeId = (String) CallStoredProcedure.getInstance().call("AD_GET_DOCTYPE",
        parameters, null);
    String strDocNo = Utility.getDocumentNo(this, vars, "AddPaymentFromFixedCashRep", "FIN_Payment",
    ...
    try {
      ComboTableData comboTableData = new ComboTableData(vars, this, "TABLEDIR", "C_Currency_ID",
          "", "",
          Utility.getContext(this, vars, "#AccessibleOrgTree", "AddPaymentFromFixedCashRep"),
          Utility.getContext(this, vars, "#User_Client", "AddPaymentFromFixedCashRep"), 0);
      Utility.fillSQLParameters(this, vars, null, comboTableData, "AddPaymentFromFixedCashRep",
          strCurrencyId);
