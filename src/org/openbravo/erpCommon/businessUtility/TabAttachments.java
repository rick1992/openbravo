...
public class TabAttachments extends HttpSecureAppServlet {
  ...
  public void post(VariablesSecureApp vars, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    if (vars.getCommand().startsWith("SAVE_NEW")) {
      ...
      if (!oberrInsert.getType().equals("Success")) {
        ...
        response.sendRedirect(getDireccion() + request.getServletPath() + "?Command=DEFAULT");
      } else {
        if (vars.commandIn("SAVE_NEW_RELATION")) {
          response.sendRedirect(getDireccion() + request.getServletPath() + "?Command=DEFAULT&inpcFileId=" + strFileReference);
        } else if (vars.commandIn("SAVE_NEW_EDIT")) {
          response.sendRedirect(getDireccion() + request.getServletPath() + "?Command=EDIT&inpcFileId=" + strFileReference);
        } else if (vars.commandIn("SAVE_NEW_NEW")) {
          response.sendRedirect(getDireccion() + request.getServletPath() + "?Command=NEW");
      ...
      if (TabAttachmentsData.update(this, vars.getUser(), strDataType, strText, strFileReference) == 0) {
        ...
        if (vars.commandIn("SAVE_EDIT_RELATION")) {
          ...
          response.sendRedirect(getDireccion() + request.getServletPath() + "?Command=NEW&inpKey=" + key);
          ...
          final String strNewFile = TabAttachmentsData.selectNext(this, Utility.getContext(this, vars, "#User_Client", strWindow), Utility.getContext(this, vars, "#AccessibleOrgTree", strWindow), strFileReference, tableId, key);
      ...
      final boolean editable = vars.getGlobalVariable("inpEditable", "TabAttachments.editable").equals("Y");
  ...
  private void printPageFileMultiple(HttpServletResponse response, VariablesSecureApp vars) throws IOException {
    ...
    try {
      ...
      OBCriteria<Attachment> attachmentFiles = OBDao.getFilteredCriteria(Attachment.class, Restrictions.eq("table.id", tableId), Restrictions.in("record", recordIds.split(",")));
      ...
      for (Attachment attachmentFile : attachmentFiles.list()) {
        String attachmentDirectory = TabAttachments.getAttachmentDirectory(tableId, recordIds, attachmentFile.getName());
        final File file = new File(globalParameters.strFTPDirectory + "/" + attachmentDirectory, attachmentFile.getName());
        String zipName = "";
        if (!writtenFiles.containsKey(file.getName())) {
          ...
          zipName = file.getName().substring(0, indDot) + " (" + num + ")" + file.getName().substring(indDot);
  ...
  private OBError insert(VariablesSecureApp vars, String strFileReference, String tableId, String key, String strDataType, String strText) throws IOException, ServletException {

    String adOrgId = vars.getOrg();
    String tablename = TabAttachmentsData.selectTableName(this, tableId);
    if (tablename != null && !tablename.isEmpty()) {
      try {
        String sqlQuery;
        sqlQuery = "SELECT AD_ORG_ID FROM " + tablename + " WHERE " + tablename + "_ID = \'" + key + "\' LIMIT 1;";
        Query q = OBDal.getInstance().getSession().createSQLQuery(sqlQuery);
        String g_adOrgId = (String) q.uniqueResult();
        if (g_adOrgId != null && !g_adOrgId.equals("")) {
          adOrgId = g_adOrgId;
        }
      } catch (Exception e) {
        e.printStackTrace();
    ...
    try {
      ...
      final TabAttachmentsData[] files = TabAttachmentsData.select(this, "'" + vars.getClient() + "'", "'" + adOrgId + "'", tableId, key);
      ...
      if (!fileExists) {
        ...
        TabAttachmentsData.insert(conn, this, cFileId, vars.getClient(), adOrgId, vars.getUser(), tableId, key, strDataType, strText, strName, path);
      } else {
  ...
  private OBError delete(VariablesSecureApp vars, String strFileReference) throws IOException, ServletException {
    OBError myMessage = null;
    myMessage = new OBError();
    myMessage.setTitle("");
    ...
    try {
      ...
      try {
        ...
        // FIXME: Get the directory separator from Java runtime
        String attachmentDirectory = TabAttachments.getAttachmentDirectory(data[0].adTableId, data[0].adRecordId, data[0].name);
        final File file = new File(globalParameters.strFTPDirectory + "/" + attachmentDirectory, data[0].name);
        if (file.exists())
          f = new FileUtility(globalParameters.strFTPDirectory + "/" + attachmentDirectory, data[0].name, false);
        else
  ...
  private void printPageFS(HttpServletResponse response, VariablesSecureApp vars) throws IOException, ServletException {
    ...
    final XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/businessUtility/TabAttachments_FS").createXmlDocument();
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strTab, String strWindow, String key, boolean editable) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    final TabAttachmentsData[] files = TabAttachmentsData.select(this, Utility.getContext(this, vars, "#User_Client", strWindow), Utility.getContext(this, vars, "#AccessibleOrgTree", strWindow), tableId, key);

    if ((files == null) || (files.length == 0))
      discard[0] = "widthData";
    final XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/businessUtility/TabAttachments_F1", discard).createXmlDocument();
  ...
  private void printPageEdit(HttpServletResponse response, VariablesSecureApp vars, String strTab, String strWindow, String key, String strFileReference) throws IOException, ServletException {
    if (log4j.isDebugEnabled())
    ...
    final XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/businessUtility/TabAttachments_Edition", discard).createXmlDocument();
    ...
    xmlDocument.setData("reportAD_Datatype_ID_D", "liststructure", DataTypeComboData.select(this, Utility.getContext(this, vars, "#User_Client", "TabAttachments"), Utility.getContext(this, vars, "#AccessibleOrgTree", "TabAttachments")));
  ...
  private void printPageFile(HttpServletResponse response, VariablesSecureApp vars, String strFileReference, HttpServletRequest request) throws IOException, ServletException {
    ...
    if (file.exists()) {
      f = new FileUtility(globalParameters.strFTPDirectory + "/" + fileDir, data[0].name, false, true);
    } else {
    ...
    if (userAgent.contains("MSIE")) {
      response.setHeader("Content-Disposition", "attachment; filename=\"" + URLEncoder.encode(data[0].name.replace("\"", "\\\""), "utf-8") + "\"");
    } else {
      response.setHeader("Content-Disposition", "attachment; filename=\"" + MimeUtility.encodeWord(data[0].name.replace("\"", "\\\""), "utf-8", "Q") + "\"");
    }
  ...
  private void printPageCheck(HttpServletResponse response, VariablesSecureApp vars, String strTab, String recordId) throws IOException, ServletException {
  ...
  private void editDescOB3(HttpServletResponse response, VariablesSecureApp vars, String attachmentId, String desc, String strTab, String key) {
    OBContext.setAdminMode(true);
    try {
  ...
  public static String getAttachmentDirectoryForNewAttachments(String tableID, String recordID) {
    ...
    try {
      saveAttachmentsOldWay = Preferences.getPreferenceValue("SaveAttachmentsOldWay", true, OBContext.getOBContext().getCurrentClient(), OBContext.getOBContext().getCurrentOrganization(), OBContext.getOBContext().getUser(), OBContext.getOBContext().getRole(), null);
    } catch (PropertyException e) {
      // if property not found, save attachments the new way
      saveAttachmentsOldWay = "N";
    }

    saveAttachmentsOldWay = "N"; // UGO FIX : ALWAYS SAVE WITH NEW WAY
  ...
  public static String getAttachmentDirectory(String tableID, String recordID, String fileName) {
    ...
    try {
      ...
      OBCriteria<Attachment> attachmentCriteria = OBDal.getInstance().createCriteria(Attachment.class);
  ...
  public static String getPath(String fileDirectory) {
    ...
    //UGO FIX: migration records contain "-" in their ids NEVER USE THE OPENBRAVO OLD WAY OF HANDLING FILES WITH PUTS EVERYTHING IN THE FILENAME AND null in path
