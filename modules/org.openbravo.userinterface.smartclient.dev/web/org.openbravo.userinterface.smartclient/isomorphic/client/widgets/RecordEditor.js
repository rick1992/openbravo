...
isc.RecordEditor.addMethods({
    ...
    makeActionButton : function () {
        ...
        var dynamicProperties = {
            ...
            left:this.isRTL() ? 0 : this.getInnerWidth() - this.getScrollbarSize(),
            ...
            icon: actionIcon,
            iconSize: this.getScrollbarSize(),
    ...
    setValuesAsCriteria : function (criteria, refresh, dropExtraCriteria) {
        var form = this.getEditForm();
        if (form == null) {
            this._initialCriteria = criteria;
            return;
        }
        form.setValuesAsCriteria(criteria, null, dropExtraCriteria);
...
//        );
    ...
    getEditorProperties : function (field) {
        ...
        // don't pick a default ODS if there's an explicit setting for ODS or for a specific
        // list of values
        if (!field.optionDataSource && !field.valueMap && !field.filterEditorValueMap) {
            
            // respect foreignKey if present
            if (field.foreignKey) {
                props.optionDataSource = 
                        isc.DS.getForeignDSName(field, this.sourceWidget.getDataSource());
                props.valueField = 
                        isc.DS.getForeignFieldName(field, this.sourceWidget.getDataSource());

            } else if (field.displayField) {
                // otherwise if there's a displayField setting with no ODS on the ListGridField
                // this strongly implies that the "large value map" pattern is in use
                // (a display value is embedded in each record).  "foreignKey" was not
                // declared, so this is not being done via includeFrom and having a related
                // DataSource, instead, the DataSource of the grid itself must be returning
                // displayField values, so that's the one to use as an ODS.
    ...
    layoutChildren : function () {
        ...
        if (this.body) {
            ...
            body.setWidth(Math.max(1, this.getInnerWidth() - this.getScrollbarSize()));
            if (this.actionButton && this.isRTL()) {
                body.setLeft(this.getScrollbarSize());
            }
        }
    ...
    syncSourceScrolling : function () {
            ...
            this.sourceWidget.body.getScrollLeft() != left) 
        {
                ...
                // RTL mode - account for the fact that scrolled to zero (IE hard left) on
                // the main grid body != scrolled to zero (hard left) on the filter editor
                // since their left coords don't align.
                if (this.isRTL()) {
                    var offset = this.sourceWidget.body.getViewportWidth() - 
                                    this.body.getViewportWidth();
                    left -= offset;
                    if (left < 0) left = 0;
                }            
    ...
    getCellAlign : function (record, rowNum, colNum, returnDefault) {
        if (!returnDefault && colNum == this.fields.getLength() -1) {
    ...
    getCellValue : function (record, recordNum, fieldNum, gridBody) {
        ...
        if (fieldNum == this.fields.getLength()-1) {
            var field = this.getField(fieldNum),
                form = this.getEditForm(),
                item = form ? form.getItem(field.name) : null,
                width = (item != null) ? item.getWidth() : null;
            if (width != null) {
                value = "<div style='text-align:" +
                        this.getCellAlign(record,recordNum,fieldNum, true) 
