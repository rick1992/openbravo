...
isc.OBStandardView.addProperties({
  ...
  setNoVisible: function(){
	  if(this.parentTabSet!=null){
		    thistab = 0;
		    for(i=0; i<this.parentTabSet.tabs.length; i++){
		    	if(this.parentTabSet.tabs[i].title == this.tabTitle){
		    		thistab =i; 
		    		this.parentTabSet.removeTab(thistab);
		    	  	break;
		    	}
		    }
  ...
  updateTabTitle: function () {
    ...
    if (this.isRootView && tab) {
      ...
      document.title = OB.User.roleName + " - " + tab.title;
    }
  },
