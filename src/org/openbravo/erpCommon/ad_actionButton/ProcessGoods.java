...
public class ProcessGoods extends HttpSecureAppServlet {
  ...
  void printPageDocAction(HttpServletResponse response, VariablesSecureApp vars,
    ...
    //Vafaster
    //VALIDATION: Cada vez que se anula un Documento, hay que verificar si despuès de la anulación 
    //quedarán las ubicaciones en negativo
    //Validacion para Anulación de NOTA DE INGRESO y DEVOLUCION DE CLIENTE
    ShipmentInOut inOut = OBDal.getInstance().get(ShipmentInOut.class, strM_Inout_ID);  
    String locatorToBeNegative ="";
    if(inOut!=null && inOut.getDocumentAction().equals("RC") && (inOut.getDocumentType().getScoSpecialdoctype().equals("SCOMMRECEIPT") || inOut.getDocumentType().getScoSpecialdoctype().equals("SWARFCRECEIPT"))){
    	String strWarehouseId = inOut.getWarehouse().getId().trim();
    	Warehouse warehouse = OBDal.getInstance().get(Warehouse.class, strWarehouseId);
    	 
    	for(int i=0 ; i< inOut.getMaterialMgmtShipmentInOutLineList().size();i++){
    		String strProductId =  inOut.getMaterialMgmtShipmentInOutLineList().get(i).getProduct().getId().trim();
    		String strLocatorId =  inOut.getMaterialMgmtShipmentInOutLineList().get(i).getStorageBin().getId().trim();
    		...
    		BigDecimal movementqty = inOut.getMaterialMgmtShipmentInOutLineList().get(i).getMovementQuantity();
    		BigDecimal qtyAvailable = BigDecimal.ZERO;
    		qtyAvailable = SWA_Utils.getWarehouseStockInfo(warehouse, locator, product, 4);//Cantidad fisica por Almacén
    		if(movementqty.compareTo(qtyAvailable)==1){
    			locatorToBeNegative = locatorToBeNegative + " - " + product.getSearchKey() + " "+ locator.getSearchKey() + " " + movementqty + "/"+qtyAvailable;
    		}
    	}
    }
    
    if(!locatorToBeNegative.equals("")){
    	OBError myAlertInfo;
    	myAlertInfo = new OBError();
    	myAlertInfo.setType("Warning");
    	myAlertInfo.setTitle(OBMessageUtils.messageBD("ALERTA"));
    	myAlertInfo.setMessage(OBMessageUtils.messageBD(this, "swa_inout_warning_tonegative", vars.getLanguage())+ " - " + locatorToBeNegative);
    	
    	xmlDocument.setParameter("messageType", myAlertInfo.getType());
        xmlDocument.setParameter("messageTitle", myAlertInfo.getTitle());
        xmlDocument.setParameter("messageMessage", myAlertInfo.getMessage());
    }
  //END VALIDATION
