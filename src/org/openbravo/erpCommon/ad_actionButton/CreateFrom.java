...
public class CreateFrom extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      final String strPath = vars.getGlobalVariable("inpPath", "CreateFrom|path",
          getDireccion() + request.getServletPath());
      ...
      if (myMessage.getType().compareTo("Error") == 0) {
  ...
  protected void printPageInvoice(HttpServletResponse response, VariablesSecureApp vars,
    ...
    String invoice_specialdoctype = invoice.getTransactionDocument().getScoSpecialdoctype();
    ...
    if (strPO.equals("") && strShipment.equals("")) {
      ...
      xmlDocument = xmlEngine
          .readXmlTemplate("org/openbravo/erpCommon/ad_actionButton/CreateFrom_Invoice", discard)
          .createXmlDocument();
      data = CreateFromInvoiceData.set();
    } else {
      xmlDocument = xmlEngine
          .readXmlTemplate("org/openbravo/erpCommon/ad_actionButton/CreateFrom_Invoice")
          .createXmlDocument();
      if (strShipment.equals("")) {
        if (vars.getLanguage().equals("en_US")) {
          System.out.println("enter shipment combo3");

          if (isSOTrx.equals("Y"))
            data = CreateFromInvoiceData.selectFromPOSOTrx(this, vars.getLanguage(), strKey,
                Utility.getContext(this, vars, "#User_Client", strWindowId),
          ...
          if (isSOTrx.equals("Y"))
            data = CreateFromInvoiceData.selectFromPOTrlSOTrx(this, vars.getLanguage(), strKey,
                Utility.getContext(this, vars, "#User_Client", strWindowId),
                Utility.getContext(this, vars, "#User_Org", strWindowId), strPO);
          else
        ...
        System.out.println("enter shipment combo4");

        if (vars.getLanguage().equals("en_US")) {
          System.out.println("enter shipment combo5");

          if (isSOTrx.equals("Y")) {
            System.out.println("ACA CRATE FROM SHIPMENT TRX");
          ...
          if (isSOTrx.equals("Y")) {
            System.out.println("ACA CRATE FROM SHIPMENT TRX PERU");
    ...
    xmlDocument.setParameter("pType",
        (!strShipment.equals("") ? "SHIPMENT" : (!strPO.equals("")) ? "PO" : ""));
    xmlDocument.setParameter("windowId", strWindowId);
    xmlDocument.setParameter("tabName", strTabName);
    ...
    if (strBPartner.equals("")) {
      ...
      // Vafaster
      // Este String "validateShipmentByCboItem" permite que el combo de entrada/salidas
      // desde el boton crearLineasde desde una Ventana de Facturacion
      // permita ser filtrado solo por aquellas guias(In/OUT) que tengan asociados
      // "Tipo de Orden"(CboItem)
      // con el check de "Permitir en Factura". Tanto para Factura Cliente / Proveedor.
      String validateShipmentByCboItem = "Y"; // Es String porque el _data.xsql se malogra cuando
                                              // quiero pasar un boleano

      String excludespecialdoctype = "''";
      String pospecialdoctype = "";
      String deliveredOrder = "";
      if (invoice_specialdoctype != null) {
        if (invoice_specialdoctype.compareTo("SCOARINVOICERETURNMAT") == 0) {
          excludespecialdoctype = "'SCOMMRECEIPT','SCOMMSHIPMENT','SSARFSRECEIPT','SWAINTERNALRECEIPT','SWAINTERNALSHIPMENT','SWAMMDISPATCH','SWAMMDISPATCHIN','SWAMMSHIPMENTINDIRECT','SWARTVRECEIPT'";
          pospecialdoctype = "SSARFCORDER";
          deliveredOrder = "deliveredOrder";
        }

        if (invoice_specialdoctype.compareTo("SCOAPCREDITMEMO") == 0) {
          excludespecialdoctype = "'SCOMMRECEIPT','SCOMMSHIPMENT','SSARFSRECEIPT','SWAINTERNALRECEIPT','SWAINTERNALSHIPMENT','SWAMMDISPATCH','SWAMMDISPATCHIN','SWAMMSHIPMENTINDIRECT','SWARFCRECEIPT'";
          pospecialdoctype = "SSARTVORDER";
        }
      }

      if (isSOTrx.equals("Y")) {// Factura Cliente
        System.out.println("enter shipment combo");
        ...
        xmlDocument.setData("reportShipmentReciept", "liststructure",
            CreateFromInvoiceData.selectFromShipmentSOTrxCombo(this, vars.getLanguage(),
                ...
                strBPartner, strIsTaxIncluded, invoice.getCurrency().getId(),
                (validateShipmentByCboItem.equals("Y") ? validateShipmentByCboItem : null),
                excludespecialdoctype,
        ...
        xmlDocument.setData("reportPurchaseOrder", "liststructure",
            CreateFromInvoiceData.selectFromPOSOTrxCombo(this, vars.getLanguage(),
                Utility.getContext(this, vars, "#User_Client", strWindowId), narturalOrgTreeList,
                strBPartner, invoice.getCurrency().getId(), strIsTaxIncluded, pospecialdoctype,
                deliveredOrder,
                (invoice.getProject() != null ? invoice.getProject().getId() : null)));
      } else { // FActura Proveedor
        System.out.println("enter shipment combo2");
      ...
      if (isSOTrx.equals("Y")) {
        xmlDocument.setData("reportInvoice", "liststructure",
            CreateFromShipmentData.selectFromInvoiceTrxCombo(this, vars.getLanguage(),
                Utility.getContext(this, vars, "#User_Client", strWindowId), narturalOrgTreeList,
                strBPartner,
                (shipment.getProject() != null ? shipment.getProject().getId() : null)));
        xmlDocument.setData("reportPurchaseOrder", "liststructure",
        ...
        xmlDocument.setData("reportInvoice", "liststructure",
            CreateFromShipmentData.selectFromInvoiceCombo(this, vars.getLanguage(),
                ...
                strBPartner,
                (shipment.getProject() != null ? shipment.getProject().getId() : null)));
        // Adicionar Validacion --VAFASTER
        // Validar que los c_order a leer (Se muestran en el Combo) sea según el tipo de ingreso
        // CompraNacional o compraImportación u otros
        // Este cambio está pensado para otros tipo de ingreso adicionar un else if mas
        if (shipment != null) {
          if (shipment.getSsaCmbitemValue().equals("CompraNacional"))
            xmlDocument.setData("reportPurchaseOrder", "liststructure",
                CreateFromShipmentData.selectFromPOComboNationalPurchase(this, vars.getLanguage(),
                    Utility.getContext(this, vars, "#User_Client", strWindowId),
                    narturalOrgTreeList, strBPartner,
                    (shipment.getProject() != null ? shipment.getProject().getId() : null)));
          else if (shipment.getSsaCmbitemValue().equals("CompraImportacion"))
            xmlDocument.setData("reportPurchaseOrder", "liststructure",
                CreateFromShipmentData.selectFromPOComboInternationalPurchase(this,
                    vars.getLanguage(), Utility.getContext(this, vars, "#User_Client", strWindowId),
                    narturalOrgTreeList, strBPartner,
                    (shipment.getProject() != null ? shipment.getProject().getId() : null)));
          else
            // ESTE ES EL ORIGINAL de OPENBRAVO
            xmlDocument.setData("reportPurchaseOrder", "liststructure",
  ...
  void printPageInvoiceCombo(HttpServletResponse response, VariablesSecureApp vars,
    ...
    if (strBPartner.equals("")) {
      ...
      if (vars.commandIn("REFRESH_INVOICES")) { // Loading the combos in
        ...
        Invoice invoice = OBDal.getInstance().get(Invoice.class, strKey);
        String strIsTaxIncluded = invoice.getPriceList().isPriceIncludesTax() ? "Y" : "N";
        String invoice_specialdoctype = invoice.getSCOSpecialDocType();
        String excludespecialdoctype = "''";
        String pospecialdoctype = null;
        String deliveredOrder = null;
        if (invoice_specialdoctype != null) {
          if (invoice_specialdoctype.compareTo("SCOARINVOICERETURNMAT") == 0) {
            excludespecialdoctype = "'SCOMMRECEIPT','SCOMMSHIPMENT','SSARFSRECEIPT','SWAINTERNALRECEIPT','SWAINTERNALSHIPMENT','SWAMMDISPATCH','SWAMMDISPATCHIN','SWAMMSHIPMENTINDIRECT','SWARTVRECEIPT'";
            pospecialdoctype = "SSARFCORDER";
        ...
        if (isSOTrx.equals("Y")) {
          strArray = Utility.arrayEntradaSimple("arrDatos",
              CreateFromInvoiceData.selectFromShipmentSOTrxCombo(this, vars.getLanguage(),
                  Utility.getContext(this, vars, "#User_Client", strWindowId),
                  Utility.getContext(this, vars, "#AccessibleOrgTree", strWindowId), strBPartner,
                  strIsTaxIncluded, invoice.getCurrency().getId(),
                  (validateShipmentByCboItem.equals("Y") ? validateShipmentByCboItem : null),
                  excludespecialdoctype,
                  (invoice.getProject() != null ? invoice.getProject().getId() : null)));
          strArray2 = Utility.arrayEntradaSimple("arrDatos2",
              CreateFromInvoiceData.selectFromPOSOTrxCombo(this, vars.getLanguage(),
                  Utility.getContext(this, vars, "#User_Client", strWindowId),
                  Utility.getContext(this, vars, "#AccessibleOrgTree", strWindowId), strBPartner,
                  invoice.getCurrency().getId(), strIsTaxIncluded, pospecialdoctype, deliveredOrder,
                  (invoice.getProject() != null ? invoice.getProject().getId() : null)));
        } else {
          strArray = Utility.arrayEntradaSimple("arrDatos",
              CreateFromInvoiceData.selectFromShipmentCombo(this, vars.getLanguage(),
          ...
          strArray2 = Utility.arrayEntradaSimple("arrDatos2",
              CreateFromInvoiceData.selectFromPOCombo(this, vars.getLanguage(),
                  Utility.getContext(this, vars, "#User_Client", strWindowId),
                  Utility.getContext(this, vars, "#AccessibleOrgTree", strWindowId), strBPartner,
                  invoice.getCurrency().getId(), strIsTaxIncluded, pospecialdoctype,
                  (invoice.getProject() != null ? invoice.getProject().getId() : null)));
        }
      }
    }
  ...
  protected OBError saveInvoice(VariablesSecureApp vars, String strKey, String strTableId,
    ...
    final String strClaves = Utility
        .stringList(vars.getRequiredInParameter("inpcOrderId", IsIDFilter.instance));
    final String isSOTrx = Utility.getContext(this, vars, "isSOTrx", strWindowId);
    final String strIsTaxIncluded = OBDal.getInstance().get(PriceList.class, strPriceList)
        .isPriceIncludesTax() ? "Y" : "N";

    String strPO = "", strInvRef = "", priceActual = "0", priceLimit = "0", priceList = "0",
        strPriceListVersion = "", priceStd = "0", priceListGross = "0", priceStdGross = "0";
    String priceGross = "0";
    CreateFromInvoiceData[] data = null;
    ...
    try {
      ...
      for (int k = 0; k < ids.length; k++) {
        if (strType.equals("SHIPMENT")) {
          if (isSOTrx.equals("Y"))
            ...
            data = CreateFromInvoiceData.selectFromShipmentUpdate(conn, this, vars.getLanguage(),
                ids[k]);
          dataAux = CreateFromInvoiceData.selectPriceList(conn, this, strDateInvoiced,
              strPriceList);
        ...
        if (data != null) {
          ...
          boolean invoiceCopied = false;
          boolean invoiceReferenceFound = false;
          for (int i = 0; i < data.length; i++) {
            final String strSequence = SequenceIdData.getUUID();
            ...
            if (data[i].cOrderlineId.equals(""))
              C_Tax_ID = Tax.get(this, data[i].mProductId, strDateInvoiced, data[i].adOrgId,
                  vars.getWarehouse(), strBPartnerLocation, strBPartnerLocation,
                  CreateFromInvoiceData.selectProject(this, strKey),
                  isSOTrx.equals("Y") ? true : false);
            else
            ...
            if (!data[i].cOrderlineId.equals("")) {
              ...
              if (isSOTrx.equals("Y")) {
                Invoice inv = OBDal.getInstance().get(Invoice.class, strKey);
                // NEW PRICELIST IMPL UGO
                String product_data[] = SCO_Utils.getProductPricesByDate(this,
                    inv.getOrganization().getId(), data[i].mProductId, strPriceList,
                    inv.getInvoiceDate());
                priceList = product_data[0];
                priceStd = product_data[1];
                priceLimit = product_data[2];
            ...
            if (invoice_specialdoctype.equals("SCOAPCREDITMEMO"))
              lineNetAmt = (new BigDecimal(priceActual)).multiply(qty).negate();
            ...
            try {
              ...
              if (!"".equals(planType) && !"".equals(periodNumber)
                  && !"".equals(startingPeriodId)) {
                isDeferred = "Y";
              } else {
                planType = "";
                periodNumber = "";
              ...
              /* Copy header info only if there arent lines */
              System.out.println("PRIMERO");
              if (!invoiceCopied) {

                System.out.println("SEGUNDO");

                if (invoice != null && invoice.getInvoiceLineList().size() == 0) {
                  if (strType.equals("SHIPMENT")) {
                    if (data[i].mInoutlineId != null) {

                      System.out.println("TERCERO");

                      ShipmentInOutLine minoutline = OBDal.getInstance()
                          .get(ShipmentInOutLine.class, data[i].mInoutlineId);

                      if (minoutline != null) {
                        OrderLine orderline = minoutline.getSalesOrderLine();
                        invoice.setScoDeliveryviarule(
                            minoutline.getShipmentReceipt().getDeliveryMethod());
                        if (orderline != null) {
                          Order order = orderline.getSalesOrder();
                          invoice.setPaymentMethod(order.getPaymentMethod());
                          invoice.setPaymentTerms(order.getPaymentTerms());
                          invoice.setSalesRepresentative(order.getSalesRepresentative());
                          invoiceCopied = true;
                        }
                      }
                    }
                    ...
                    if (data[i].cOrderlineId != null) {
                      ...
                      if (orderline != null) {
                        ...
                        invoice.setPaymentTerms(order.getPaymentTerms());
                        invoice.setSalesRepresentative(order.getSalesRepresentative());
                        invoice.setSsaIssummaryprint(order.isSsaIssummaryprintInv());
                        invoice.setScoDeliveryviarule(order.getDeliveryMethod());
                        invoice.setSPRExcluirDeProyecto(order.isSPRExcluirDeProyecto());
                        invoice.setSprReference(order.getSPRReferenciaDeProyecto());
                        invoiceCopied = true;
                      }

                    }
                  }
                } else if (invoice != null && invoice.getInvoiceLineList().size() > 0) {

                  if (data[i].mInoutlineId != null) {
                    ShipmentInOutLine minoutline = OBDal.getInstance().get(ShipmentInOutLine.class,
                        data[i].mInoutlineId);
              ...
              /* Getting Invoice Ref on Credit Memo for Return Material */
              if (!invoiceReferenceFound) {

                if ("SCOARINVOICERETURNMAT".compareTo(invoice.getSCOSpecialDocType()) == 0) {
                  if (strType.equals("SHIPMENT")) {
                    if (data[i].mInoutlineId != null) {
                      ShipmentInOutLine minoutline = OBDal.getInstance()
                          .get(ShipmentInOutLine.class, data[i].mInoutlineId);
                      ...
                      if (minoutline != null) {
                        OrderLine orderline = minoutline.getSalesOrderLine();
                        if (orderline != null
                            && orderline.getSalesOrder().getSsaInvoiceref() != null) {
                          strInvRef = orderline.getSalesOrder().getSsaInvoiceref().getId();
                          invoiceReferenceFound = true;
                        }
                      }
                    }
                  } else {
        ...
        if (!strInvRef.equals("")) {
          try {
            final int total = CreateFromInvoiceData.deleteSCO_InvoiceRef_ID(conn, this, strKey,
                strInvRef);
            if (total == 0) {
              CreateFromInvoiceData.updateSCO_InvoiceRef_ID(conn, this, strInvRef, strKey);
            }
          } catch (final ServletException ex) {
            myMessage = Utility.translateError(this, vars, vars.getLanguage(), ex.getMessage());
            releaseRollbackConnection(conn);
        ...
        if (strType.equals("INVOICE")) {
          ...
          if (isSOTrx.equals("Y"))
            data = CreateFromShipmentData.selectFromPOUpdate(conn, this, vars.getLanguage(),
                ids[k]); // selectFromPOUpdateSOTrx
          else
            data = CreateFromShipmentData.selectFromPOUpdate(conn, this, vars.getLanguage(),
                ids[k]);
        }
        if (data != null) {
          strPOLineId = data[0].cOrderlineId;
          for (int i = 0; i < data.length; i++) {
            ...
            final String strMovementqty = vars
                .getRequiredNumericParameter("inpmovementqty" + strLineId);
            ...
            if ("Y".equals(strbreakdown)) {
              ...
              while (qty.compareTo(ZERO) > 0) {
                ...
                try {
                  ...
                  CreateFromShipmentData.insert(conn, this, strSequence, strKey, vars.getClient(),
                      ...
                      data[i].explode, data[i].isorder, data[i].isexcludedfromproject);
              ...
              final String strSequence = SequenceIdData.getUUID();
              try {
                // Edit BY Vafaster: Boton Copy FROM: Guia(Proveedor) Si no se selecciona StorageBin
                // en las lineas seleccionadas, entonces se busca en el warehouse asociado el campo
                // em_swa_receipt_locator

                if (strType.equals("PO") || strType.equals("INVOICE")) {
                  if (isSOTrx.equals("N")) {
                    if (strLocator.equals("")) {
                      ShipmentInOut shipment = OBDal.getInstance().get(ShipmentInOut.class, strKey);
                      if (shipment != null) {
                        if (shipment.getWarehouse().getSwaReceiptLocator() != null)
                          // strLocator = shipment.getWarehouse().getSwaReceiptLocator().getId();
                          strLocator = shipment.getWarehouse().getSwaReceiptLocator().getId();
                      }
                    }
                  }
          ...
          // Update m_inout
          try {
            OrderLine orderline = OBDal.getInstance().get(OrderLine.class, strPOLineId);
            if (orderline != null && orderline.getSalesOrder().isSalesTransaction() == false) {
              String strPODocumentno = orderline.getSalesOrder().getDocumentNo();
              CreateFromShipmentData.updateInout(conn, this, strPODocumentno, strKey);
            }
          } catch (final ServletException ex) {
            myMessage = Utility.translateError(this, vars, vars.getLanguage(), ex.getMessage());
            releaseRollbackConnection(conn);
