...
import pe.com.unifiedgo.accounting.data.SCODoctypeareaLine;
...
public abstract class AcctServer {
  ...
  public boolean isReceiptForConversion = true;// by PALK
  ...
  public String C_Invoice_ID_Ref = "";
  ...
  public String otherSaveError = "";
  public String orgArea = "SN";
  public String SCO_Area_ID = "";
  public String typeDoc = "00";
  public String defaultCorrelative = "";
  ...
  /** Document Status */
  public static final String STATUS_PeriodAreaClosed = "sco_pA";

  public static final String STATUS_Migrated = "sco_M";
  /** Document Status */
  ...
  public static final String TABLEID_SWithHolding = "251100CF447643CFAAFCF7F6A4975DA8";
  public static final String TABLEID_Billofexchange = "29987BE302E04781BE3868E573123BDE";
  public static final String TABLEID_InternalDoc = "A64BF5FB928C4EC1BACC023D6DC87F3C";
  public static final String TABLEID_Prepayment = "135FAE2571DB4028A90D5CAA6FAC154C";

  public static final String TABLEID_GLJournal = "224";
  public static final String TABLEID_GLJournalline = "226";
  ...
  public static final String ACCTTYPE_InvGain = "52";
  ...
  /*
   * public static boolean isInternalWithReeval(SCOInternalDoc doc){ if(doc.getCuenta()==null ||
   * doc.getCuenta().length()<2) return false;
   * 
   * String cuenta = doc.getCuenta().substring(0,2);
   * 
   * if(cuenta.equals("12") || cuenta.equals("13") || cuenta.equals("14") || cuenta.equals("16") ||
   * cuenta.equals("17") || cuenta.equals("19") || cuenta.equals("41") || cuenta.equals("42") ||
   * cuenta.equals("43") || cuenta.equals("44") || cuenta.equals("45") || cuenta.equals("46") ||
   * cuenta.equals("47") || cuenta.equals("49")) return true; return false; }
   */
  ...
  // para tipo de cambio
  public static Date getDateForInvoiceReference(Invoice inv) {

    // for debit memos return the debit memo dateacct
    String specialdoctype = inv.getTransactionDocument().getScoSpecialdoctype() != null
        ? inv.getTransactionDocument().getScoSpecialdoctype()
        : "";
    if (specialdoctype.compareTo("SCOARDEBITMEMO") == 0) {
      if (inv.isSalesTransaction())
        return inv.getInvoiceDate();
      else
        return inv.getScoNewdateinvoiced();
    }
    if (specialdoctype.compareTo("SCOAPINVOICE") == 0) {
      SCRComboItem podoctypecmb = inv.getScoPodoctypeComboitem();
      if (podoctypecmb != null) {
        if (podoctypecmb.getSearchKey().compareTo("tabla10_08") == 0
            || podoctypecmb.getSearchKey().compareTo("tabla10_88") == 0
            || podoctypecmb.getSearchKey().compareTo("tabla10_98") == 0) {
          if (inv.isSalesTransaction())
            return inv.getInvoiceDate();
          else
            return inv.getScoNewdateinvoiced();
    ...
    // LETRA DSCTO
    if (inv.getScoBoeType() != null && inv.getScoBoeType().equals("SCO_DIS")) {
      List<SCOBoetodiscLine> boetodislines = inv.getSCOBoetodiscLineInvoicerefIDList();
      if (boetodislines.size() != 0) {// nueva
        // buscar canje
        ...
        return discpayment.getPaymentDate();
      } else {
        // es migrada, no importa a la fecha de la letra (despues cuadran el asiento)
        return inv.getInvoiceDate();
      }
    }
    // FACTORED INV
    /*
     * else if(inv.getScoFactinvType()!=null && inv.getScoFactinvType().equals("SCO_FACT")){
     * List<SCOFactinvLine> factinvlines = inv.getSCOFactinvLineInvoicerefIDList();
     * if(factinvlines.size() != 0){//nueva //buscar canje de factoring FIN_Payment factpayment =
     * null; for(int ii=0; ii<factinvlines.size(); ii++){ factpayment =
     * factinvlines.get(ii).getSCOFactoringinvoice().getPayment(); if(factpayment !=null) break; }
     * return factpayment.getPaymentDate(); }else{ //es migrada, no importa a la fecha del factoring
     * inv (despues cuadran el asiento) return inv.getAccountingDate(); } }
     */

    while (invPtr.getScoInvoiceref() != null) {
      invPtr = invPtr.getScoInvoiceref();
      rr--;
      if (rr < 0)
        return null;
    }

    if (invPtr.getScoManualinvoicerefdate() != null)
      return invPtr.getScoManualinvoicerefdate();

    if (invPtr.isSalesTransaction())
      return invPtr.getInvoiceDate();
    else
      return invPtr.getScoNewdateinvoiced();
  }
  ...
  public void run(VariablesSecureApp vars, String strDateFrom, String strDateTo)
    ...
    try {
      ...
      // GET THE FIRST WHERE CLAUSE FOR THIS AD_CLIENT
      for (AcctSchema as : m_as) {
        additionalWhereClause = AcctServerData.selectBackgroundWhereClause(connectionProvider,
            as.getC_AcctSchema_ID(), AD_Table_ID);
      ...
      data = AcctServerData.select(connectionProvider, tableName, strDateColumn, AD_Client_ID,
          strOrgs, additionalWhereClause, strDateFrom, strDateTo, 0,
  ...
  public static ConversionRateDoc getConversionRateDocStatic(String table_ID, String record_ID,
    ...
    if (record_ID != null) {
      ...
      } else if (table_ID.equals(TABLEID_SWithHolding)) {
        SCOSwithholdingReceipt withd = OBDal.getInstance().get(SCOSwithholdingReceipt.class,
            record_ID);
        if (withd.getMultiplyexchangerate() != null && curFrom_ID.equals("100")
            && curTo_ID.equals("308")) {
          ConversionRateDoc crd = new ConversionRateDoc();
          crd.setCurrency(OBDal.getInstance().get(Currency.class, "100"));// dolares
          crd.setToCurrency(OBDal.getInstance().get(Currency.class, "308"));// soles
          crd.setRate(withd.getMultiplyexchangerate());
          return crd;
        }
      } else {
        return null;
  ...
  public static AcctServer get(String AD_Table_ID, String AD_Client_ID, String AD_Org_ID,
    ...
    Table table = OBDal.getInstance().get(Table.class, AD_Table_ID);
  ...
  private boolean post(FieldProvider[] data, boolean force, VariablesSecureApp vars,
    ...
    // COMMENT ADDED BY WALTER: Here we already have the correct AD_Org_ID = document and the
    // reloadActSchemaArray sets the correct accSchema array.
    ...
    // ///ADDED BY UGO: AFTER FACT CREATION MAP THE ACCFACT USING SCO_POSTING_MAPPING

    org.openbravo.model.financialmgmt.accounting.coa.AcctSchema[] acctschemas = new org.openbravo.model.financialmgmt.accounting.coa.AcctSchema[m_as.length];
    for (int i = 0; i < m_as.length; i++) {
      org.openbravo.model.financialmgmt.accounting.coa.AcctSchema acctschema = OBDal.getInstance()
          .get(org.openbravo.model.financialmgmt.accounting.coa.AcctSchema.class,
              m_as[i].m_C_AcctSchema_ID);
      acctschemas[i] = acctschema;
    }

    // SCO_Posting_Mapping implementation
    for (int _i = 0; _i < m_as.length; _i++) {

      FactLine[] factLines = m_fact[_i].getLines();
      for (int i = 0; i < factLines.length; i++) {
        if (factLines[i].getM_acct() == null)
          continue;

        if (factLines[i].doc_Ad_Org_ID == null) {
          factLines[i].doc_Ad_Org_ID = "";
        }
        if (factLines[i].doc_C_Doctype_ID == null) {
          factLines[i].doc_C_Doctype_ID = "";
        }
        if (factLines[i].doc_C_Currency_ID == null) {
          factLines[i].doc_C_Currency_ID = "";
        }

        System.out.println("factLines[i].doc_Ad_Org_ID:" + factLines[i].doc_Ad_Org_ID
            + " - factLines[i].doc_C_Doctype_ID:" + factLines[i].doc_C_Doctype_ID
            + " - factLines[i].doc_C_Currency_ID:" + factLines[i].doc_C_Currency_ID
            + " - AD_Org_ID:" + AD_Org_ID);

        String replacetoAcctId = AcctServer.getMappedAccount(conn,
            acctschemas[_i].getClient().getId(), acctschemas[_i].getId(),
            factLines[i].getM_acct().C_ValidCombination_ID, factLines[i].doc_Ad_Org_ID,
            factLines[i].doc_C_Doctype_ID, factLines[i].doc_C_Currency_ID);

        if (!replacetoAcctId.isEmpty()) {
          // found match replace
          Account new_acc = new Account(conn, replacetoAcctId);
          FactLine factLine = m_fact[_i].getLines()[i];
          factLine.setAccount(m_as[_i], new_acc);
        }
    ...
    try {
      ...
      if (Status.equals(AcctServer.STATUS_Posted)) {
        if (m_fact != null && m_fact.length != 0) {
          ...
          System.out.println("HELLOOOOOOOOO: AD_Table_ID:" + AD_Table_ID + " - Record_ID:"
              + Record_ID + " - C_DocType_ID:" + C_DocType_ID + " - SCO_AREA_ID:" + SCO_Area_ID);

          if (orgArea.equals("SN")) {

            if (SCO_Area_ID != null && !SCO_Area_ID.isEmpty()) {
              orgArea = SCO_Area_ID;
            } else {

              System.out.println("HELLOOOOO ENTER 1");
              DocumentType docType = OBDal.getInstance().get(DocumentType.class, C_DocType_ID);
              if (docType != null) {
                System.out.println("HELLOOOOO ENTER 2");

                String keyword = "";
                if (AD_Table_ID.equals(TABLEID_Invoice)) {
                  Invoice inv = OBDal.getInstance().get(Invoice.class, Record_ID);
                  if (inv != null) {
                    boolean istaxdeferred = inv.isScoIsmanualTranstax() != null
                        ? inv.isScoIsmanualTranstax()
                        : false;

                    if (istaxdeferred) {
                      keyword = "IMPUESTODIFERIDO";
                    } else {
                      keyword = inv.getScoPodoctypeComboitem() != null
                          ? inv.getScoPodoctypeComboitem().getSearchKey()
                          : "";
                    }
                  }
                  ...
                  if (payment != null) {
                    keyword = payment.getAccount().getName();
                  }
                  ...
                  FIN_FinaccTransaction trx = OBDal.getInstance().get(FIN_FinaccTransaction.class,
                      Record_ID);
                  if (trx != null) {
                    keyword = trx.getAccount().getName();
                  }
                } else if (AD_Table_ID.equals(TABLEID_Reconciliation)) {
                  FIN_Reconciliation rec = OBDal.getInstance().get(FIN_Reconciliation.class,
                      Record_ID);
                  if (rec != null) {
                    keyword = rec.getAccount().getName();
                  }

                } else if (AD_Table_ID.equals(TABLEID_GLJournal)) {
                  GLJournal journal = OBDal.getInstance().get(GLJournal.class, Record_ID);
                  if (journal != null) {
                    List<Invoice> definvs = journal.getInvoiceEMScoMtrtaxGljournalIDList();
                    if (definvs.size() > 0) {
                      Invoice inv = definvs.get(0);
                      keyword = inv.getScoPodoctypeComboitem() != null
                          ? inv.getScoPodoctypeComboitem().getSearchKey()
                          : "";
                    }
                ...
                System.out.println("HELLOOOOO ENTER 3 keyword:" + keyword);

                boolean areafound = false;
                if (!keyword.isEmpty()) {
                  OBCriteria<SCODoctypeareaLine> dtarea_obc = OBDal.getInstance()
                      .createCriteria(SCODoctypeareaLine.class);
                  dtarea_obc
                      .add(Restrictions.eq(SCODoctypeareaLine.PROPERTY_DOCUMENTTYPE, docType));
                  dtarea_obc.add(Restrictions.eq(SCODoctypeareaLine.PROPERTY_KEYWORD, keyword));
                  dtarea_obc.setMaxResults(1);

                  List<SCODoctypeareaLine> dtarealines = dtarea_obc.list();
                  if (dtarealines.size() > 0) {
                    orgArea = dtarealines.get(0).getOrganizationArea().getId();
                    areafound = true;
                    System.out.println("HELLOOOOO ENTER 4 orgArea:" + orgArea);
                ...
                if (!areafound) {
                  orgArea = docType.getScoArea().getId();
                  System.out.println("HELLOOOOO ENTER 5 orgArea:" + orgArea);
                }

                System.out.println("HELLOOOOO ENTER 6 orgArea:" + orgArea);

              } else {
                System.out.println("HELLOOOOO ENTER 7 orgArea:" + orgArea);

              }
            }
            /*
             * if (docType != null) orgArea = docType.getScoArea().getId(); else {
             * OBCriteria<DocumentType> documentCrit = OBDal.getInstance()
             * .createCriteria(DocumentType.class); documentCrit.add(Restrictions.eq(
             * org.openbravo.model.common.enterprise.DocumentType.PROPERTY_DOCUMENTCATEGORY,
             * this.DocumentType)); documentCrit.add(Restrictions.eq(
             * org.openbravo.model.common.enterprise.DocumentType.PROPERTY_CLIENT,
             * OBDal.getInstance().get(Client.class, this.AD_Client_ID)));
             * documentCrit.add(Restrictions.sqlRestriction("AD_ISORGINCLUDED('" + this.AD_Org_ID +
             * "', ad_org_id, '" + this.AD_Client_ID + "') > -1"));
             * documentCrit.setFilterOnReadableClients(false);
             * documentCrit.setFilterOnReadableOrganization(false); boolean foundArea = false; for
             * (int i = 0; i < documentCrit.list().size(); i++) { DocumentType dt =
             * documentCrit.list().get(i); List<SCOAreaschema> lsSchemas =
             * dt.getScoArea().getSCOAreaschemaList();
             * 
             * for (int j = 0; j < lsSchemas.size(); j++) { if
             * (lsSchemas.get(j).getGeneralLedger().getId() .equals(m_as[0].m_C_AcctSchema_ID)) {
             * orgArea = dt.getScoArea().getId(); foundArea = true; break; } } if (foundArea) break;
             * } }
             */
          }

          if (typeDoc.equals("00")) {
            DocumentType docType = OBDal.getInstance().get(DocumentType.class, C_DocType_ID);
            ...
            if (docType != null) {
              ...
              if (specialDocType == null) {
                ...
                typeDoc = OBDal.getInstance().get(Invoice.class, Record_ID)
                    .getScoPodoctypeComboitem().getCode();
              } else if (specialDocType.equals("SCOARINVOICE")
                  || specialDocType.equals("SCOARBOEINVOICE")) {
                typeDoc = "01";
              } else if (specialDocType.equals("SCOARINVOICERETURNMAT")
                  || specialDocType.equals("SCOARCREDITMEMO")
          ...
          for (int i = 0; i < m_fact.length; i++) {
            ...
            String result = "N";
            if (m_fact[i] != null) {
              result = m_fact[i].save(con, conn, vars, orgArea, typeDoc, defaultCorrelative);
            }
            ...
            if (result.equals("Y"))
              ...
              unlock(conn);
              if (result.equals("AREASCHEMACLOSED")) {
                Status = AcctServer.STATUS_PeriodAreaClosed;
              } else {
                Status = AcctServer.STATUS_Error;
              }
            }
      ...
      if (e.getMessage().equals("AREASCHEMACLOSED")) {
        Status = AcctServer.STATUS_PeriodAreaClosed;
      } else {
        Status = AcctServer.STATUS_Error;
      }
  ...
  private final boolean save(ConnectionProvider conn, String strUser) {
    int no = 0;
    try {
      // UGO MODIFIED: do not update the audit fields when posting the document
      // no = AcctServerData.updateSave(conn, tableName, Status, strUser, Record_ID);
      no = AcctServerData.updateSaveNoAudit(conn, tableName, Status, Record_ID);
    } catch (ServletException e) {
      log4j.warn(e);
      e.printStackTrace();
    }
  ...
  public boolean loadDocument(FieldProvider[] data, boolean force, ConnectionProvider conn,
    ...
    SCO_Area_ID = data[0].getField("SCO_Area_ID");
  ...
  private final String postLogic(int index, ConnectionProvider conn, Connection con,
    ...
    // rejectUnconvertible
    /*
     * if (!isConvertible(m_as[index], conn)) return STATUS_NotConvertible;
     */
    ...
    for (int i = 0; i < m_fact[index].getLines().length; i++) {
      System.out.println("comb3:" + m_fact[index].getLines()[i].getM_acct().combination);
    }
    ...
    for (int i = 0; i < m_fact[index].getLines().length; i++) {
      System.out.println("comb2:" + m_fact[index].getLines()[i].getM_acct().combination);
    }
   ...
   */
  /*
   * public boolean isConvertible(AcctSchema acctSchema, ConnectionProvider conn) throws
   * ServletException { // No Currency in document if (NO_CURRENCY.equals(C_Currency_ID)) { // if
   * (log4j.isDebugEnabled()) // log4j.debug("AcctServer - isConvertible (none) - " + DocumentNo);
   * return true; } // Get All Currencies Vector<Object> set = new Vector<Object>();
   * set.addElement(C_Currency_ID); for (int i = 0; p_lines != null && i < p_lines.length; i++) {
   * String currency = p_lines[i].m_C_Currency_ID; if (currency != null && !currency.equals(""))
   * set.addElement(currency); }
   * 
   * // just one and the same if (set.size() == 1 &&
   * acctSchema.m_C_Currency_ID.equals(C_Currency_ID)) { // if (log4j.isDebugEnabled()) log4j.debug
   * // ("AcctServer - isConvertible (same) Cur=" + C_Currency_ID + " - " // + DocumentNo); return
   * true; } boolean convertible = true; for (int i = 0; i < set.size() && convertible == true; i++)
   * { // if (log4j.isDebugEnabled()) log4j.debug // ("AcctServer - get currency"); String currency
   * = (String) set.elementAt(i); if (currency == null) currency = ""; // if
   * (log4j.isDebugEnabled()) log4j.debug // ("AcctServer - currency = " + currency); if
   * (!currency.equals(acctSchema.m_C_Currency_ID)) { // if (log4j.isDebugEnabled()) log4j.debug //
   * ("AcctServer - get converted amount (init)"); String amt = ""; OBQuery<ConversionRateDoc>
   * conversionQuery = null; int conversionCount = 0; if (AD_Table_ID.equals(TABLEID_Invoice)) {
   * conversionQuery = OBDal.getInstance().createQuery( ConversionRateDoc.class, "invoice = '" +
   * Record_ID + "' and currency='" + currency + "' and toCurrency='" + acctSchema.m_C_Currency_ID +
   * "'"); } else if (AD_Table_ID.equals(TABLEID_Payment)) { conversionQuery =
   * OBDal.getInstance().createQuery( ConversionRateDoc.class, "payment = '" + Record_ID +
   * "' and currency='" + currency + "' and toCurrency='" + acctSchema.m_C_Currency_ID + "'"); }
   * else if (AD_Table_ID.equals(TABLEID_Transaction)) { conversionQuery =
   * OBDal.getInstance().createQuery( ConversionRateDoc.class, "financialAccountTransaction = '" +
   * Record_ID + "' and currency='" + currency + "' and toCurrency='" + acctSchema.m_C_Currency_ID +
   * "'"); } if (conversionQuery != null) { conversionCount = conversionQuery.count(); } if
   * (conversionCount > 0) { List<ConversionRateDoc> conversionRate = conversionQuery.list();
   ...
   * currencyCrit.setProjection(Projections.max(Currency.PROPERTY_STANDARDPRECISION)); Long
   * precision = 0L; if (currencyCrit.count() > 0) { List<Currency> toCurrency =
   * currencyCrit.list(); precision = toCurrency.get(0).getStandardPrecision(); } BigDecimal
   * convertedAmount = new BigDecimal("1") .multiply(conversionRate.get(0).getRate()); amt =
   * convertedAmount.setScale(precision.intValue(), RoundingMode.HALF_UP).toString(); } if
   * (("").equals(amt) || amt == null) amt = getConvertedAmt("1", currency,
   * acctSchema.m_C_Currency_ID, DateAcct, acctSchema.m_CurrencyRateType, AD_Client_ID, AD_Org_ID,
   * conn); // if (log4j.isDebugEnabled()) log4j.debug // ("get converted amount (end)"); if (amt ==
   * null || ("").equals(amt)) { convertible = false;
   * log4j.warn("AcctServer - isConvertible NOT from " + currency + " - " + DocumentNo); } else if
   * (log4j.isDebugEnabled()) log4j.debug("AcctServer - isConvertible from " + currency); } } // if
   * (log4j.isDebugEnabled()) log4j.debug // ("AcctServer - isConvertible=" + convertible +
   * ", AcctSchemaCur=" + // acctSchema.m_C_Currency_ID + " - " + DocumentNo); return convertible; }
   * // isConvertible
   */
  ...
  // by PALK añadido isReceipt para saber si usar T/C compra o venta
  public static String getConvertedAmtGeneral(String Amt, String CurFrom_ID, String CurTo_ID,
      String ConvDate, String RateType, String client, String org, ConnectionProvider conn,
      boolean isReceipt, boolean isClose) {

    System.out.println("Payment es receipt?: " + isReceipt);

    System.out.println("AcctServer - getConvertedAmountGeneral - starting method - Amt : " + Amt
        + " - CurFrom_ID : " + CurFrom_ID + " - CurTo_ID : " + CurTo_ID + "- ConvDate: " + ConvDate
        + " - RateType:" + RateType + " - client:" + client + "- org:" + org + " - isReceipt:"
        + isReceipt + " - isClose:" + isClose);

    if (log4j.isDebugEnabled())
      log4j.debug("AcctServer - getConvertedAmountGeneral - starting method - Amt : " + Amt
          + " - CurFrom_ID : " + CurFrom_ID + " - CurTo_ID : " + CurTo_ID + "- ConvDate: "
          + ConvDate + " - RateType:" + RateType + " - client:" + client + "- org:" + org);
    ...
    try {
      ...
      boolean isUseSalesTCOnly = SCO_Utils.isUseSalesTCOnly(conn, client);
      ...
      if (isClose) {
        if (isReceipt) { // venta
          data = AcctServerData.currencyCloseConvert(conn, Amt, CurFrom_ID, CurTo_ID, ConvDate,
              client, org);
        } else { // compra
          data = AcctServerData.currencyCloseConvert(conn, "1000000000", CurTo_ID, CurFrom_ID,
              ConvDate, client, org);

          String converted = data[0].converted;
          Currency curr = OBDal.getInstance().get(Currency.class, CurTo_ID);
          data[0].converted = (new BigDecimal(Amt)).multiply(new BigDecimal("1000000000"))
              .divide(new BigDecimal(converted), curr.getStandardPrecision().intValue(),
                  RoundingMode.HALF_UP)
              .toString();
        }
      } else {
        if (isReceipt || isUseSalesTCOnly) { // venta
          data = AcctServerData.currencyConvert(conn, Amt, CurFrom_ID, CurTo_ID, ConvDate, RateType,
              client, org);
        } else { // compra
          data = AcctServerData.currencyConvert(conn, "1000000000", CurTo_ID, CurFrom_ID, ConvDate,
              RateType, client, org);
  ...
  public static String getConvertedAmt(String Amt, String CurFrom_ID, String CurTo_ID,
    ...
    return getConvertedAmtGeneral(Amt, CurFrom_ID, CurTo_ID, ConvDate, RateType, client, org, conn,
        isReceipt, false);
  } // getConvertedAmt

  public static String getConvertedAmtPrecision(String Amt, String CurFrom_ID, String CurTo_ID,
      String ConvDate, String RateType, String client, String org, int precision,
    ...
    try {
      ...
      if (RateType == null || RateType.equals(""))
        RateType = "S";
      data = AcctServerData.currencyConvertPrecision(conn, Amt, CurFrom_ID, CurTo_ID, ConvDate,
          RateType, client, org, String.valueOf(precision));
    } catch (ServletException e) {
      log4j.warn(e);
      e.printStackTrace();
    }
  ...
  public final Account getAccount(String AcctType, AcctSchema as, ConnectionProvider conn) {
    ...
    try {
      ...
      } else if (AcctType.equals(ACCTTYPE_InvGain)) {
        /** Inventory Accounts */
        data = AcctServerData.selectWGainAcct(conn, M_Warehouse_ID, as.getC_AcctSchema_ID());
  ...
  public final String getCostcenter(String AcctType, AcctSchema as, ConnectionProvider conn) {
    AcctServerData[] data = null;
    try {
      if (AcctType.equals(ACCTTYPE_ConvertChargeDefaultAmt)) {
        data = AcctServerData.selectConvertChargeDefaultAmtCostCenter(conn,
            as.getC_AcctSchema_ID());
      } else if (AcctType.equals(ACCTTYPE_WriteOff)) {
        data = AcctServerData.selectWriteOffCostCenter(conn, as.getC_AcctSchema_ID());
      } else if (AcctType.equals(ACCTTYPE_WriteOffDefault)) {
        data = AcctServerData.selectWriteOffCostCenter(conn, as.getC_AcctSchema_ID());
      }
    } catch (ServletException e) {
  ...
  public final Account getAccountBPartner(String cBPartnerId, AcctSchema as, boolean isReceipt,
    ...
    try {
      ...
      String FIN_SpecialPaymentMethod = "";
      String C_SpecialDocType = "";
      String BoeType = "";
      String FactinvType = "";
      Invoice inv = null;
      if (C_Invoice_ID_Ref != "") {// payment
        inv = OBDal.getInstance().get(Invoice.class, C_Invoice_ID_Ref);
        FIN_SpecialPaymentMethod = inv.getPaymentMethod().getScoSpecialmethod();
        C_SpecialDocType = inv.getDocumentType().getScoSpecialdoctype();
        BoeType = inv.getScoBoeType();
        FactinvType = inv.getScoFactinvType();
      } else if (OBDal.getInstance().get(Invoice.class, Record_ID) != null) {// invoice
        inv = OBDal.getInstance().get(Invoice.class, Record_ID);
        FIN_SpecialPaymentMethod = inv.getPaymentMethod().getScoSpecialmethod();
        C_SpecialDocType = OBDal.getInstance().get(DocumentType.class, C_DocType_ID)
      ...
      if (isReceipt) {
        ...
        if (customerAccounts != null && customerAccounts.size() > 0 && !isPrepayment) {

          if (C_SpecialDocType != null && C_SpecialDocType.equals("SCOARBOEINVOICE")
              && BoeType != null && BoeType.equals("SCO_DIS")
              && customerAccounts.get(0).getSCODiscountBillOfExchangeAcc() != null)
            strValidCombination = customerAccounts.get(0).getSCODiscountBillOfExchangeAcc().getId();
          else if (C_SpecialDocType != null && C_SpecialDocType.equals("SCOARBOEINVOICE")
              && BoeType != null && BoeType.equals("SCO_COL")
              && customerAccounts.get(0).getScoLetracolAcct() != null)
            strValidCombination = customerAccounts.get(0).getScoLetracolAcct().getId();
          else if (C_SpecialDocType != null && C_SpecialDocType.equals("SCOARBOEINVOICE")
              && customerAccounts.get(0).getSCOBillOfExchangeAcc() != null)
            strValidCombination = customerAccounts.get(0).getSCOBillOfExchangeAcc().getId();

          else if (C_SpecialDocType != null && C_SpecialDocType.equals("SCOARDEBITMEMO")
              && customerAccounts.get(0).getSCODebitNoteAcc() != null)
            strValidCombination = customerAccounts.get(0).getSCODebitNoteAcc().getId();

          // FOR FACTINV ONLY IN PAYMENT TABLE
          else if (AD_Table_ID.equals(TABLEID_Payment) && C_SpecialDocType != null
              && (C_SpecialDocType.equals("SCOARINVOICE") || C_SpecialDocType.equals("SCOARTICKET"))
              && FactinvType != null && FactinvType.equals("SCO_FACT")
              && customerAccounts.get(0).getScoFactinvAcct() != null)
            strValidCombination = customerAccounts.get(0).getScoFactinvAcct().getId();
          else if (AD_Table_ID.equals(TABLEID_Payment) && C_SpecialDocType != null
              && (C_SpecialDocType.equals("SCOARINVOICE") || C_SpecialDocType.equals("SCOARTICKET"))
              && FactinvType != null && FactinvType.equals("SCO_COL")
              && customerAccounts.get(0).getScoColfactinvAcct() != null)
            strValidCombination = customerAccounts.get(0).getScoColfactinvAcct().getId();

          else if (customerAccounts.get(0).getCustomerReceivablesNo() != null)
        ...
        if (vendorAccounts != null && vendorAccounts.size() > 0 && C_SpecialDocType != null
            && C_SpecialDocType.equals("SCOAPBOEINVOICE")
            && vendorAccounts.get(0).getScoPoletraAcct() != null) {
          strValidCombination = vendorAccounts.get(0).getScoPoletraAcct().getId();
        } else if (vendorAccounts != null && vendorAccounts.size() > 0 && C_SpecialDocType != null
            && C_SpecialDocType.equals("SCOAPLOANINVOICE") && inv != null
            && inv.getSCOLoanDocList().size() > 0) {
          SCOLoanDoc loandoc = inv.getSCOLoanDocList().get(0);
          final StringBuilder whereClausegl = new StringBuilder();
          whereClausegl.append(" as glacc ");
          whereClausegl
              .append(" where glacc.gLItem.id = '" + loandoc.getLoanGlitem().getId() + "'");
          whereClausegl.append(" and glacc.accountingSchema.id = '" + as.m_C_AcctSchema_ID + "'");
          final OBQuery<GLItemAccounts> obqParametersgl = OBDal.getInstance()
              .createQuery(GLItemAccounts.class, whereClausegl.toString());
          obqParametersgl.setFilterOnReadableClients(false);
          obqParametersgl.setFilterOnReadableOrganization(false);
          final List<GLItemAccounts> glaccs = obqParametersgl.list();
          if (glaccs.size() > 0) {
            strValidCombination = glaccs.get(0).getGlitemDebitAcct().getId();
          }
        } else if (vendorAccounts != null && vendorAccounts.size() > 0 && C_SpecialDocType != null
            && C_SpecialDocType.equals("SCOAPLOANINVOICE")
            && vendorAccounts.get(0).getScoPoloanAcct() != null) {
          strValidCombination = vendorAccounts.get(0).getScoPoloanAcct().getId();
        } else if (vendorAccounts != null && vendorAccounts.size() > 0
  ...
  public boolean checkDocuments(String dateFrom, String dateTo) throws ServletException {
    ...
    for (int i = 0; i < docTypes.length; i++) {
      ...
      AcctServerData data = AcctServerData.selectDocumentsDates(connectionProvider, strDateColumn,
          tableName, AD_Client_ID, strorgs, docTypes[i].name, additionalWhereClause, dateFrom,
          dateTo);
  ...
  public void setMessageResult(ConnectionProvider conn, VariablesSecureApp vars, String _strStatus,
    ...
    } else if (strStatus.equals(STATUS_PeriodAreaClosed)) {
      strTitle = "@SCO_PeriodAreaClosed@";
  ...
  public ConversionRateDoc getConversionRateDoc(String table_ID, String record_ID,
    ...
    System.out.println("tableId:" + table_ID + " " + record_ID + " " + curFrom_ID + " " + curTo_ID);
    ...
    if (record_ID != null) {
      if (table_ID.equals(TABLEID_Invoice)) {
        ...
        FIN_Payment pay = OBDal.getInstance().get(FIN_Payment.class, record_ID);
        if (pay != null)
          ...
          docRateCriteria.add(Restrictions.eq(ConversionRateDoc.PROPERTY_ID, "----"));
        ...
        System.out
            .println("tableId:" + table_ID + " " + record_ID + " " + curFrom_ID + " " + curTo_ID);
        ...
        if (withd.getMultiplyexchangerate() != null && curFrom_ID.equals("100")
          ...
          return null;
      } else if (table_ID.equals(TABLEID_Billofexchange)) {

        System.out
            .println("tableId:" + table_ID + " " + record_ID + " " + curFrom_ID + " " + curTo_ID);

        SCOBillofexchange boe = OBDal.getInstance().get(SCOBillofexchange.class, record_ID);
        if (boe.getOtherCurrency().getId().equals(curFrom_ID)
            && boe.getCurrency().getId().equals(curTo_ID)) {
          ConversionRateDoc crd = new ConversionRateDoc();
          crd.setCurrency(boe.getOtherCurrency());
          crd.setToCurrency(boe.getCurrency());
          crd.setRate(boe.getConvertRate());
          return crd;
        } else
          return null;
      } else if (table_ID.equals(TABLEID_Prepayment)) {
        docRateCriteria.add(Restrictions.eq(ConversionRateDoc.PROPERTY_SCOPREPAYMENT,
            OBDal.getInstance().get(SCOPrepayment.class,
                OBDal.getInstance().get(SCOPrepayment.class, record_ID).getId())));
      } else if (table_ID.equals(TABLEID_GLJournalline)) {

        System.out
            .println("tableId:" + table_ID + " " + record_ID + " " + curFrom_ID + " " + curTo_ID);

        GLJournalLine jline = OBDal.getInstance().get(GLJournalLine.class, record_ID);
        GLJournal gljournal = jline.getJournalEntry();

        if (gljournal.getCurrency().getId().equals(curFrom_ID)
            && gljournal.getAccountingSchema().getCurrency().getId().equals(curTo_ID)) {
          ConversionRateDoc crd = new ConversionRateDoc();
          crd.setCurrency(gljournal.getCurrency());
          crd.setToCurrency(gljournal.getAccountingSchema().getCurrency());
          crd.setRate(jline.getRate());
          return crd;
        } else
          return null;
      } else {
   ...
   * 
   * @param record_ID
   * @param dttConversionTo
   * @return true signfica usar TC venta, false TC compra
   */
  public static boolean getTCenVigencia(Invoice invoice, String dttConversionTo) {
     ...
     * .getProperty("dateFormat.java"); final SimpleDateFormat dateFormat = new
     * SimpleDateFormat(strDateFormat);
     * 
     * Date conversionTo = null; try{ conversionTo = dateFormat.parse(dttConversionTo);
     * }catch(Exception ex){}
     * 
     * if(invoice==null) return true;
     * 
     * Calendar aCalendar = Calendar.getInstance(); aCalendar.setTime(conversionTo); // add -1 month
     * to current month aCalendar.add(Calendar.MONTH, -1); // set DATE to 1, so first date of
     * previous month aCalendar.set(Calendar.DATE, 1); aCalendar.set(Calendar.DATE,
     * aCalendar.getActualMaximum(Calendar.DAY_OF_MONTH));
     * 
     * Date lastDateOfPreviousMonth = aCalendar.getTime();
     * 
     * 
     * if(getDateForInvoiceReference(invoice).before(lastDateOfPreviousMonth)){ return false; }
     * 
     * return true;
     */
  }

  public static Date getTCenVigenciaFecha(Invoice invoice, String dttConversionTo,
      Vector<String> vIsClosing) {

    String strDateFormat = OBPropertiesProvider.getInstance().getOpenbravoProperties()
        .getProperty("dateFormat.java");
    final SimpleDateFormat dateFormat = new SimpleDateFormat(strDateFormat);

    Date conversionTo = null;
    try {
      conversionTo = dateFormat.parse(dttConversionTo);
    } catch (Exception ex) {
    }

    Calendar aCalendar = Calendar.getInstance();
    aCalendar.setTime(conversionTo);
    ...
    if (invoice.getAccountingDate().before(lastDateOfPreviousMonth)) {
      if (vIsClosing != null)
        vIsClosing.add("Y");
      return lastDateOfPreviousMonth;
    }

    return getDateForInvoiceReference(invoice);
  }

  public BigDecimal convertAmount(BigDecimal _amount, boolean isReceipt, String dateAcct,
      String table_ID, String record_ID, String currencyIDFrom, String currencyIDTo, DocLine line,
      AcctSchema as, Fact fact, String Fact_Acct_Group_ID, String seqNo, ConnectionProvider conn,
      Vector<BigDecimal> vTcOut) throws ServletException {
    return convertAmount(_amount, isReceipt, dateAcct, table_ID, record_ID, currencyIDFrom,
        currencyIDTo, line, as, fact, Fact_Acct_Group_ID, seqNo, conn, true, vTcOut);
  }
  ...
  public BigDecimal convertAmount(BigDecimal _amount, boolean isReceipt, String dateAcct,
      String table_ID, String record_ID, String currencyIDFrom, String currencyIDTo, DocLine line,
      AcctSchema as, Fact fact, String Fact_Acct_Group_ID, String seqNo, ConnectionProvider conn,
      boolean bookDifferences, Vector<BigDecimal> vOutTC) throws ServletException {
    BigDecimal amtDiff = BigDecimal.ZERO;
    if (_amount == null || _amount.compareTo(BigDecimal.ZERO) == 0) {
      vOutTC.add(BigDecimal.ONE);
      return _amount;
    }
    ...
    if (AD_Table_ID.equals(TABLEID_Invoice)) {
      ...
      conversionDate = dateFormat.format(getDateForInvoiceReference(invoice));
    } else if (AD_Table_ID.equals(TABLEID_Payment)) {
      FIN_Payment payment = OBDal.getInstance().get(FIN_Payment.class, Record_ID);
    ...
    if (conversionRateCurrentDoc != null) {
      ...
      if (conversionRateCurrentDoc != null) {
        ...
        amtTo = new BigDecimal(getConvertedAmt(_amount.toString(), currencyIDFrom, currencyIDTo,
            conversionDate, "", AD_Client_ID, AD_Org_ID, conn, isReceipt));

        status = 3;
      }
    }

    String conversionTo = conversionDate;
    boolean changeTC = false;

    Date dttConversionTo = null;
    try {
      dttConversionTo = dateFormat.parse(conversionTo);
    } catch (Exception e) {
    }
    if (table_ID.equals(TABLEID_Invoice)) {
      Invoice invoice = OBDal.getInstance().get(Invoice.class, record_ID);
      ...
      Calendar aCalendar = Calendar.getInstance();
      aCalendar.setTime(dttConversionTo);
      // add -1 month to current month
      aCalendar.add(Calendar.MONTH, -1);
      // set DATE to 1, so first date of previous month
      aCalendar.set(Calendar.DATE, 1);
    ...
    if (conversionRateDoc != null && record_ID != null) {
      ...
      if (conversionRateDoc != null) {
        ...
        boolean tmpIsReceipt = isReceipt;
        if (changeTC) {// solo a tipo de cambio de venta
          isReceipt = true;
        }
        ...
        isReceipt = tmpIsReceipt;
      }

    }

    vOutTC.add(amtFrom.divide(_amount, 16, RoundingMode.HALF_UP));
    ...
    // amtDiff = amtDiff.add(calculateMultipleRatesDifferences(_amount, currencyIDFrom,
    // currencyIDTo,
    // line, conn)); //no le encuentro sentido
    System.out.println("X1 2???:" + amtDiff);
    ...
    if (bookDifferences) {
      if ((!isReceipt && amtDiff.compareTo(BigDecimal.ZERO) == 1)
        ...
        FactLine fl = fact.createLine(line,
            getAccount(AcctServer.ACCTTYPE_ConvertGainDefaultAmt, as, conn), currencyIDTo, "",
        ...
        FactLine fl = fact.createLine(line,
            getAccount(AcctServer.ACCTTYPE_ConvertChargeDefaultAmt, as, conn), currencyIDTo,
            amtDiff.abs().toString(), "", Fact_Acct_Group_ID, seqNo, DocumentType, conn);
  ...
  public BigDecimal convertAmountInDiferentTCFromDate(BigDecimal _amount, boolean isReceipt,
      String dateAcct, String convDate, String table_ID, String record_ID, String currencyIDFrom,
      String currencyIDTo, DocLine line, AcctSchema as, Fact fact, String Fact_Acct_Group_ID,
      String seqNo, ConnectionProvider conn, boolean bookDifferences, boolean tcFrom, boolean tcTo,
      Vector<BigDecimal> vOutTC) throws ServletException {
    return convertAmountInDiferentTCFromDate(_amount, isReceipt, dateAcct, convDate, table_ID,
        record_ID, currencyIDFrom, currencyIDTo, line, as, fact, Fact_Acct_Group_ID, seqNo, conn,
        bookDifferences, tcFrom, tcTo, vOutTC, false);
  }

  // tcfrom tcto true tc venta, false tc compra @
  public BigDecimal convertAmountInDiferentTCFromDate(BigDecimal _amount, boolean isReceipt,
      String dateAcct, String convDate, String table_ID, String record_ID, String currencyIDFrom,
      String currencyIDTo, DocLine line, AcctSchema as, Fact fact, String Fact_Acct_Group_ID,
      String seqNo, ConnectionProvider conn, boolean bookDifferences, boolean tcFrom, boolean tcTo,
      Vector<BigDecimal> vOutTC, boolean invRedondeo) throws ServletException {
    BigDecimal amtDiff = BigDecimal.ZERO;
    if (_amount == null || _amount.compareTo(BigDecimal.ZERO) == 0) {
      vOutTC.add(BigDecimal.ONE);
      return _amount;
    ...
    System.out.println("amtTo: " + amtTo + " " + _amount + " " + currencyIDFrom + " " + currencyIDTo
        + " " + conversionDate + " " + tcTo);
    ...
    if (AD_Table_ID.equals(TABLEID_Invoice)) {
      ...
      System.out.println("R1e2");
      FIN_Payment payment = OBDal.getInstance().get(FIN_Payment.class, Record_ID);
      conversionDate = dateFormat.format(payment.getPaymentDate());
    } else if (AD_Table_ID.equals(TABLEID_Transaction)
        || AD_Table_ID.equals(TABLEID_Reconciliation)) {
      System.out.println("R1e3");
      String transactionID = Record_ID;
      // When TableID= Reconciliation info is loaded from transaction
      if (AD_Table_ID.equals(AcctServer.TABLEID_Reconciliation)
          && line instanceof DocLine_FINReconciliation) {
    ...
    if (conversionRateCurrentDoc != null) {
      amtTo = applyRate(_amount, conversionRateCurrentDoc, true);
      status = 1;
      System.out.println("amtTo1: " + amtTo + " " + _amount + " " + currencyIDFrom + " "
          + currencyIDTo + " " + conversionDate + " " + tcTo);
      ...
      if (conversionRateCurrentDoc != null) {
        amtTo = applyRate(_amount, conversionRateCurrentDoc, false);
        System.out.println("amtTo2: " + amtTo + " " + _amount + " " + currencyIDFrom + " "
            + currencyIDTo + " " + conversionDate + " " + tcTo);
        ...
        amtTo = new BigDecimal(getConvertedAmt(_amount.toString(), currencyIDFrom, currencyIDTo,
            convDate, "", AD_Client_ID, AD_Org_ID, conn, tcTo));
        System.out.println("amtTo3: " + amtTo + " " + _amount + " " + currencyIDFrom + " "
            + currencyIDTo + " " + conversionDate + " " + tcTo);
    ...
    if (conversionRateDoc != null && record_ID != null) {
      ...
      if (conversionRateDoc != null) {
        ...
        amtFrom = new BigDecimal(getConvertedAmt(_amount.toString(), currencyIDFrom, currencyIDTo,
            conversionDate, "", AD_Client_ID, AD_Org_ID, conn, tcFrom));
    ...
    System.out.println("???:" + amtTo + " " + amtFrom + " " + conversionTo + " " + conversionDate);
    ...
    /*
     * amtDiff = amtDiff.add(calculateMultipleRatesDifferences(_amount, currencyIDFrom,
     * currencyIDTo, line, conn)); //no le encuentro sentido
     */
    System.out.println("X2 2???:" + amtDiff);
    ...
    if (bookDifferences) {
      ...
      if ((!isReceipt && amtDiff.compareTo(BigDecimal.ZERO) == 1)
        ...
        fl.forceDescription = false;
        /*
         * }else{ FactLine fl = fact.createLine(line,
         * getAccount(AcctServer.ACCTTYPE_ConvertGainDefaultAmt, as, conn), currencyIDTo,
         * amtDiff.abs().toString(), "", Fact_Acct_Group_ID, seqNo, DocumentType, conn);
         * fl.descripcion = "PERDIDA POR DIFERENCIA DE CAMBIO";
         * fl.setCostcenter(getCostcenter(AcctServer.ACCTTYPE_ConvertChargeDefaultAmt, as, conn));}
         ...
         * }else{ FactLine fl = fact.createLine(line,
         * getAccount(AcctServer.ACCTTYPE_ConvertChargeDefaultAmt, as, conn), currencyIDTo, "",
         * amtDiff.abs().toString(), Fact_Acct_Group_ID, seqNo, DocumentType, conn); fl.descripcion
  ...
  public BigDecimal convertAmount(BigDecimal _amount, boolean isReceipt, String dateAcct,
      String table_ID, String record_ID, String currencyIDFrom, String currencyIDTo, DocLine line,
      AcctSchema as, Fact fact, String Fact_Acct_Group_ID, String seqNo, ConnectionProvider conn,
      boolean bookDifferences, boolean tcFrom, boolean tcTo, Vector<BigDecimal> vOutTC)
      throws ServletException {
    return convertAmount(_amount, isReceipt, dateAcct, table_ID, record_ID, currencyIDFrom,
        currencyIDTo, line, as, fact, Fact_Acct_Group_ID, seqNo, conn, bookDifferences, tcFrom,
        tcTo, vOutTC, false, false);
  }
  ...
  public BigDecimal convertAmount(BigDecimal _amount, boolean isReceipt, String dateAcct,
      ...
      AcctSchema as, Fact fact, String Fact_Acct_Group_ID, String seqNo, ConnectionProvider conn,
      boolean bookDifferences, boolean tcFrom, boolean tcTo, Vector<BigDecimal> vOutTC,
      boolean invRedondeo, boolean avoidTCierre) throws ServletException {

    System.out.print("enter convertAmount: " + "_amount:" + _amount + " - isReceipt:" + isReceipt
        + " - dateAcct:" + dateAcct + " - table_ID:" + table_ID + " - record_ID:" + record_ID
        + " - currencyIDFrom:" + currencyIDFrom + " - currencyIDTo:" + currencyIDTo + " - tcFrom:"
        + tcFrom + " - tcTo:" + tcTo + " - invRedondeo:" + invRedondeo + " - avoidTCierre:"
        + avoidTCierre);
    if (vOutTC.size() > 0) {
      System.out.println(" - vOutTC.get(0):" + vOutTC.get(0));
    } else {
      System.out.println(" - vOutTC.get(0): NULL");
    }

    if (currencyIDFrom.equals(currencyIDTo)) {
      System.out.println("convertAmount currency from=to currencyIDFrom:" + currencyIDFrom
          + " - currencyIDTo:" + currencyIDTo + " returning _amount:" + _amount);
      vOutTC.add(BigDecimal.ONE);
      return _amount;
    }
    ...
    boolean isUseSalesTCOnly = SCO_Utils.isUseSalesTCOnly(conn, AD_Client_ID);
    ...
    if (conversionRateCurrentDoc != null) {
      ...
      if (conversionRateCurrentDoc != null) {
        ...
        amtTo = new BigDecimal(getConvertedAmt(_amount.toString(), currencyIDFrom, currencyIDTo,
            conversionDate, "", AD_Client_ID, AD_Org_ID, conn, tcTo));
        System.out.println("amtTo3: " + amtTo + " " + _amount + " " + currencyIDFrom + " "
            + currencyIDTo + " " + conversionDate + " " + tcTo);

        if (AD_Table_ID.equals(TABLEID_Payment)) {
          FIN_Payment payment = OBDal.getInstance().get(FIN_Payment.class, Record_ID);
          String currencyFAcct = payment.getAccount().getCurrency().getId();

          if (!currencyIDFrom.equals(currencyIDTo) && !currencyFAcct.equals(currencyIDTo)) {
            // Special case no ratedocfound -> currencyIDFrom and currencyFAcct are not equal to
            // currencyIDTo(PEN)
            DocLine_FINPayment pline = (DocLine_FINPayment) line;
            BigDecimal paymentAmount = new BigDecimal(pline.getPaymentAmount());
            BigDecimal docamount = new BigDecimal(pline.getAmount());

            BigDecimal leftConvertRate = paymentAmount.divide(docamount, 16, RoundingMode.HALF_UP);
            BigDecimal middleConvertRate = BigDecimal.ONE;
            if (tcTo || isUseSalesTCOnly) {
              // T/C Venta
              middleConvertRate = getConvertionRate(currencyFAcct, currencyIDTo, conversionDate, "",
                  AD_Client_ID, AD_Org_ID, conn);
            } else {
              // T/C Compra
              AcctServerData[] tmpdata = AcctServerData.currencyConvert(conn, "1000000000",
                  currencyIDTo, currencyFAcct, conversionDate, "", AD_Client_ID, AD_Org_ID);
              middleConvertRate = new BigDecimal("1000000000")
                  .divide(new BigDecimal(tmpdata[0].converted), 16, RoundingMode.HALF_UP);
            }

            BigDecimal newConvertRate = leftConvertRate.multiply(middleConvertRate);
            amtTo = (new BigDecimal(_amount.toString()).multiply(newConvertRate));
          }
        }
    ...
    // CHECK SPECIAL CASE PURCHASE DETRACC amtTo Should be converted using invoice convert rate not
    // the payment documentdoc
    if (AD_Table_ID.equals(TABLEID_Payment) && table_ID.equals(TABLEID_Invoice)) {
      DocLine_FINPayment pline = (DocLine_FINPayment) line;
      if (pline.getIsDetraccPayment().compareTo("Y") == 0
          && pline.getIsReceipt().compareTo("N") == 0
          && currencyIDFrom.compareTo(currencyIDTo) != 0) {
        Invoice invoice = OBDal.getInstance().get(Invoice.class, record_ID);
        String invConversionDate = dateFormat.format(getDateForInvoiceReference(invoice));
        // Use Sales T/C
        amtTo = new BigDecimal(getConvertedAmt(_amount.toString(), currencyIDFrom, currencyIDTo,
            invConversionDate, "", AD_Client_ID, AD_Org_ID, conn, tcFrom));
      }
    }

    String conversionTo = conversionDate;
    boolean isDttCierre = false;
    Date dttConversionTo = null;
    try {
      dttConversionTo = dateFormat.parse(conversionTo);
    } catch (Exception e) {
    ...
    if (table_ID.equals(TABLEID_Invoice)) {
      ...
      // modified by UGO
      boolean skipreeval = invoice.isScoSkipreeval() != null ? invoice.isScoSkipreeval() : false;
      if (!avoidTCierre && invoice.getAccountingDate().before(firstDateOfPreviousMonth)
          && !skipreeval) {
      ...
      System.out.println("1 new covnersionDate:" + conversionDate);

      /*
       * } else if (table_ID.equals(TABLEID_InternalDoc)) { SCOInternalDoc invoice =
       * OBDal.getInstance().get(SCOInternalDoc.class, record_ID); conversionDate =
       * dateFormat.format(invoice.getInvoiceDate());
       * 
       * Calendar aCalendar = Calendar.getInstance(); aCalendar.setTime(dttConversionTo); // add -1
       * month to current month aCalendar.add(Calendar.MONTH, -1); // set DATE to 1, so first date
       * of previous month aCalendar.set(Calendar.DATE, 1); aCalendar.set(Calendar.DATE,
       * aCalendar.getActualMaximum(Calendar.DAY_OF_MONTH));
       * 
       * Date firstDateOfPreviousMonth = aCalendar.getTime(); // modified by UGO boolean skipreeval
       * = invoice.isScoSkipreeval() != null ? invoice.isScoSkipreeval() : false; if
       * (invoice.getInvoiceDate().before(firstDateOfPreviousMonth) && !skipreeval) {
       * 
       * // set actual maximum date of previous month aCalendar.set(Calendar.DATE,
       * aCalendar.getActualMaximum(Calendar.DAY_OF_MONTH)); // read it Date lastDateOfPreviousMonth
       * = aCalendar.getTime();
       * 
       * conversionDate = dateFormat.format(lastDateOfPreviousMonth); isDttCierre = true; }
       */
    } else if (table_ID.equals(TABLEID_Payment)) {
      FIN_Payment payment = OBDal.getInstance().get(FIN_Payment.class, record_ID);
      if (payment != null)
        ...
        SCOPrepayment prep = OBDal.getInstance().get(SCOPrepayment.class, record_ID);
        if (prep != null) {
          conversionDate = dateFormat.format(prep.getGeneratedDate());
        } else {
          ScoRendicioncuentas rendcuentas = OBDal.getInstance().get(ScoRendicioncuentas.class,
              record_ID);
          if (rendcuentas != null) {
            conversionDate = dateFormat.format(rendcuentas.getDategen());
          }
        }
      }
      ...
      SCOBillofexchange boe = OBDal.getInstance().get(SCOBillofexchange.class, record_ID);
      conversionDate = dateFormat.format(boe.getAccountingDate());
    } else if (table_ID.equals(TABLEID_GLJournalline)) {
      GLJournalLine jline = OBDal.getInstance().get(GLJournalLine.class, record_ID);
      conversionDate = dateFormat.format(jline.getJournalEntry().getAccountingDate());
    }

    // DttCierre has preference over conversionratedoc
    if (isDttCierre) {
      amtFrom = new BigDecimal(getConvertedAmtGeneral(_amount.toString(), currencyIDFrom,
          currencyIDTo, conversionDate, "", AD_Client_ID, AD_Org_ID, conn, tcFrom, true));
    } else if (conversionRateDoc != null && record_ID != null) {
      amtFrom = applyRate(_amount, conversionRateDoc, true);
     ...
     * currencyIDTo, line, conn)); //no le encuentro sentido
     */
    System.out.println("X3 2???:" + amtDiff);

    Currency currencyTo = OBDal.getInstance().get(Currency.class, currencyIDTo);
    amtDiff = amtDiff.setScale(currencyTo.getStandardPrecision().intValue(),
        BigDecimal.ROUND_HALF_EVEN);
    if (bookDifferences) {
      System.out.println("isReceipt:" + isReceipt + " - amtDiff:" + amtDiff);
      if (invRedondeo)
        isReceipt = !isReceipt;
      System.out.println("2 isReceipt:" + isReceipt + " - amtDiff:" + amtDiff);

      if ((!isReceipt && amtDiff.compareTo(BigDecimal.ZERO) == 1)
          || (isReceipt && amtDiff.compareTo(BigDecimal.ZERO) == -1)) {
        // if(amtDiff.compareTo(BigDecimal.ZERO)==1){
        System.out.println("ENTER GANANCIA POR DIFERENCIA DE CAMBIO:"
            + getAccount(AcctServer.ACCTTYPE_ConvertGainDefaultAmt, as, conn).combination);
        FactLine fl = fact.createLine(line,
            getAccount(AcctServer.ACCTTYPE_ConvertGainDefaultAmt, as, conn), currencyIDTo, "",
            amtDiff.abs().toString(), Fact_Acct_Group_ID, seqNo, DocumentType, conn);
        fl.descripcion = "GANANCIA POR DIFERENCIA DE CAMBIO";
        ...
        System.out.println("ENTER PERDIDA POR DIFERENCIA DE CAMBIO:"
            + getAccount(AcctServer.ACCTTYPE_ConvertChargeDefaultAmt, as, conn).combination);
  ...
  public BigDecimal convertAmount(BigDecimal _amount, boolean isReceipt, String dateAcct,
      String table_ID, String record_ID, String currencyIDFrom, String currencyIDTo, DocLine line,
      AcctSchema as, Fact fact, String Fact_Acct_Group_ID, String seqNo, ConnectionProvider conn,
      boolean bookDifferences, BigDecimal finalAmount, boolean tcFrom, boolean tcTo,
      Vector<BigDecimal> vOutTC) throws ServletException {
    BigDecimal amtDiff = BigDecimal.ZERO;
    if (_amount == null || _amount.compareTo(BigDecimal.ZERO) == 0) {
      vOutTC.add(BigDecimal.ONE);
    ...
    System.out.println("X4 2???:" + amtDiff);
    if (bookDifferences) {
      if ((!isReceipt && amtDiff.compareTo(BigDecimal.ZERO) == 1)
          || (isReceipt && amtDiff.compareTo(BigDecimal.ZERO) == -1)) {
        FactLine fl = fact.createLine(line,
      ...
      if (docType.equals(EXCHANGE_DOCTYPE_Invoice) && recordId != null) {
        ...
        if (!reversedCriteria.list().isEmpty()) {
          ...
          data = AcctServerData.currencyConvert(conn, Amt, CurFrom_ID, CurTo_ID, ConvDate, RateType,
              client, org);
          docRateCriteria
              .add(Restrictions.eq(ConversionRateDoc.PROPERTY_INVOICE, OBDal.getInstance().get(
                  Invoice.class, reversedCriteria.list().get(0).getReversedInvoice().getId())));
        } else {
        ...
        APRM_FinaccTransactionV a = OBDal.getInstance().get(APRM_FinaccTransactionV.class,
            recordId);
   ...
   * String currencyIDTo, DocLine line, ConnectionProvider conn) { // _amount * ((TrxRate * //
   * AccountingRateCurrencyFromCurrencyTo)-AccountingRateCurrencyDocCurrencyTo) String
   * conversionDate = DateAcct; String strDateFormat =
   * OBPropertiesProvider.getInstance().getOpenbravoProperties() .getProperty("dateFormat.java");
   * final SimpleDateFormat dateFormat = new SimpleDateFormat(strDateFormat); // Calculate
   * accountingRateCurrencyFromCurrencyTo BigDecimal accountingRateCurrencyFromCurrencyTo =
   * BigDecimal.ONE; if (!currencyIDFrom.equals(currencyIDTo)) { ConversionRateDoc
   * conversionRateCurrentDoc = getConversionRateDoc(AD_Table_ID, Record_ID, currencyIDFrom,
   * currencyIDTo); if (AD_Table_ID.equals(AcctServer.TABLEID_Reconciliation) && line instanceof
   * DocLine_FINReconciliation) { String transactionID = ((DocLine_FINReconciliation)
   * line).getFinFinAccTransactionId(); FIN_FinaccTransaction transaction =
   * OBDal.getInstance().get(FIN_FinaccTransaction.class, transactionID); conversionDate =
   * dateFormat.format(transaction.getDateAcct()); conversionRateCurrentDoc =
   * getConversionRateDoc(TABLEID_Transaction, transaction.getId(), currencyIDFrom, currencyIDTo); }
   * if (conversionRateCurrentDoc != null) { accountingRateCurrencyFromCurrencyTo =
   * conversionRateCurrentDoc.getRate(); } else { // I try to find a reversal rate for the doc, if
   * exists i apply it reversal as well if (AD_Table_ID.equals(AcctServer.TABLEID_Reconciliation) &&
   * line instanceof DocLine_FINReconciliation) { String transactionID =
   * ((DocLine_FINReconciliation) line).getFinFinAccTransactionId(); FIN_FinaccTransaction
   * transaction = OBDal.getInstance().get(FIN_FinaccTransaction.class, transactionID);
   * conversionRateCurrentDoc = getConversionRateDoc(TABLEID_Transaction, transaction.getId(),
   * currencyIDTo, currencyIDFrom); } else { conversionRateCurrentDoc =
   * getConversionRateDoc(AD_Table_ID, Record_ID, currencyIDTo, currencyIDFrom); } if
   * (conversionRateCurrentDoc != null) { accountingRateCurrencyFromCurrencyTo =
   * BigDecimal.ONE.divide( conversionRateCurrentDoc.getRate(), MathContext.DECIMAL64); } else {
   ...
   * 
   * // Calculate accountingRateCurrencyFromCurrencyTo BigDecimal
   * accountingRateCurrencyDocCurrencyTo = BigDecimal.ONE; if (!C_Currency_ID.equals(currencyIDTo))
   * { ConversionRateDoc conversionRateCurrentDoc = getConversionRateDoc(AD_Table_ID, Record_ID,
   * C_Currency_ID, currencyIDTo); if (AD_Table_ID.equals(AcctServer.TABLEID_Reconciliation) && line
   * instanceof DocLine_FINReconciliation) { String transactionID = ((DocLine_FINReconciliation)
   * line).getFinFinAccTransactionId(); FIN_FinaccTransaction transaction =
   * OBDal.getInstance().get(FIN_FinaccTransaction.class, transactionID); conversionDate =
   * dateFormat.format(transaction.getTransactionDate()); conversionRateCurrentDoc =
   * getConversionRateDoc(TABLEID_Transaction, transaction.getId(), C_Currency_ID, currencyIDTo); }
   * if (conversionRateCurrentDoc != null) { accountingRateCurrencyDocCurrencyTo =
   * conversionRateCurrentDoc.getRate(); } else { // I try to find a reversal rate for the doc, if
   * exists i apply it reversal as well if (AD_Table_ID.equals(AcctServer.TABLEID_Reconciliation) &&
   * line instanceof DocLine_FINReconciliation) { String transactionID =
   * ((DocLine_FINReconciliation) line).getFinFinAccTransactionId(); FIN_FinaccTransaction
   * transaction = OBDal.getInstance().get(FIN_FinaccTransaction.class, transactionID);
   * conversionRateCurrentDoc = getConversionRateDoc(TABLEID_Transaction, transaction.getId(),
   * currencyIDTo, C_Currency_ID); } else { conversionRateCurrentDoc =
   * getConversionRateDoc(AD_Table_ID, Record_ID, currencyIDTo, C_Currency_ID); } if
   * (conversionRateCurrentDoc != null) { accountingRateCurrencyDocCurrencyTo =
   * BigDecimal.ONE.divide( conversionRateCurrentDoc.getRate(), MathContext.DECIMAL64); } else {
   * accountingRateCurrencyDocCurrencyTo = getConvertionRate(C_Currency_ID, currencyIDTo,
   * conversionDate, "", AD_Client_ID, AD_Org_ID, conn); } } } // Calculate transaction rate
   * BigDecimal trxRate = BigDecimal.ONE; if (!C_Currency_ID.equals(currencyIDFrom)) { if
   * (AD_Table_ID.equals(TABLEID_Payment)) { FIN_Payment payment =
   * OBDal.getInstance().get(FIN_Payment.class, Record_ID); trxRate =
   * payment.getFinancialTransactionConvertRate(); } else if
   * (AD_Table_ID.equals(TABLEID_Transaction) || AD_Table_ID.equals(TABLEID_Reconciliation)) {
   * String transactionID = Record_ID; // When TableID = Reconciliation info is loaded from
   * transaction if (AD_Table_ID.equals(AcctServer.TABLEID_Reconciliation) && line instanceof
   * DocLine_FINReconciliation) { transactionID = ((DocLine_FINReconciliation)
   * line).getFinFinAccTransactionId(); } FIN_FinaccTransaction transaction =
   * OBDal.getInstance().get(FIN_FinaccTransaction.class, transactionID); trxRate =
   * transaction.getForeignConversionRate(); } } Currency currencyFrom =
   * OBDal.getInstance().get(Currency.class, currencyIDTo);
   * 
   * return _amount.multiply( trxRate.multiply(accountingRateCurrencyDocCurrencyTo).subtract(
  ...
  public BigDecimal getPaymentDetailAmount(List<FIN_PaymentDetail> paymentDetails,
    ...
    if (psi == null && pso == null) {
      return paymentDetails.get(currentPaymentDetailIndex).getScoPaymentamount();
    }
    ...
    if ((psi != null && psi.equals(ps)) || pso == null) {
      ...
      if (pso == null && psdNext != null && psdNext.getInvoicePaymentSchedule() == psi
        ...
        return paymentDetails.get(currentPaymentDetailIndex).getScoPaymentamount()
            .add(paymentDetails.get(currentPaymentDetailIndex - 1).getScoPaymentamount());
      } else {
        return paymentDetails.get(currentPaymentDetailIndex).getScoPaymentamount();
      }
    } else {
  ...
  public HashMap<String, BigDecimal> getPaymentDetailWriteOffAndAmount(
    ...
    amountAndWriteOff.put("amountpay",
        paymentDetails.get(currentPaymentDetailIndex).getScoPaymentamount());
    ...
    if (psi != null || pso != null) {
      ...
      if ((psi != null && psi.equals(ps)) || pso == null) {
        ...
        List<FIN_PaymentScheduleDetail> lpsdNext = (currentPaymentDetailIndex == paymentDetails
            .size() - 1) ? null
                : paymentDetails.get(currentPaymentDetailIndex + 1)
                    .getFINPaymentScheduleDetailList();

        List<FIN_PaymentScheduleDetail> lpsdPrevious = (currentPaymentDetailIndex == 0) ? null
            : paymentDetails.get(currentPaymentDetailIndex - 1).getFINPaymentScheduleDetailList();

        FIN_PaymentScheduleDetail psdNext = lpsdNext != null && lpsdNext.size() > 0
            ? lpsdNext.get(0)
            : null;
        FIN_PaymentScheduleDetail psdPrevious = lpsdPrevious != null && lpsdPrevious.size() > 0
            ? lpsdPrevious.get(0)
            : null;
        ...
        if (pso == null && psdNext != null && psdNext.getInvoicePaymentSchedule() == psi
          ...
          amountAndWriteOff.put("amountpay", null);
  ...
  public static String getMappedAccount(ConnectionProvider conn, String AD_Client_ID,
      String Replaceto_Acctschema_ID, String Replacefrom_Acct_ID, String Replacefrom_Org_ID,
      String Replacefrom_Doctype_ID, String Replacefrom_Currency_ID) throws ServletException {

    System.out.println(
        "enter getMappedAccount: AD_Client_ID:" + AD_Client_ID + " - Replaceto_Acctschema_ID:"
            + Replaceto_Acctschema_ID + " - Replacefrom_Acct_ID:" + Replacefrom_Acct_ID
            + " - Replacefrom_Org_ID:" + Replacefrom_Org_ID + " - Replacefrom_Doctype_ID:"
            + Replacefrom_Doctype_ID + " - Replacefrom_Currency_ID:" + Replacefrom_Currency_ID);

    String scoPostingMappingId = AcctServerData.selectPostingMapping(conn, AD_Client_ID,
        Replaceto_Acctschema_ID, Replacefrom_Acct_ID);

    System.out.println("scoPostingMappingId:" + scoPostingMappingId);
    if (scoPostingMappingId == null)
      return "";
    if (scoPostingMappingId.isEmpty())
      return "";

    String replacetoAcctId = AcctServerData.selectReplaceto_acct_FromPostingMapping(conn,
        AD_Client_ID, scoPostingMappingId, Replacefrom_Org_ID, Replacefrom_Doctype_ID,
        Replacefrom_Currency_ID);
    System.out.println("replacetoAcctId:" + replacetoAcctId);

    if (replacetoAcctId == null)
      return "";
    if (replacetoAcctId.isEmpty())
      return "";

    return replacetoAcctId;
  }

  public static Invoice getInvoicerefForMapping(Invoice invoice) {
    Invoice invref = invoice;
    String invspecialdoctype = invoice.getTransactionDocument().getScoSpecialdoctype();
    boolean isapdebit = false;
    if (invspecialdoctype.equals("SCOAPINVOICE") && invoice.getScoPodoctypeComboitem() != null) {
      String pocmbvalue = invoice.getScoPodoctypeComboitem().getSearchKey();
      if (pocmbvalue.equals("tabla10_98") || pocmbvalue.equals("tabla10_08")
          || pocmbvalue.equals("tabla10_88")) {
        isapdebit = true;
      }
    }
    if ((invspecialdoctype.equals("SCOARDEBITMEMO") || invspecialdoctype.equals("SCOARCREDITMEMO")
        || invspecialdoctype.equals("SCOARINVOICERETURNMAT")
        || invspecialdoctype.equals("SCOAPCREDITMEMO") || isapdebit) && !invoice.isScoIsmanualref()
        && invoice.getScoInvoiceref() != null) {
      invref = invoice.getScoInvoiceref();
    }
